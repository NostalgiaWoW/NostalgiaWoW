								 ...
								 dtRaycastHit* hit, dtPolyRef prevRef) const
{
	...
	while (curRef)
	{
		...
		else
		{
			status |= DT_BUFFER_TOO_SMALL;
			hit->pathCount = n;
			//#GIPERION: Return status immidiately. Even if we don't reach correct position, 
			// we can't allow spin infinitely. Sometimes, baked nav cards have mistake
			// Example: .go xyz -8031.19971 -5333.3335 10.25 1	And try aggro some of the mobs
			return status;
		}
