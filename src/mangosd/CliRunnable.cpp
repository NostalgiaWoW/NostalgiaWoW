...
bool ChatHandler::GetDeletedCharacterInfoList(DeletedInfoList& foundList, bool useName, std::string searchString)
{
    ...
    if (!searchString.empty())
    {
        if (useName)
        {
            ...
            if (isNumeric(searchString))
                resultChar = CharacterDatabase.PQuery("SELECT guid, deleteInfos_Name, deleteInfos_Account, deleteDate, level, class FROM characters WHERE deleteDate IS NOT NULL AND guid = %u LIMIT 0,50", uint32(atoi(searchString.c_str())));
            // search by name
            else
            {
                ...
                resultChar = CharacterDatabase.PQuery("SELECT guid, deleteInfos_Name, deleteInfos_Account, deleteDate, level, class FROM characters WHERE deleteDate IS NOT NULL AND deleteInfos_Name " _LIKE_ " " _CONCAT3_("'%%'", "'%s'", "'%%'") " LIMIT 0,50", searchString.c_str());
            }
        }
        else
        {
            ...
            if (isNumeric(searchString))
                resultChar = CharacterDatabase.PQuery("SELECT guid, deleteInfos_Name, deleteInfos_Account, deleteDate, level, class FROM characters WHERE deleteDate IS NOT NULL AND deleteInfos_Account = %u LIMIT 0,50", uint32(atoi(searchString.c_str())));
            ...
            else
            {
                ...
                accounts.pop_back();
                resultChar = CharacterDatabase.PQuery("SELECT guid, deleteInfos_Name, deleteInfos_Account, deleteDate, level, class FROM characters WHERE deleteDate IS NOT NULL AND deleteInfos_Account IN (%s) LIMIT 0,50", accounts.c_str());
            }
        }
    }
    else
        resultChar = CharacterDatabase.Query("SELECT guid, deleteInfos_Name, deleteInfos_Account, deleteDate, level, class FROM characters WHERE deleteDate IS NOT NULL LIMIT 0,50");
    ...
    if (resultChar)
    {
        do
        {
            ...
            info.characterLevel = fields[4].GetUInt32();
            uint8 characterClass = fields[5].GetUInt8();
            //save the class name string off instead of the numeric value 
            info.characterClass = GetClassName(characterClass, GetSessionDbcLocale());
    ...
    for (DeletedInfoList::const_iterator itr = foundList.begin(); itr != foundList.end(); ++itr)
    {
        ...
        if (!m_session)
            PSendSysMessage(LANG_CHARACTER_DELETED_LIST_LINE_CONSOLE,
                itr->lowguid, itr->name.c_str(), itr->characterClass.c_str(), itr->characterLevel, itr->accountName.empty() ? "<nonexistent>" : itr->accountName.c_str(),
...
bool ChatHandler::HandleAccountOnlineListCommand(char* args)
{
    ...
    //                                                 0   1         2        3        4
    QueryResult *result = LoginDatabase.PQuery("SELECT id, username, last_ip, gmlevel, expansion FROM account WHERE current_realm = %u", realmID);
