...
int MangosSocket<SessionType, SocketName, Crypt>::SendPacket(const WorldPacket& pct)
{
    ...
    if (((SocketName*)this)->iSendPacket(pct) == -1)
    {
        ...
        int result = m_PacketQueue.enqueue_tail(npct);
        if (result == -2)
        {
            delete npct;
            sLog.outError("SendPacket: Client exceed maximum out packet quota, disconnecting...");
            return -1;
        }
        if (result == -1)
        {
...
bool MangosSocket<SessionType, SocketName, Crypt>::iFlushPacketQueue()
{
    ...
    while (m_PacketQueue.dequeue_head(pct) == 0)
    {
        if (((SocketName*)this)->iSendPacket(*pct) == -1)
        {
            int result = m_PacketQueue.enqueue_head(pct);
            if (result == -2)
            {
                delete pct;
                sLog.outError("iFlushPacketQueue: Client exceed maximum out packet quota, disconnecting...");
                return false;
            }
            if (result == -1)
            {
...
template <typename SessionType, typename SocketName, typename Crypt>
void MangosSocket<SessionType, SocketName, Crypt>::SetMaximumPacketQueueSize(int InMaxQueueSize)
{
    m_PacketQueue.Init(InMaxQueueSize);
}
