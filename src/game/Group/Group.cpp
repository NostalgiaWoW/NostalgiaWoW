...
bool Group::Create(ObjectGuid guid, const char * name)
{
    ...
    if (!isBGGroup())
    {
        ...
        CharacterDatabase.PExecute("INSERT INTO groups(groupId,leaderGuid,mainTank,mainAssistant,lootMethod,looterGuid,lootThreshold,icon1,icon2,icon3,icon4,icon5,icon6,icon7,icon8,isRaid, masterLooterGuid) "
                                   ...
                                   isRaidGroup(), m_masterLooterGuid.GetCounter());
    }
...
bool Group::LoadGroupFromDB(Field* fields)
{
    ...
    // result = CharacterDatabase.Query("SELECT mainTank, mainAssistant, lootMethod, looterGuid, lootThreshold, icon1, icon2, icon3, icon4, icon5, icon6, icon7, icon8, isRaid, leaderGuid, groupId, masterLooterGuid FROM groups");
    ...
    m_masterLooterGuid = ObjectGuid(HIGHGUID_PLAYER, fields[16].GetUInt32());
            ...
            if (player && player->GetSession())
            {
                ...
                if (msg == EQUIP_ERR_OK)
                {
                    ...
                    if (Item* newItem = player->StoreNewItem(dest, roll->itemid, true, item->randomPropertyId))
                        player->OnReceivedItem(newItem, uint32(item->count), uint32(item->itemid), true);
                    else
                        player->OnReceivedItem(newItem, uint32(item->count), uint32(item->itemid), false);
                }
                else
                {
    ...
    for (member_citerator citr = m_memberSlots.begin(); citr != m_memberSlots.end(); ++citr)
    {
        ...
        if (GetMembersCount() - 1)
        {
            ...
            if (GetLootMethod() == MASTER_LOOT)
                data << m_masterLooterGuid;                       // looter guid
            else
                data << m_looterGuid;
    ...
    if (!PvP)
    {
                ...
                pGroupGuy->getLevel() <= not_gray_member_with_max_level->getLevel())
        {
            ...
            if (pVictim->GetTypeId() == TYPEID_UNIT)
                sXPLogMgr->FromNpcKill(pGroupGuy->GetObjectGuid(), pVictim->GetEntry(), itr_xp);
