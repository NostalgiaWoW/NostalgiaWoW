    ...
    for (AuctionEntryMap::const_iterator itr = AuctionsMap.begin(); itr != AuctionsMap.end(); ++itr)
    {
...

        {
            ...
            if (!query.wsearchedname.empty())
            {
                ...
                // searching of random property ids was added in 1.8.0 according to http://wowwiki.wikia.com/wiki/Patch_1.8.0
                if (sWorld.GetWowPatch() >= WOW_PATCH_108 && item->GetItemRandomPropertyId())
                {
                    if (auto const randomProperty = sItemRandomPropertiesStore.LookupEntry(item->GetItemRandomPropertyId()))
                    {
                        std::stringstream str;
                        str << name << " " << randomProperty->nameSuffix[loc_idx >= 0 ? loc_idx : 0];
                        name = str.str();
                    }
                }
