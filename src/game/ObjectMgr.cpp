...
{
    { LANG_ADDON,           0, 0,                       TEAM_NEUTRAL    },
    { LANG_UNIVERSAL,       0, 0,                       TEAM_NEUTRAL    },
    { LANG_ORCISH,        669, SKILL_LANG_ORCISH,       TEAM_HORDE      },
    { LANG_DARNASSIAN,    671, SKILL_LANG_DARNASSIAN,   TEAM_ALLIANCE   },
    { LANG_TAURAHE,       670, SKILL_LANG_TAURAHE,      TEAM_HORDE      },
    { LANG_DWARVISH,      672, SKILL_LANG_DWARVEN,      TEAM_ALLIANCE   },
    { LANG_COMMON,        668, SKILL_LANG_COMMON,       TEAM_ALLIANCE   },
    { LANG_DEMONIC,       815, SKILL_LANG_DEMON_TONGUE, TEAM_NEUTRAL    },
    { LANG_TITAN,         816, SKILL_LANG_TITAN,        TEAM_NEUTRAL    },
    { LANG_THALASSIAN,    813, SKILL_LANG_THALASSIAN,   TEAM_NEUTRAL    },
    { LANG_DRACONIC,      814, SKILL_LANG_DRACONIC,     TEAM_NEUTRAL    },
    { LANG_KALIMAG,       817, SKILL_LANG_OLD_TONGUE,   TEAM_NEUTRAL    },
    { LANG_GNOMISH,      7340, SKILL_LANG_GNOMISH,      TEAM_ALLIANCE   },
    { LANG_TROLL,        7341, SKILL_LANG_TROLL,        TEAM_HORDE      },
    { LANG_GUTTERSPEAK, 17737, SKILL_LANG_GUTTERSPEAK,  TEAM_HORDE      },
};

bool IsAllianceLanguageSkill(uint32 Skill)
{
    for (int i = 0; i < LANGUAGES_COUNT; ++i)
    {
        if (lang_description[i].team == TEAM_ALLIANCE && lang_description[i].skill_id == Skill)
            return true;
    }
    return false;
}

bool IsHordeLanguageSkill(uint32 Skill)
{
    for (int i = 0; i < LANGUAGES_COUNT; ++i)
    {
        if (lang_description[i].team == TEAM_HORDE && lang_description[i].skill_id == Skill)
            return true;
    }
    return false;
}
...
void ObjectMgr::LoadGroups()
{
    ...
    QueryResult *result = CharacterDatabase.Query("SELECT mainTank, mainAssistant, lootMethod, looterGuid, lootThreshold, icon1, icon2, icon3, icon4, icon5, icon6, icon7, icon8, isRaid, leaderGuid, groupId, masterLooterGuid FROM groups");
...
void ObjectMgr::LoadQuests()
{
    ...
    QueryResult *result = WorldDatabase.PQuery("SELECT entry, Method, ZoneOrSort, MinLevel, QuestLevel, Type, RequiredClasses, RequiredRaces, RequiredSkill, RequiredSkillValue,"
                          ...
                          //   123          124          125        126
                          "StartScript, CompleteScript, RewardXP, MaxLevel"
...
void ObjectMgr::LoadVendors(char const* tableName, bool isTemplates)
{
    ...
    QueryResult *result = WorldDatabase.PQuery("SELECT entry, item, maxcount, incrtime, conditionId FROM %s WHERE (item NOT IN (SELECT entry FROM forbidden_items WHERE (AfterOrBefore = 0 && patch <= %u) || (AfterOrBefore = 1 && patch >= %u)))", tableName, sWorld.GetWowPatch(), sWorld.GetWowPatch());
    if (!result)
    {
        BarGoLink bar(1);
    ...
    do
    {
        ...
        uint32 conditionId  = fields[4].GetUInt32();
...
std::shared_ptr<EasterStatistic> ObjectMgr::GetEasterStatisticForPlayer(ObjectGuid playerGuid)
{
    auto PlayerStatIter = m_mEasterStatistic.find(playerGuid);
    if (PlayerStatIter == m_mEasterStatistic.end())
    {
        std::shared_ptr<EasterStatistic> NewPtr(new EasterStatistic);
        m_mEasterStatistic[playerGuid] = NewPtr;
        return NewPtr;
    }
    else
    {
        return PlayerStatIter->second;
    }

    return std::shared_ptr<EasterStatistic>();
}

uint32 ObjectMgr::GetEasterGeneralSmallEggsCollected()
{
    uint32 SmallEggsCollected = 0;
    for (auto& StatIter : m_mEasterStatistic)
    {
        SmallEggsCollected += StatIter.second->SmallEggsCollected;
    }

    return SmallEggsCollected;
}

uint32 ObjectMgr::GetEasterGeneralSmallEggsWasted()
{
    uint32 SmallEggsWasted = 0;
    for (auto& StatIter : m_mEasterStatistic)
    {
        SmallEggsWasted += StatIter.second->SmallEggsWasted;
    }

    return SmallEggsWasted;
}

uint32 ObjectMgr::GetEasterGeneralBigEggsCollected()
{
    uint32 BigEggsCollected = 0;
    for (auto& StatIter : m_mEasterStatistic)
    {
        BigEggsCollected += StatIter.second->BigEggsCollected;
    }

    return BigEggsCollected;
}

uint32 ObjectMgr::GetEasterGeneralBigEggsWasted()
{
    uint32 BigEggsWasted = 0;
    for (auto& StatIter : m_mEasterStatistic)
    {
        BigEggsWasted += StatIter.second->BigEggsWasted;
    }

    return BigEggsWasted;
}

void ObjectMgr::LoadClientSpamRegexes(bool reload)
{
    if (reload)
        m_ClientSpamRegex.clear();

    QueryResult* result = LoginDatabase.Query("SELECT Regex FROM antispam_client");
    if (result == nullptr)
    {

        sLog.outString();
        sLog.outString(">> No client side regexes were found. Client side antispam will not be working.");
        return;
    }
    ...
    do
    {
        ...
        m_ClientSpamRegex.push_back(fields[0].GetCppString());

    } while (result->NextRow());

    sLog.outString(">> Loaded %u client side regexes", m_ClientSpamRegex.size());
    sLog.outString();
}
