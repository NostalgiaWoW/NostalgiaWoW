...
void ScriptMgr::GetQuestInstancesForPlayer(ObjectGuid PlayerGuid, std::vector<std::shared_ptr<QuestInstance>>& OutInstances)
{
    auto QuestInstancesPairBound = m_questInstancies.equal_range(PlayerGuid);
    if (QuestInstancesPairBound.first != m_questInstancies.end())
    {
        for (auto StartIter = QuestInstancesPairBound.first; StartIter != QuestInstancesPairBound.second; StartIter++)
        {
            OutInstances.push_back(StartIter->second);
        }
    }
}

std::shared_ptr<QuestInstance> ScriptMgr::GetQuestInstance(ObjectGuid PlayerGuid, uint32 QuestId)
{
    std::vector<std::shared_ptr<QuestInstance>> AllPlayerQuestInstances;
    GetQuestInstancesForPlayer(PlayerGuid, AllPlayerQuestInstances);
    for (std::shared_ptr<QuestInstance> questInst : AllPlayerQuestInstances)
    {
        if (questInst && questInst->GetQuestId() == QuestId)
        {
            return questInst;
        }
    }
...
void QuestInstance::GetStatus(ChatHandler& handler)
{
    handler.PSendSysMessage("");
    handler.PSendSysMessage("Quest Instance: %s", QuestTemplate->GetTitle().c_str());
    handler.PSendSysMessage("Stage: %u", QuestStage);
    if (Player* questHolder = GetPlayer())
    {
        handler.PSendSysMessage("\tPlayer: %s (GUID %llu)", questHolder->GetName(), PlayerGuid.GetRawValue());
        handler.PSendSysMessage("\tHave group - %s, Is Alive - %s", 
            questHolder->GetGroup() != nullptr ? "Yes" : "No",
            questHolder->isAlive() ? "Yes" : "No");
    }
    else
    {
        handler.PSendSysMessage("\tPlayer is offline, GUID: %llu", PlayerGuid.GetRawValue());
    }
}
