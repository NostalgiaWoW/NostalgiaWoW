...
void BattleGroundQueue::UpdateAll()
{
    for (int32 BGTypeID = 1; BGTypeID < MAX_BATTLEGROUND_TYPE_ID; ++BGTypeID)
    {
        for (int32 BracketID = 0; BracketID < MAX_BATTLEGROUND_BRACKETS; ++BracketID)
        {
            Update((BattleGroundTypeId)BGTypeID, (BattleGroundBracketId)BracketID);
        }
    }
}
...
bool BGQueueRemoveEvent::Execute(uint64 /*e_time*/, uint32 /*p_time*/)
{
    ...
    if (queueSlot < PLAYER_MAX_BATTLEGROUND_QUEUES)         // player is in queue, or in Battleground
    {
        ...
        if (bgQueue.IsPlayerInvited(m_PlayerGuid, m_BgInstanceGUID, m_RemoveTime))
        {
            ...
            {
                if (plr->GetSession()->GetAccountFlags() & ACCOUNT_FLAG_ALWAYS_DESERTER_ON_LEAVE || plr->bDeclineBGInvite)
                {
                    if (plr->IsBeingTeleportedFar())
                        plr->ScheduleDelayedOperation(DELAYED_SPELL_CAST_DESERTER);
                    else
                        plr->AddAura(26013, 0, plr);               // Deserter
                }
                else
                {
                    plr->bDeclineBGInvite = true;
                    plr->AddAura(26013, 0, plr);               // Deserter
...
void BattleGroundMgr::Update(uint32 diff)
{
    ...
    {
        m_UpdateIntervalTimer -= diff;
        if (m_UpdateIntervalTimer <= 0)
        {
            m_UpdateIntervalTimer = m_UpdateInterval;

            //Perform force update all queues
            for (int i = 0; i < MAX_BATTLEGROUND_QUEUE_TYPES; ++i)
            {
                m_BattleGroundQueues[i].UpdateAll();
            }
        }
    }
}
...
void BattleGroundMgr::SetUpdateInterval(int32 InUpdateInterval)
{
    if (InUpdateInterval < 0)
    {
        sLog.outError("[BattleGroundMgr]: Update interval should be more that zero, abort");
        return;
    }
    m_UpdateInterval = InUpdateInterval;
}
...
/* Very Garbage for stupid ideas.
	New functionality is simple.
	If AV Queue pops and is accepted, wipe AB and WSG queues
	If WSG queue pops and is accepted, keep AV queues but wipe AB
	if AB queue pops and is accepted, keep AV queues but wipe WSG.
*/
void BattleGroundMgr::RemovePlayerFromQueues(Player* player, int8_t activeSlot, bool logout)
{
    ...
    for (int i = 0; i < PLAYER_MAX_BATTLEGROUND_QUEUES; ++i)
    {
        ...
        {
			removeQueue = (bgQueueTypeId > BATTLEGROUND_QUEUE_AV);
            if (logout || (removeQueue && activeSlot != i))
            {
                // This should only be called on logout or on queues we didn't accept
                player->RemoveBattleGroundQueueId(bgQueueTypeId);
            }

            if (!logout)
            {
				if (removeQueue || activeSlot == i)
					m_BattleGroundQueues[bgQueueTypeId].RemovePlayer(player->GetObjectGuid(), false);

                // Visually update client only in this case. Logout is unaffected.
                if (removeQueue)
                {
                    BuildBattleGroundStatusPacket(&data, nullptr, i, STATUS_NONE, 0, 0);
