...
#define BLESSING_OF_PROTECTION_ICON_ID 303

SpellCastResult BattleGroundWS::CheckSpellCast(Player* caster, SpellEntry const* spell, SpellCastTargets const* targets)
{
    //Don't allow cast BOP on friendly flag carrier
    if (spell->SpellIconID == BLESSING_OF_PROTECTION_ICON_ID && targets)
    {
        if (Unit* SpellTarget = targets->getUnitTarget())
        {
            if (!caster->IsHostileTo(SpellTarget))
            {
                if (GetAllianceFlagPickerGuid() == SpellTarget->GetObjectGuid() || GetHordeFlagPickerGuid() == SpellTarget->GetObjectGuid())
                {
    ...
    return BattleGround::CheckSpellCast(caster, spell, targets);
}
