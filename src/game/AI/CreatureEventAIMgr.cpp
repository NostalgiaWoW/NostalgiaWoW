    ...
    if (result)
    {
        ...
        do
        {
            ...
            switch (temp.event_type)
            {
                ...
                case EVENT_T_DELAYED_ACTION:
                    if (temp.raw.param1 == 0)
                    {
                        sLog.outErrorDb("CreatureEventAI: Creature %u using Delayed Action event, but does not specify any delay (DelayTime = 0)", temp.creature_id);
                    }
                    break;
                default:
