...
ByteBuffer& operator<<(ByteBuffer& b, LootView const& lv)
{
    ...
    switch (lv.permission)
    {
        ...
        case RESTRICTED_PERMISSION:
        {
            ...
            {
                if (!l.items[i].is_looted && !l.items[i].freeforall && !l.items[i].conditionId && l.items[i].AllowedForPlayer(lv.viewer, l.GetLootTarget()))
                {
                    uint8 slot_type;
        ...
        case OWNER_PERMISSION:
        {
            uint8 slot_type = lv.permission == OWNER_PERMISSION ? /*LOOT_SLOT_TYPE_OWNER*/LOOT_SLOT_TYPE_ALLOW_LOOT : LOOT_SLOT_TYPE_ALLOW_LOOT;
            for (uint8 i = 0; i < l.items.size(); ++i)
            {
                if (!l.items[i].is_looted && !l.items[i].freeforall && l.items[i].AllowedForPlayer(lv.viewer, l.GetLootTarget()))
                {
    ...
    if (nn_itr != lootPlayerNonQuestNonFFAConditionalItems.end())
    {
        ...
        for (QuestItemList::const_iterator ci = conditional_list->begin() ; ci != conditional_list->end(); ++ci)
        {
            ...
            if (lv.permission == RESTRICTED_PERMISSION && item.is_blocked)
                slot_type = LOOT_SLOT_TYPE_LOCKED;
