...
void FleeingMovementGenerator<T>::_setTargetLocation(T &owner)
{
    ...
    if (owner.hasUnitState((UNIT_STAT_CAN_NOT_REACT | UNIT_STAT_STUNNED | UNIT_STAT_PENDING_STUNNED) & ~UNIT_STAT_FLEEING))
        return;
...
bool FleeingMovementGenerator<T>::Update(T &owner, const uint32 & time_diff)
{
    ...
    // ignore in case other no reaction state
    if (!owner.CanMove() || owner.hasUnitState((UNIT_STAT_CAN_NOT_REACT | UNIT_STAT_STUNNED | UNIT_STAT_PENDING_STUNNED) & ~UNIT_STAT_FLEEING))
    {
        owner.clearUnitState(UNIT_STAT_FLEEING_MOVE);
        return true;
    }
