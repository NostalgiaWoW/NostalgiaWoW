...
void MotionMaster::MoveFall(uint32 id /*=0*/, bool addFlagForNPC)
{
    ...
    if (m_owner->GetTypeId() == TYPEID_PLAYER)
    {
        m_owner->AddUnitMovementFlag(MOVEFLAG_JUMPING);
        m_owner->m_movementInfo.SetFallTime(0);
        m_owner->ToPlayer()->SetFallInformation(time(NULL), m_owner->GetPositionZ());
    }
    else if (m_owner->GetTypeId() == TYPEID_UNIT && addFlagForNPC)
    {
        m_owner->RemoveUnitMovementFlag(MOVEFLAG_MASK_MOVING);
        m_owner->AddUnitMovementFlag(MOVEFLAG_JUMPING);
        m_owner->m_movementInfo.SetFallTime(0);
