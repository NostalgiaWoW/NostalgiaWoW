...
void TargetedMovementGeneratorMedium<T, D>::_setTargetLocation(T &owner)
{
    ...
    if (!owner.CanMove() || owner.hasUnitState(UNIT_STAT_CONTROLLED))
        return;
    ...
    else if (!i_offset)
    {
        ...
        // if even at the random attack point we will not be able to reach the target, the
        // target is considered not reachable.  this happens when the target is fly hacking.
        if (!owner.IsWithinMeleeRange(x, y, z, i_target.getTarget()))
        {
            i_reachable = false;
            return;
        }
...
void TargetedMovementGeneratorMedium<T, D>::UpdateAsync(T &owner, uint32 /*diff*/)
{
    ...
    if (!i_target.isValid() || !i_target->IsInWorld() || !owner.isAlive() || !owner.CanMove() || owner.hasUnitState(UNIT_STAT_CONTROLLED)
