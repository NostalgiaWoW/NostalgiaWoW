...
bool ChatHandler::HandleGameObjectTempAddCommand(char *args)
{
    ...
    bool attach = true;
    if (char* secondParam = ExtractArg(&args, false))
    {
        if (strcmp(secondParam, "detach") == 0)
        {
            attach = false;
        }
    }
    ...
    ExtractOptUInt32(&args, spawntm, 300);
    ...
    if (!chr->SummonGameObject(id, x, y, z, ang, 0, 0, rot2, rot3, spawntm, attach))
        return false;
    return true;
}
...
bool ChatHandler::HandleCharacterChangeRaceCommand(char* args)
{
    if (Player* pPlayer = getSelectedPlayer())
    {
        ...
        ExtractUInt32(&args, newRaceId);

        uint32 gender = 255;
        uint32 playerbyte1 = 0;
        uint32 playerbyte2 = 0;
        ExtractOptUInt32(&args, gender, 255);
        ExtractOptUInt32(&args, playerbyte1, 0);
        ExtractOptUInt32(&args, playerbyte2, 0);

        if (pPlayer->ChangeRace((uint8)newRaceId, gender, playerbyte1, playerbyte2))
        {
            sLog.outInfo("Player (GUID: %llu) \"%s\" changed race to %u", pPlayer->GetObjectGuid(), pPlayer->GetName(), newRaceId);
            //PSendSysMessage("Race changed to %u", newRaceId);
            return true;
        }
...
bool ChatHandler::HandleBGRandomizerCommand(char* args)
{
    bool bEnable = false;
    ...
    //sWorld.getConfig(CONFIG_BOOL_BATTLEGROUND_RANDOMIZE)
    sWorld.setConfig(CONFIG_BOOL_BATTLEGROUND_RANDOMIZE, bEnable);
    SendSysMessage(bEnable ? "BG Randomizer enabled" : "BG Randomizer disabled");

    return true;
}

bool ChatHandler::HandleBGUpdateIntervalCommand(char* args)
{
    int32 NewUpdateInterval = 0;
    if (!ExtractInt32(&args, NewUpdateInterval))
    {
        return false;
    }

    sBattleGroundMgr.SetUpdateInterval(NewUpdateInterval);
...
bool ChatHandler::HandleReloadClientAntispamCommand(char*)
{
    sObjectMgr.LoadClientSpamRegexes(true);
    SendSysMessage(">> Table `antispam_client` reloaded.");

    if (sWorld.getConfig(CONFIG_BOOL_ENABLE_CLIENT_REGEX))
    {
        for (const auto& itr : sWorld.GetAllSessions())
        {
            Player* pPlayer = itr.second->GetPlayer();
            if (pPlayer != nullptr && pPlayer->IsInWorld())
                itr.second->SendExpectedSpamRecords();
        }
    }
