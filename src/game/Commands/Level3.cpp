...
bool ChatHandler::HandleAccountSetPacketLogCommand(char* args)
{
    ...
    PSendSysMessage("Packet logging for %s set to %s", account_name.c_str(), value ? "true" : "false");
    LoginDatabase.PExecute("UPDATE account SET flags = flags | %u WHERE id = %u", ACCOUNT_FLAG_PACKETLOG, targetAccountId);

    if (auto session = sWorld.FindSession(targetAccountId))
    {
        session->SetPacketLogging(value);
        PSendSysMessage("Packet logging %s for active session", value ? "enabled" : "disabled");
    }
...
bool ChatHandler::HandleAddItemCommand(char* args)
{
    ...
    // remove binding (let GM give it to another player later) / REMOVED MAY 18 2018 (webshop is using .additem as well)
#if 0
    if (pl == plTarget)
...
{ MECHANIC_FUMBLE          , "MECHANIC_FUMBLE" },
{ MECHANIC_ROOT            , "MECHANIC_ROOT" },
{ MECHANIC_PACIFY          , "MECHANIC_PACIFY" },
...
{ MECHANIC_PERSUADE        , "MECHANIC_PERSUADE" },
...
bool ChatHandler::HandleServerRestartCommand(char* args)
{
    ...
    int32 exitCode = RESTART_EXIT_CODE;
    ...
    if (reason.length())
        sWorld.SetRestartReason(reason);
...
bool ChatHandler::HandleQuestAddCommand(char* args)
{
    ...
    Player* player;
    const char* characterName = ExtractArg(&args, false);
    if (characterName != NULL && strlen(characterName) != 0) {
        player = sObjectMgr.GetPlayer(characterName);
    }
    else {
    ...
    if (player == nullptr) {
        PSendSysMessage(LANG_NON_EXIST_CHARACTER, entry);
        SetSentErrorMessage(true);
        return false;
    }
...
bool ChatHandler::HandleQuestStatusCommand(char* args)
{
    ...
    std::shared_ptr<QuestInstance> iQuestInstance = sScriptMgr.GetQuestInstance(player->GetObjectGuid(), quest->GetQuestId());
    if (iQuestInstance)
    {
        PSendSysMessage("Have quest instance bounded");
        iQuestInstance->GetStatus(*this);
    }
    return true;
}
...
bool ChatHandler::HandleScanMultiboxCommand(char* args)
{
    if (!*args) //check all multi - logins with ident ip
    {
        bool found = false;
        typedef std::pair<std::string, uint32> pair;
        std::map<std::string, uint32> multi_ip;
        PSendSysMessage("|cFFFF1100 (All ident Ip) Multi-logins: |r");
        for (auto ses : sWorld.GetAllSessions())
        {
            if (!ses.second->GetPlayer())
                continue;

            std::map<std::string, uint32>::iterator it = multi_ip.find(ses.second->GetRemoteAddress());
             if (it != multi_ip.end())
                 continue;

            const std::string ipAddrs = ses.second->GetRemoteAddress();
            std::unordered_map<uint32, WorldSession*> ip_sessions = sWorld.GetSessionsByIp(ipAddrs, true);
            if (!ip_sessions.empty() && ip_sessions.size() > 1)
            {
                multi_ip.insert({ ipAddrs, ip_sessions.size() });
                found = true;
            }
        }

        std::vector<pair> multi_ip_vec;
        std::copy(multi_ip.begin(), multi_ip.end(),
            std::back_inserter<std::vector<pair>>(multi_ip_vec));
        std::sort(multi_ip_vec.begin(), multi_ip_vec.end(),
        ...
        for (auto const& pair : multi_ip_vec)
            PSendSysMessage("|cFFFF1100 %u multi-logins found! IP:|r %s", pair.second, playerLink(pair.first).c_str());

        if (!found)
            PSendSysMessage("|cFFFF1100 (All ident Ip) Multi-logins not found! |r");

        return true;
    }

    char* last_ip = ExtractArg(&args);
    if (!last_ip)
        return false;

    if (IsIPAddress(last_ip)) //check multi-logins by ip
    {
        std::unordered_map<uint32, WorldSession*> ip_sessions = sWorld.GetSessionsByIp(last_ip, true);
        bool found = false;
        PSendSysMessage("|cFFFF1100 Multi-logins IP:|r %s", playerLink(last_ip).c_str());
        if (!ip_sessions.empty() && ip_sessions.size() > 1)
        {
            PSendSysMessage("|cFFFF1100 %u multi-logins found! IP:|r %s", ip_sessions.size(), playerLink(last_ip).c_str());
            for (auto mb : ip_sessions)
                mb.second->GetAnticheat()->SendPlayerLoginInfo(this);

            found = true;
        }

        if (!found)
            PSendSysMessage("|cFFFF1100 Multi-logins not found for IP:|r %s", playerLink(last_ip).c_str());
...
bool ChatHandler::HandleWarnCharacterCommand(char* args)
{
    ...
    std::string authorName = m_session ? m_session->GetPlayerName() : "Console";
    char* reason;
    int32 mstring_id_reason;
    if (!ExtractOptInt32(&args, mstring_id_reason, 0))
        reason = ExtractQuotedOrLiteralArg(&args);

    if (!reason)
        reason = "<no reason given>";

    MangosStrings mstring = LANG_WARN_INFORM;
    if (mstring_id_reason >= 1700 && mstring_id_reason <= 1800)
    {
        mstring = (MangosStrings)mstring_id_reason;
        //reason = (char*)GetMangosString(mstring_id_reason);
        sWorld.WarnAccount(playerData->uiAccount, authorName, std::to_string(mstring_id_reason), "WARN");
        LoginDatabase.PExecute("INSERT INTO account_banned (id, bandate, unbandate, bannedby, banreason, active, realm) VALUES ('%u', UNIX_TIMESTAMP()+1, UNIX_TIMESTAMP()+2, '%s', '%s', '0', %u)",
            playerData->uiAccount, authorName.c_str(), (std::string("NOTE: ") + std::to_string(mstring_id_reason)).c_str(), realmID);
    }
    else
    {
        sWorld.WarnAccount(playerData->uiAccount, authorName, reason, "WARN");
        LoginDatabase.PExecute("INSERT INTO account_banned (id, bandate, unbandate, bannedby, banreason, active, realm) VALUES ('%u', UNIX_TIMESTAMP()+1, UNIX_TIMESTAMP()+2, '%s', '%s', '0', %u)",
            playerData->uiAccount, authorName.c_str(), (std::string("NOTE: ") + reason).c_str(), realmID);
    }
    ...
    if (target)
    {
        if (mstring == LANG_WARN_INFORM)
        {
            ...
            PSendSysMessage("|cFFFF1100 Account #%u (character %s): a note has been added |r \n \"%s\"",
                playerData->uiAccount, playerData->sName.c_str(), (std::string("NOTE: ") + reason).c_str());
        }
        else
        {
            ...
            PSendSysMessage("|cFFFF1100 Account #%u (character %s) has been warned (warn id:%u). |r",
                playerData->uiAccount, playerData->sName.c_str(), mstring_id_reason);
...
bool ChatHandler::HandleBanHelper(BanMode mode, char* args)
{
    ...
    case BAN_ACCOUNT:
        ...
        {
            QueryResult* resultAcc = LoginDatabase.PQuery("SELECT id FROM account WHERE username = '%s'", nameOrIP.c_str());
            if (resultAcc)
            {
                uint32 time = 0;
                QueryResult* resultChar = CharacterDatabase.PQuery("SELECT name, online, logout_time FROM characters WHERE account = '%u'", resultAcc->Fetch()[0].GetUInt32());
                if (resultChar)
                {
                    do
                    {
                        Field* fields = resultChar->Fetch();
                        uint32 logout_time = fields[2].GetUInt32();
                        if (fields[1].GetBool())
                        {
                            characterName = fields[0].GetCppString();
                            break;
                        } 
                        else if (time < logout_time)
                        {
                            characterName = fields[0].GetCppString();
                            time = logout_time;
                        }
    ...
    case BAN_IP:
        ...
        {
            QueryResult* resultAcc = LoginDatabase.PQuery("SELECT id FROM account WHERE last_ip = '%s'", nameOrIP.c_str());
            if (resultAcc)
            {
                uint32 time = 0;
                QueryResult* resultChar = CharacterDatabase.PQuery("SELECT name, online, logout_time FROM characters WHERE account = '%u'", resultAcc->Fetch()[0].GetUInt32());
    ...
    case BAN_SUCCESS:
        ...
        else
        {
            ...
            if (characterName != "")
                sWorld.SendWorldText(LANG_WORLD_PLAYER_PERMBANNED, characterName.c_str(), reason);
        }
        break;
...
bool ChatHandler::HandleServerMemcompactCommand(char* args)
{
#ifndef USE_STANDARD_MALLOC
	scalable_allocation_command(TBBMALLOC_CLEAN_ALL_BUFFERS, nullptr);
	PSendSysMessage("Memory purged");
#else
	PSendSysMessage("Server builded without TBB! Command do nothing");
#endif
...
bool ChatHandler::ChathookHelper(char* args, ChathookTarget targetType)
{
    bool bEnable = false;
    ...
    if (WorldSession* PlayerSession = GetSession())
    {
        if (Player* pl = PlayerSession->GetPlayer())
        {
            Player* targetPlayer = pl->getSelectedPlayer();
            if (targetPlayer == nullptr)
            {
                if (!ExtractPlayerTarget(&args, &targetPlayer))
                {
                    PSendSysMessage("You should select target, or enter player name");
                    return false;
                }
            }
            ...
            case CH_ALL:
            {
                Guild* gld = sGuildMgr.GetGuildById(targetPlayer->GetGuildId());
                if (bEnable)
                {
                    sChatHookMgr.InstallHook(pl, targetPlayer->GetGroup());
                    sChatHookMgr.InstallHook(pl, gld);
                    sChatHookMgr.InstallHook(pl, targetPlayer);
                    sChatHookMgr.InstallHookOnMiscChannels(pl, targetPlayer);
                }
                else
                {
                    sChatHookMgr.RemoveHook(pl, targetPlayer->GetGroup());
                    sChatHookMgr.RemoveHook(pl, gld);
                    sChatHookMgr.RemoveHook(pl, targetPlayer);
                }
            }
                break;
            case CH_GROUP:
...
bool ChatHandler::HandleChathookGroupCommand(char* args)
{
    return ChathookHelper(args, CH_GROUP);
}

bool ChatHandler::HandleChathookGuildCommand(char* args)
{
    return ChathookHelper(args, CH_GUILD);
}

bool ChatHandler::HandleChathookWhisperCommand(char* args)
{
    return ChathookHelper(args, CH_WHISPER);
}

bool ChatHandler::HandleChathookAllCommand(char* args)
{
    return ChathookHelper(args, CH_ALL);
}

bool ChatHandler::HandleChathookClearCommand(char* args)
{
    if (WorldSession* PlayerSession = GetSession())
    {
        if (Player* pl = PlayerSession->GetPlayer())
        {
            sChatHookMgr.ClearAllHooksRelatedToListener(pl->GetObjectGuid());
            return true;
        }
    }
    return false;
...
{
    PSendSysMessage("Anticheat is not builded with server!");
    return true;
}
...
bool ChatHandler::HandleEasterWorldEggSpawnTime(char* args)
{
    //this is hell of shit
//     for (auto mapIter : sMapMgr.Maps())
//     {
//         Map* pMap = mapIter->second;
//         if (pMap && pMap->IsContinent())
//         {
//             pMap->Get
//         }
//     }
    SendSysMessage("NOT IMPLEMENTED!");
    return false;
}

#define NPC_RABBIT_BOSS 17997
#define NPC_RABBIT_MINION 17998

bool ChatHandler::HandleEasterRabbitLayEggTime(char* args)
{
    if (Player* pl = GetSession()->GetPlayer())
    {
        if (Unit* selectedUnit = pl->getSelectedUnit())
        {
            if (selectedUnit->GetEntry() == NPC_RABBIT_MINION)
            {
                uint32 NewLayEggTime = 0;
                if (strlen(args) != 0 && ExtractOptUInt32(&args, NewLayEggTime, 0))
                {
                    selectedUnit->AI()->SetData(EasterDataIndexes::RabbitLayEggTime, NewLayEggTime);
                }
                else
                {
                    uint32 LayEggTime = 0;
                    selectedUnit->AI()->GetData(EasterDataIndexes::RabbitLayEggTime, LayEggTime);
                    PSendSysMessage("GLOBAL Rabbit lay egg time: %u", LayEggTime);
                }
        ...
        {
            SendSysMessage("Please select Cottontail children");
...
bool ChatHandler::HandleEasterPInfo(char* args)
{
    Player* pl = nullptr;
    ObjectGuid pObjGuid;
    if (ExtractPlayerTarget(&args, &pl, &pObjGuid))
    {
        std::shared_ptr<EasterStatistic> pStat = sObjectMgr.GetEasterStatisticForPlayer(pObjGuid);
        if (pStat)
        {
            PSendSysMessage("Small Eggs Collected: %u; Small Eggs Wasted: %u", pStat->SmallEggsCollected, pStat->SmallEggsWasted);
            PSendSysMessage("Big Eggs Collected: %u; Big Eggs Wasted: %u", pStat->BigEggsCollected, pStat->BigEggsWasted);
        }
    }
    return true;
}
bool ChatHandler::HandleEasterInfo(char* args)
{
    uint32 OverallSmallEggsCollected = sObjectMgr.GetEasterGeneralSmallEggsCollected();
    uint32 OverallSmallEggsWasted = sObjectMgr.GetEasterGeneralSmallEggsWasted();
    uint32 OverallBigEggsCollected = sObjectMgr.GetEasterGeneralBigEggsCollected();
    uint32 OverallBigEggsWasted = sObjectMgr.GetEasterGeneralBigEggsWasted();

    PSendSysMessage("OVERALL: Small Eggs Collected: %u; Small Eggs Wasted: %u", OverallSmallEggsCollected, OverallSmallEggsWasted);
    PSendSysMessage("OVERALL: Big Eggs Collected: %u; Big Eggs Wasted: %u", OverallBigEggsCollected, OverallBigEggsWasted);
    SendSysMessage("Press F to pay respect for Giperion, I want to sleep");
    return true;
}

bool ChatHandler::HandleEasterRabbitEnrageTime(char* args)
{
    if (Player* pl = GetSession()->GetPlayer())
    {
        if (Unit* selectedUnit = pl->getSelectedUnit())
        {
            if (selectedUnit->GetEntry() == NPC_RABBIT_BOSS)
            {
                uint32 NewEnrageTime = 0;
                if (strlen(args) != 0 && ExtractOptUInt32(&args, NewEnrageTime, 0))
                {
                    selectedUnit->AI()->SetData(EasterDataIndexes::EnrageTime, NewEnrageTime);
                }
                else
                {
                    uint32 EnrageTime = 0;
                    selectedUnit->AI()->GetData(EasterDataIndexes::EnrageTime, EnrageTime);
                    PSendSysMessage("GLOBAL Enrage time: %u", EnrageTime);
                }
        ...
        {
            SendSysMessage("Please select Cottontail");
            SetSentErrorMessage(false);
        }
    }
...
bool ChatHandler::HandleEasterRabbitCalmdownTime(char* args)
{
    if (Player* pl = GetSession()->GetPlayer())
    {
        if (Unit* selectedUnit = pl->getSelectedUnit())
        {
            if (selectedUnit->GetEntry() == NPC_RABBIT_BOSS)
            {
                uint32 NewCalmdownTime = 0;
                if (strlen(args) != 0 && ExtractOptUInt32(&args, NewCalmdownTime, 0))
                {
                    selectedUnit->AI()->SetData(EasterDataIndexes::CalmdownTime, NewCalmdownTime);
                }
                else
                {
                    uint32 CalmdownTime = 0;
                    selectedUnit->AI()->GetData(EasterDataIndexes::CalmdownTime, CalmdownTime);
                    PSendSysMessage("GLOBAL calmdown time: %u", CalmdownTime);
                }
...
bool ChatHandler::HandleEasterRabbitWarnTime(char* args)
{
    if (Player* pl = GetSession()->GetPlayer())
    {
        if (Unit* selectedUnit = pl->getSelectedUnit())
        {
            if (selectedUnit->GetEntry() == NPC_RABBIT_BOSS)
            {
                uint32 NewWarnTime = 0;
                if (strlen(args) != 0 && ExtractOptUInt32(&args, NewWarnTime, 0))
                {
                    selectedUnit->AI()->SetData(EasterDataIndexes::WarnTime, NewWarnTime);
                }
                else
                {
                    uint32 WarnTime = 0;
                    selectedUnit->AI()->GetData(EasterDataIndexes::WarnTime, WarnTime);
                    PSendSysMessage("GLOBAL player warning time before polymorph: %u", WarnTime);
                }
...
bool ChatHandler::HandleEasterRabbitVisionRadius(char* args)
{
    if (Player* pl = GetSession()->GetPlayer())
    {
        if (Unit* selectedUnit = pl->getSelectedUnit())
        {
            if (selectedUnit->GetEntry() == NPC_RABBIT_BOSS)
            {
                float NewVisionRadius = 0.0f;
                if (strlen(args) != 0 && ExtractOptFloat(&args, NewVisionRadius, 0.0f))
                {
                    selectedUnit->AI()->SetData(EasterDataIndexes::VisionRadius, NewVisionRadius);
                }
                else
                {
                    float VisionRadius = 0;
                    selectedUnit->AI()->GetData(EasterDataIndexes::VisionRadius, VisionRadius);
                    PSendSysMessage("GLOBAL boss polymorph vision radius: %f", VisionRadius);
                }
...
bool ChatHandler::HandleEasterRabbitChangeOrientationTime(char* args)
{
    if (Player* pl = GetSession()->GetPlayer())
    {
        if (Unit* selectedUnit = pl->getSelectedUnit())
        {
            if (selectedUnit->GetEntry() == NPC_RABBIT_BOSS)
            {
                uint32 NewOrientationTime = 0;
                if (strlen(args) != 0 && ExtractOptUInt32(&args, NewOrientationTime, 0))
                {
                    selectedUnit->AI()->SetData(EasterDataIndexes::OrientationTime, NewOrientationTime);
                }
                else
                {
                    uint32 OrientationTime = 0;
                    selectedUnit->AI()->GetData(EasterDataIndexes::OrientationTime, OrientationTime);
                    PSendSysMessage("GLOBAL time after rabbit change his orientation: %u", OrientationTime);
                }
...
bool ChatHandler::HandleEasterRabbitEnrageFlamestrikeTime(char* args)
{
    if (Player* pl = GetSession()->GetPlayer())
    {
        if (Unit* selectedUnit = pl->getSelectedUnit())
        {
            if (selectedUnit->GetEntry() == NPC_RABBIT_BOSS)
            {
                uint32 NewEnrageFlamestrikeTime = 0;
                if (strlen(args) != 0 && ExtractOptUInt32(&args, NewEnrageFlamestrikeTime, 0))
                {
                    selectedUnit->AI()->SetData(EasterDataIndexes::EnrageFlamestrikeTime, NewEnrageFlamestrikeTime);
                }
                else
                {
                    uint32 EnrageFlamestrikeTime = 0;
                    selectedUnit->AI()->GetData(EasterDataIndexes::EnrageFlamestrikeTime, EnrageFlamestrikeTime);
                    PSendSysMessage("GLOBAL time of flamestrike: %u", EnrageFlamestrikeTime);
                }
...
bool ChatHandler::HandleEasterRabbitEnrage(char* args)
{
    if (Player* pl = GetSession()->GetPlayer())
    {
        if (Unit* selectedUnit = pl->getSelectedUnit())
...
bool ChatHandler::HandleEasterRabbitCalmdown(char* args)
{
    if (Player* pl = GetSession()->GetPlayer())
    {
        if (Unit* selectedUnit = pl->getSelectedUnit())
...
bool ChatHandler::HandleEasterRabbitBurnRadius(char* args)
{
    if (Player* pl = GetSession()->GetPlayer())
    {
        if (Unit* selectedUnit = pl->getSelectedUnit())
        {
            if (selectedUnit->GetEntry() == NPC_RABBIT_BOSS)
            {
                uint32 NewBurnRadius = 0;
                if (strlen(args) != 0 && ExtractOptUInt32(&args, NewBurnRadius, 0))
                {
                    selectedUnit->AI()->SetData(EasterDataIndexes::BurnAuraDamageRadius, NewBurnRadius);
                }
                else
                {
                    uint32 BurnRadius = 0;
                    selectedUnit->AI()->GetData(EasterDataIndexes::BurnAuraDamageRadius, BurnRadius);
                    PSendSysMessage("GLOBAL burn radius: %u", BurnRadius);
                }
...
bool ChatHandler::HandleEasterRabbitBurnDamage(char* args)
{
    if (Player* pl = GetSession()->GetPlayer())
    {
        if (Unit* selectedUnit = pl->getSelectedUnit())
        {
            if (selectedUnit->GetEntry() == NPC_RABBIT_BOSS)
            {
                uint32 NewBurnDamagePercent = 0;
                if (strlen(args) != 0 && ExtractOptUInt32(&args, NewBurnDamagePercent, 0))
                {
                    selectedUnit->AI()->SetData(EasterDataIndexes::BurnAuraDamagePercent, NewBurnDamagePercent);
                }
                else
                {
                    uint32 BurnDamagePercent = 0;
                    selectedUnit->AI()->GetData(EasterDataIndexes::BurnAuraDamagePercent, BurnDamagePercent);
                    PSendSysMessage("GLOBAL burn damage percented: %u", BurnDamagePercent);
                }
...
bool ChatHandler::HandleSuspiciousMovementDetectValue(char* args)
{
    ...
    if (!ExtractFloat(&args, MovementDetectValue)) return false;
...
bool ChatHandler::HandleDebugInactivityDataCommand(char* args)
{
    if (WorldSession* session = GetSession())
    {
        if (Player* player = session->GetPlayer())
