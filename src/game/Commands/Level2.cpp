...
bool ChatHandler::HandleMuteCommand(char* args)
{
    ...
    // only find player from the same account, if any.
    if (!target)
        if (WorldSession* session = sWorld.FindSession(account_id))
            target = session->GetPlayer();

    // must have lower security level.
    ...
    std::string l_AuthorName = m_session ? m_session->GetPlayer()->GetName() : "Console";
    ...
    if (muteReason != "")
        reason << " for \"" << muteReason << "\"";

    sWorld.WarnAccount(playerData->uiAccount, l_AuthorName, reason.str(), "WARNING");
    return true;
}
...
bool ChatHandler::HandleGameObjectAddCommand(char* args)
{
    ...
    PSendSysMessage("Reminder: .gobject add should be used rarely, because it's writting permanent record in DB. If you need permanent object in world - contact developers");
    PSendSysMessage("Consider to use .gobject tmpadd with parameter 'detach', if you want to place something in world");
...
bool ChatHandler::HandleNpcAddCommand(char* args)
{
    ...
    SendSysMessage("Reminder: .npc add should be used rarely, because it's writting permanent record in DB. If you need permanent object in world - contact developers");
    SendSysMessage("Consider to use .npc summon, if you want to place something in world");
...
/*	if (!*args)
	...
	if (!creature)
	{
		PSendSysMessage("No model ID specified!");
		SetSentErrorMessage(true);
		return false;
	}
...
bool ChatHandler::HandlePInfoCommand(char* args)
{
    ...
    std::string accIdStr = std::to_string(accId);
    PSendSysMessage(LANG_PINFO_ACCOUNT, raceName, className, (target ? "" : GetMangosString(LANG_OFFLINE)), nameLink.c_str(), target_guid.GetCounter(), playerLink(username).c_str(), 
        playerLink(accIdStr).c_str(), sAccountMgr.IsAccountBanned(accId) ? ", banned" : "", security, playerLink(last_ip).c_str(), sAccountMgr.IsIPBanned(last_ip) ? " [BANIP]" : "", 
        last_login.c_str(), latency, localeNames[loc], two_factor_enabled.c_str());
    ...
    if (target)
    {
        ...
        if (IsIPAddress(last_ip.c_str()))
        {
            int i = 1;
            std::unordered_map<uint32, WorldSession*> ip_sessions = sWorld.GetSessionsByIp(last_ip, true);
            for (auto m_ses : ip_sessions)
                if (m_ses.second && m_ses.second->GetAccountId() != accId)
                {
                    if (i == 1)
                        PSendSysMessage("|cFFFF1100 (IP) Multi-logins found:|r");

                    i++;
                    m_ses.second->GetAnticheat()->SendPlayerLoginInfo(this);
                }

            if (i > 1)
            {
                target->GetSession()->GetAnticheat()->SendPlayerLoginInfo(this);
                PSendSysMessage("|cFFFF1100 Total Multi-logins:%u for IP:%s|r", i, playerLink(last_ip).c_str());
            }
        }
    }
...
bool ChatHandler::HandleAccInfoCommand(char * args)
{
    ...
    //                                                  0     1         2      3
    QueryResult *result = LoginDatabase.PQuery("SELECT id, username, email, last_ip FROM account WHERE username = '%s'", account.c_str());

    if (!result)
    {
        SendSysMessage(LANG_ACCOUNT_DOESNT_EXIST);
        return true;
    }
    ...
    do
    {
        ...
        if (showIp)
            lastIp = fields[3].GetCppString();
        else
            lastIp = "-";

        PSendSysMessage(LANG_ACCOUNT_INFO_STRING, account, fields[1].GetString(), playerLink(email).c_str(), playerLink(lastIp).c_str(), security);
...
bool ChatHandler::HandleHonorAddKillCommand(char* args)
{
    ...
    {
        target->GetHonorMgr().Add(0, HONORABLE, target, true);
    }
    return true;
}
...
bool ChatHandler::HandleCombatListCommand(char* args)
{
    ...
    PSendSysMessage("Player %s %s in combat.  Combat timer: %u", target->GetName(), target->isInCombat() ? "is" : "is not", target->GetCombatTimer());

    SendSysMessage("Attackers:");

    for (auto const &attacker : target->getAttackers())
        PSendSysMessage("%s (%f, %f, %f)", attacker->GetName(), attacker->GetPositionX(), attacker->GetPositionY(), attacker->GetPositionZ());

    SendSysMessage("Threats:");

    for (auto const& threat : target->getThreatManager().getThreatList())
    {
        ObjectGuid const& guid = threat->getUnitGuid();
        if (Unit const* threatUnit = target->GetMap()->GetUnit(guid))
            PSendSysMessage("%s (%f, %f, %f)", threatUnit->GetName(), threatUnit->GetPositionX(), threatUnit->GetPositionY(), threatUnit->GetPositionZ());
        else
            PSendSysMessage("No unit found with guid %s", guid.GetString().c_str());
    }

    auto const instanceId = target->GetInstanceId();

    if (auto const group = target->GetGroup())
    {
        PSendSysMessage("Group %s in combat in instance %d", group->InCombatToInstance(instanceId) ? "is" : "is not", instanceId);
        ...
        for (GroupReference *itr = group->GetFirstMember(); itr != NULL; itr = itr->next())
        {
            ...
            {
                PSendSysMessage("Group member %s in combat.  Attackers:", pPlayer->GetName());

                for (auto const &attacker : pPlayer->getAttackers())
                    PSendSysMessage("%s (%f, %f, %f)", attacker->GetName(), attacker->GetPositionX(), attacker->GetPositionY(), attacker->GetPositionZ());
            }
        }
    }
...
bool ChatHandler::HandleLookupAccountIpListCommand(char* p_Args)
{
    char* l_AccountStr = ExtractQuotedOrLiteralArg(&p_Args);
    if (!l_AccountStr)
        return false;

    std::string l_AccountName = l_AccountStr;

    uint32 l_AccountId = sAccountMgr.GetId(l_AccountName);
    if (!l_AccountId)
        return false;

    QueryResult* l_Result = LogsDatabase.PQuery("SELECT `time`, `ip` FROM `logs_characters` WHERE `account` ='%u' ORDER BY `time` DESC", l_AccountId);
    if (l_Result == nullptr)
        return false;

    PSendSysMessage(("IP history for account - " + l_AccountName).c_str());
    ...
    do
    {
        ...
        std::string l_Time = l_Field[0].GetCppString();
        std::string l_Ip = l_Field[1].GetCppString();

        if (l_Ip != l_LastIp)
        {
            PSendSysMessage(("Date: " + l_Time + ", IP: " + l_Ip).c_str());
            l_LastIp = l_Ip;
        }
