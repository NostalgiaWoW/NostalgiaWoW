...
bool ChatHandler::HandleSanctionDeserter(char* args)
{
    if (WorldSession* session = GetSession())
    {
        if (Player* player = session->GetPlayer())
        {
            ...
            {
                WorldSession* selectedPlayerSession = selectedPlayer->GetSession();
                if (!selectedPlayerSession) return false;
                
                bool bEnable = false;
                if (!ExtractOnOff(&args, bEnable)) return false;

                uint32 flags = selectedPlayerSession->GetAccountFlags();

                if (bEnable)
                {
                    session->SetAccountFlags(flags | ACCOUNT_FLAG_ALWAYS_DESERTER_ON_LEAVE);
                    LoginDatabase.PExecute("UPDATE account SET flags = flags | 0x%x WHERE id = %u",
                        session->GetAccountId(), ACCOUNT_FLAG_ALWAYS_DESERTER_ON_LEAVE);
                    PSendSysMessage("Player '%s' will now always get deserter debuff from declining bg invite", selectedPlayer->GetName());
                }
                else
                {
                    session->SetAccountFlags(flags & ~ACCOUNT_FLAG_ALWAYS_DESERTER_ON_LEAVE);
                    LoginDatabase.PExecute("UPDATE account SET flags = flags & ~0x%x WHERE id = %u",
                        session->GetAccountId(), ACCOUNT_FLAG_ALWAYS_DESERTER_ON_LEAVE);
                    PSendSysMessage("Player '%s' will no longer get deserter debuff from first bg invite decline", selectedPlayer->GetName());
                }
            ...
            {
                SendSysMessage("Please select a player");
                return false;
            }
        }
    }
