...
void Guild::BroadcastToGuild(WorldSession *session, const std::string& msg, uint32 language)
{
    if (session && session->GetMasterPlayer() && HasRankRight(session->GetMasterPlayer()->GetRank(), GR_RIGHT_GCHATSPEAK))
    {
        ...
        for (MemberList::const_iterator itr = members.begin(); itr != members.end(); ++itr)
        {
			...
			{
				if (!sSocialMgr.GetPlayerSocial(pl->GetObjectGuid())->HasIgnore(session->GetMasterPlayer()->GetObjectGuid()))
				{
...
void Guild::BroadcastToGuild(WorldSession *session, WorldPacket& data)
{
    if (session && session->GetMasterPlayer() && HasRankRight(session->GetMasterPlayer()->GetRank(), GR_RIGHT_GCHATSPEAK))
    {
        for (MemberList::const_iterator itr = members.begin(); itr != members.end(); ++itr)
...
void Guild::BroadcastToOfficers(WorldSession *session, WorldPacket& data)
{
    if (session && session->GetMasterPlayer() && HasRankRight(session->GetMasterPlayer()->GetRank(), GR_RIGHT_OFFCHATSPEAK))
    {
        for (MemberList::const_iterator itr = members.begin(); itr != members.end(); ++itr)
