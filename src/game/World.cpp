...
/// Find a sessions by ip
std::unordered_map<uint32, WorldSession*> World::GetSessionsByIp(const std::string ip, bool online)
{
    std::unordered_map<uint32, WorldSession*> ip_sessions;

    if (!IsIPAddress(ip.c_str()))
        return ip_sessions;
    
    for (SessionMap::const_iterator itr = m_sessions.begin(); itr != m_sessions.end(); ++itr)
        if (itr->second->GetRemoteAddress() == ip && (online ? itr->second->GetPlayer() != nullptr : true))
            ip_sessions.insert({ itr->first, itr->second });
...
void World::LoadConfigSettings(bool reload)
{
    ...
    setConfigMinMax(CONFIG_UINT32_HONOR_ENABLE, "HonorEnable", 0, 0, 2);
    ...
    setConfig(CONFIG_BOOL_LEVEL_ANNOUNCE, "Level.Announce", false);
...
//    sLog.outString( "WORLD: VMap config keys are: vmap.enableLOS, vmap.enableHeight, vmap.ignoreMapIds, vmap.ignoreSpellIds");
    ...
    uint32 maxSupportedPages = sConfig.GetIntDefault("MaxSupportedCharacterPages", 0);
    for (size_t i = 0; i < maxSupportedPages; ++i)
    {
        char buff[32];
        sprintf(buff, "CharSecondPageBindIP%u", uint32(i + 1));

        m_CharacterPageIPs.emplace_back(sConfig.GetStringDefault(buff, ""));
    }

    setConfig(CONFIG_INT32_XP_DIVIDE_THRESHOLD, "XPDivideThreshold", 20);
    setConfig(CONFIG_BOOL_XP_DIVIDE_IN_DUNGEON, "XPDivideInDungeon", false);
    setConfig(CONFIG_BOOL_XP_DIVIDE_IN_RAID, "XPDivideInRaid", false);
    setConfig(CONFIG_BOOL_XP_DIVIDE_IN_WORLD, "XPDivideInWorld", false);
    setConfig(CONFIG_BOOL_WEB_DATABASE_ENABLED, "WebDatabase.Enabled", false);

    setConfig(CONFIG_BOOL_ENABLE_CLIENT_REGEX, "ClientAntispam", false);
}
...
void World::LoadNostalriusConfig(bool reload)
{
    ...
    setConfig(CONFIG_UINT32_ITEM_INSTANTSAVE_QUALITY,                   "Item.InstantSave.Quality", ITEM_QUALITY_EPIC);
    ...
    setConfig(CONFIG_BOOL_SURPRISE_BOX,                                 "ElysiumSurpriseBox.Enable", false);
    setConfig(CONFIG_UINT32_MIN_GOLD_FOR_SURPRISE_BOX,                  "ElysiumSurpriseBox.MinGold", 0);
    setConfig(CONFIG_UINT32_MAX_GOLD_FOR_SURPRISE_BOX,                  "ElysiumSurpriseBox.MaxGold", 0);
    setConfig(CONFIG_UINT32_MIN_LVL_FOR_SURPRISE_BOX,                   "ElysiumSurpriseBox.Minlvl", 10);

    setConfig(CONFIG_BOOL_AUTOBROADCAST_USE_CONST_TIMER, "AutoBroadcast.UseConstTimer", true);
    ...
    setConfig(CONFIG_UINT32_AREA_EXPLORATION_TIME_THRESHOLD, "AreaExplorationTimeThreshold", 30);

    setConfig(CONFIG_INT32_XP_DIVIDE_THRESHOLD, "XPDivideThreshold", 20);
    setConfig(CONFIG_INT32_XP_LOG_SAVE_PERIOD, "XPLogSavePeriod", 30);

    ///< Premium accounts

    setConfig(CONFIG_BOOL_PREMIUM_INSTANT_FLY_ENABLED, "Premium.InstantFly.Enabled", true);
    setConfig(CONFIG_BOOL_PREMIUM_INSTANT_INGAME_MAIL_ENABLED, "Premium.InGameMale.Enabled", true);

    setConfig(CONFIG_UINT32_PREMIUM_SPEC_CHANGE_DISCOUNT, "Premium.Discount.SpecChange", 25);
    if (getConfig(CONFIG_UINT32_PREMIUM_SPEC_CHANGE_DISCOUNT) > 100)
    {
        sLog.outError("WORLD: CONFIG_UINT32_PREMIUM_SPEC_CHANGE_DISCOUNT value is too big (%u). Max is 100", getConfig(CONFIG_UINT32_PREMIUM_SPEC_CHANGE_DISCOUNT));
        setConfig(CONFIG_UINT32_PREMIUM_SPEC_CHANGE_DISCOUNT, 100);
    }

    setConfig(CONFIG_UINT32_PREMIUM_ITEMS_REPAIR_DISCOUNT, "Premium.Discount.ItemsRepair", 25);
    if (getConfig(CONFIG_UINT32_PREMIUM_ITEMS_REPAIR_DISCOUNT) > 100)
    {
        sLog.outError("WORLD: CONFIG_UINT32_PREMIUM_ITEMS_REPAIR_DISCOUNT value is too big (%u). Max is 100", getConfig(CONFIG_UINT32_PREMIUM_ITEMS_REPAIR_DISCOUNT));
        setConfig(CONFIG_UINT32_PREMIUM_ITEMS_REPAIR_DISCOUNT, 100);
    }

    setConfig(CONFIG_UINT32_PREMIUM_HEARTHSTONE_CD_REDUCTION, "Premium.Reduction.HearthstoneCD", 50);
    if (getConfig(CONFIG_UINT32_PREMIUM_HEARTHSTONE_CD_REDUCTION) > 100)
    {
        sLog.outError("WORLD: CONFIG_UINT32_PREMIUM_HEARTHSTONE_CD_REDUCTION value is too big (%u). Max is 100", getConfig(CONFIG_UINT32_PREMIUM_HEARTHSTONE_CD_REDUCTION));
        setConfig(CONFIG_UINT32_PREMIUM_HEARTHSTONE_CD_REDUCTION, 100);
    }

    setConfig(CONFIG_UINT32_PREMIUM_HERBALISM_SKILL_MOD, "Premium.SkillUpMod.Herbalism", 50);
    setConfig(CONFIG_UINT32_PREMIUM_MINING_SKILL_MOD, "Premium.SkillUpMod.Mining", 50);
    setConfig(CONFIG_UINT32_PREMIUM_SKINNING_SKILL_MOD, "Premium.SkillUpMod.Skinning", 50);
    setConfig(CONFIG_UINT32_PREMIUM_ALCHEMY_SKILL_MOD, "Premium.SkillUpMod.Alchemy", 50);
    setConfig(CONFIG_UINT32_PREMIUM_BLACKSMITHING_SKILL_MOD, "Premium.SkillUpMod.Blacksmithing", 50);
    setConfig(CONFIG_UINT32_PREMIUM_ENCHANTING_SKILL_MOD, "Premium.SkillUpMod.Enchanting", 50);
    setConfig(CONFIG_UINT32_PREMIUM_ENGINEERING_SKILL_MOD, "Premium.SkillUpMod.Engineering", 50);
    setConfig(CONFIG_UINT32_PREMIUM_LEATHERWORKING_SKILL_MOD, "Premium.SkillUpMod.Leatherworking", 50);
    setConfig(CONFIG_UINT32_PREMIUM_TAILORING_SKILL_MOD, "Premium.SkillUpMod.Tailoring", 50);
    setConfig(CONFIG_UINT32_PREMIUM_COOKING_SKILL_MOD, "Premium.SkillUpMod.Cooking", 50);
    setConfig(CONFIG_UINT32_PREMIUM_FIRST_AID_SKILL_MOD, "Premium.SkillUpMod.FirstAid", 50);
    setConfig(CONFIG_UINT32_PREMIUM_FISHING_SKILL_MOD, "Premium.SkillUpMod.Fishing", 50);
    ...
    setConfig(CONFIG_BOOL_EVENT_CHAT_UNITED, "EventChatUnited", false);

    std::string vipsVisuals = sConfig.GetStringDefault("Premium.Visuals", "22518 22576 22577 22578 22579 22580 22581 25039 ");

    size_t pos = vipsVisuals.find(' ');
    size_t initialPos = 0;

    while (pos != std::string::npos)
    {
        m_VIPsAllowableVisuals.insert(std::stoul(vipsVisuals.substr(initialPos, pos - initialPos)));
        initialPos = pos + 1;
        pos = vipsVisuals.find(' ', initialPos);
    }
}
...
bool World::IsHonorEnabled() const
{
    auto const configValue = getConfig(CONFIG_UINT32_HONOR_ENABLE);

/*
        Default: 0 - determined by the patch level (>= 1.4 means enabled)
                 1 - explicitly enable honor sytem, regardless of patch level
                 2 - explicitly disable honor system, regardless of patch level
*/
    ...
    return GetWowPatch() >= WOW_PATCH_104;
}
...
void World::SetInitialWorldSettings()
{
    ...
    sLog.outString("Loading client antispam regexes");
    sObjectMgr.LoadClientSpamRegexes();
...
public:
    ...
    explicit WorldWorldTextBuilder(int32 textId, va_list* args = nullptr, bool isNotification = false) : i_textId(textId), i_args(args), b_isNonification(isNotification) {}
    ...
    void do_helper(WorldPacketList& data_list, char* text)
    {
        ...
        while (char* line = lineFromMessage(pos))
        {
            ...
            if (b_isNonification)
            {
                ...
                data->Initialize(SMSG_NOTIFICATION, msg.size() + 1);                // guess size
                *data << msg;
            }
            else
            {
...
void World::SendWorldNotification(int32 string_id, ...)
{
    va_list ap;
    va_start(ap, string_id);

    MaNGOS::WorldWorldTextBuilder wt_builder(string_id, &ap, true);
    MaNGOS::LocalizedPacketListDo<MaNGOS::WorldWorldTextBuilder> wt_do(wt_builder);
    for (SessionMap::iterator itr = m_sessions.begin(); itr != m_sessions.end(); ++itr)
    {
        if (!itr->second || !itr->second->GetPlayer() || !itr->second->GetPlayer()->IsInWorld())
...
void World::ShutdownMsg(bool show, Player* player)
{
            ...
            (m_ShutdownTimer > 12 * HOUR   && (m_ShutdownTimer % (12 * HOUR)) == 0))
    {
        ...
        if (!GetRestartReason().empty())
            str += "Reason: " + GetRestartReason();
