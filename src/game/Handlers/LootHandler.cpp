...
void WorldSession::HandleLootMasterGiveOpcode(WorldPacket & recv_data)
{
    ...
    if (!_player->GetGroup() || _player->GetGroup()->GetLootMethod() != MASTER_LOOT || _player->GetGroup()->GetMasterLooterGuid() != _player->GetObjectGuid())
    {
    ...
    if (Item* newitem = target->StoreNewItem(dest, item.itemid, true, item.randomPropertyId))
    {
        ...
        target->OnReceivedItem(newitem, uint32(item.count), uint32(item.itemid), true);
    }
