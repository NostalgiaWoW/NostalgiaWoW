...
void WorldSession::HandleBattleFieldPortOpcode(WorldPacket &recv_data)
{
    ...
    switch (action)
    {
        case 1:                                         // port to battleground
            ...
            _player->GetSession()->SendPacket(&data);
            // Not reloadable. Restart if you want it changed.
            static const bool clearAllQueues = sConfig.GetBoolDefault("ResetQueueOnPort", true);
            if (clearAllQueues)
                sBattleGroundMgr.RemovePlayerFromQueues(_player, queueSlot, false);
            else
