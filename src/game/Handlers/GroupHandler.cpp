...
void WorldSession::HandleGroupInviteOpcode(WorldPacket & recv_data)
{
    ...
    // just ignore us
	MasterPlayer* mPlayer = player->GetSession()->GetMasterPlayer();
	PlayerSocialPointer social = sSocialMgr.GetPlayerSocial(mPlayer->GetObjectGuid());
    if (social->HasIgnore(GetPlayer()->GetObjectGuid()))
    {
...
void WorldSession::HandleGroupRaidConvertOpcode(WorldPacket & /*recv_data*/)
{
    ...
    if (_player->getLevel() <= 49)
    {
        SendPartyResult(PARTY_OP_INVITE, "", ERR_GROUP_FULL); // the messages are limited, can't set yup a custom one
        return;
    }
