...
void WorldSession::HandleMessagechatOpcode(WorldPacket & recv_data)
{
    ...
    sChatHookMgr.HookMessage(_player, (ChatMsg)type, msg);

    if (type != CHAT_MSG_AFK || type != CHAT_MSG_DND)
    {
        GetPlayer()->UpdateChatActivityTimer();
    }
    ...
    switch (type)
    {
        case CHAT_MSG_CHANNEL:
        {
            ...
            std::vector<ChannelMgr*> channels;
            if (ChannelMgr* cMgr = channelMgr(playerPointer->GetTeam()))
                channels.push_back(cMgr);
            if (GetSecurity() > SEC_PLAYER)
                if (ChannelMgr* cMgr = channelMgr(playerPointer->GetTeam() == ALLIANCE ? HORDE : ALLIANCE))
                    channels.push_back(cMgr);

            for (ChannelMgr* cMgr : channels)
            {
                if (!cMgr)
                    continue;
                ...
                if (Channel* chn = cMgr->GetChannel(channel, playerPointer, IsMaster()))
                {
                    ...
                    {
                        ChatHandler(this).PSendSysMessage(LANG_CHAN_LVL_LIMIT, sWorld.getConfig(CONFIG_UINT32_WORLD_CHAN_MIN_LEVEL));
                        return;
                    }
                    ...
                    if (!chn->HasFlag(Channel::CHANNEL_FLAG_CUSTOM))
                    {
                        ...
                        {
                            ChatHandler(this).SendSysMessage(LANG_CHAN_GM_BLOCK);
                            return;
                        }
                        ...
                        if (sWorld.getConfig(CONFIG_BOOL_STRICT_LATIN_IN_GENERAL_CHANNELS))
                        {
                            ...
                            if (Utf8toWStr(normMsg, w_normMsg))
                            {
                                if (!isBasicLatinString(w_normMsg, true))
                                {
                                    if (!isBasicLatinString(w_normMsg, true))
                                    {
                                        ChatHandler(this).SendSysMessage(LANG_CHAN_LATIN_ONLY);
                                        return;
                                    }
                                }
                            }
                    ...
                    sChatHookMgr.HookChannelMessage(_player, channel, msg);
            ...
            if (lang != LANG_ADDON && !_anticheat->IsSilenced())
            {
        ...
        case CHAT_MSG_SAY:
            if (GetPlayer()->getLevel() < sWorld.getConfig(CONFIG_UINT32_SAY_MIN_LEVEL)
                && GetAccountMaxLevel() < sWorld.getConfig(CONFIG_UINT32_PUB_CHANS_MUTE_VANISH_LEVEL))
            {
                ChatHandler(this).PSendSysMessage(LANG_SAY_LVL_LIMIT, sWorld.getConfig(CONFIG_UINT32_SAY_MIN_LEVEL));
                return;
            }
        ...
        case CHAT_MSG_EMOTE:
            if (GetPlayer()->getLevel() < sWorld.getConfig(CONFIG_UINT32_SAY_EMOTE_MIN_LEVEL)
                && GetAccountMaxLevel() < sWorld.getConfig(CONFIG_UINT32_PUB_CHANS_MUTE_VANISH_LEVEL))
            {
                ChatHandler(this).PSendSysMessage(LANG_EMOTE_LVL_LIMIT, sWorld.getConfig(CONFIG_UINT32_SAY_EMOTE_MIN_LEVEL));
                return;
            }
        ...
        case CHAT_MSG_YELL:
        {
            if (GetPlayer()->getLevel() < sWorld.getConfig(CONFIG_UINT32_YELL_MIN_LEVEL)
                && GetAccountMaxLevel() < sWorld.getConfig(CONFIG_UINT32_PUB_CHANS_MUTE_VANISH_LEVEL))
            {
                ChatHandler(this).PSendSysMessage(LANG_YELL_LVL_LIMIT, sWorld.getConfig(CONFIG_UINT32_YELL_MIN_LEVEL));
                return;
            }
        ...
        case CHAT_MSG_WHISPER: // Master Side
        {
            ...
            if (tSecurity == SEC_PLAYER && pSecurity == SEC_PLAYER)
            {
                ...
                {
                    ChatHandler(this).PSendSysMessage(LANG_WHISPER_LVL_LIMIT, sWorld.getConfig(CONFIG_UINT32_WHISP_DIFF_ZONE_MIN_LEVEL));
                    return;
                }
            }
            ...
            if (Player* toPlayer = player->GetSession()->GetPlayer())
            {
                ...
                if (masterPlr->isGameMaster() || allowSendWhisper)
                {
                    if (lang != LANG_ADDON)
                    {
                        sChatHookMgr.HookMessage(masterPlr, player, msg);
                    }
        ...
        case CHAT_MSG_PARTY: // Master Side: TODO
        {
            ...
            sChatHookMgr.HookMessage(group, data);
            if (lang != LANG_ADDON && !_anticheat->IsSilenced())
        ...
        case CHAT_MSG_GUILD: // Master side
        {
            ...
            if (GetMasterPlayer()->GetGuildId())
            {
                if (Guild* guild = sGuildMgr.GetGuildById(GetMasterPlayer()->GetGuildId()))
                {
                    WorldPacket data;
                    ChatHandler::FillMessageData(&data, this, type, lang, msg.c_str());
                    guild->BroadcastToGuild(this, data);
                    sChatHookMgr.HookMessage(guild, data);
                }
            }
        ...
        case CHAT_MSG_OFFICER: // Master side
        {
            ...
            if (GetMasterPlayer()->GetGuildId())
            {
                if (Guild* guild = sGuildMgr.GetGuildById(GetMasterPlayer()->GetGuildId()))
                {
                    WorldPacket data;
                    ChatHandler::FillMessageData(&data, this, type, lang, msg.c_str());
                    guild->BroadcastToOfficers(this, data);
                    sChatHookMgr.HookMessage(guild, data);
                }
            }
...
void WorldSession::HandleChatFilteredOpcode(WorldPacket & recvPacket)
{
    ObjectGuid filteredPlayerGuid;
    recvPacket >> filteredPlayerGuid;

    Player* filteredPlayer = sObjectAccessor.FindPlayer(filteredPlayerGuid);
    if (filteredPlayer != nullptr)
        filteredPlayer->IncrementRegexFilterCounter();
}
