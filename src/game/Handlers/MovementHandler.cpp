...
void WorldSession::HandleMoveWorldportAckOpcode()
{
    ...
    const Position pos { loc.coord_x, loc.coord_y, loc.coord_z };
    _anticheat->Teleport(pos);
...
void WorldSession::HandleMoveTeleportAckOpcode(WorldPacket& recv_data)
{
    ...
    //Giperion: Check if mover is not deleted
    _player->ValidateMover();
    ...
    const Position pos { dest.coord_x, dest.coord_y, dest.coord_z };
    _anticheat->Teleport(pos);
...
void WorldSession::HandleMovementOpcodes(WorldPacket & recv_data)
{
    ...
    // two extra fields with this packet
    if (recv_data.GetOpcode() == CMSG_MOVE_SPLINE_DONE)
    {
        ...
        // since this packet is server-generated, it can cause problems if we pass it to the anticheat
        skipAnticheat = true;
    }
    ...
    if (!skipAnticheat && !_anticheat->Movement(movementInfo, recv_data))
        return;

    //If anticheat says ok, we still can detect suspisious activities over time
    sSuspiciousStatisticMgr.OnMovement(plMover, movementInfo);
    ...
    //Giperion Elysium: This is fix for moment, when you jump in flying mode, which result a dropping swimming flag.
    //Client do that without asking server, so we must correct client.
    //bool bFlyFix = false;
...
void WorldSession::HandleMoveNotActiveMoverOpcode(WorldPacket &recv_data)
{
    ...
    sSuspiciousStatisticMgr.OnMovement(_player, mi);
...
void WorldSession::HandleMoverRelocation(MovementInfo& movementInfo)
{
    ...
    if (Player* plMover = mover->ToPlayer())
    {
        ...
        if (movementInfo.HasMovementFlag(MOVEFLAG_ONTRANSPORT))
        {
            ...
            if (!plMover->GetTransport())
            {
                if (Transport* t = plMover->GetMap()->GetTransport(movementInfo.GetTransportGuid()))
                {
                    ...
                    plMover->UpdateSavedVelocityPositionToCurrentPos();
        ...
        if (movementInfo.HasMovementFlag(MOVEFLAG_SWIMMING) != plMover->IsInWater())
        {
            ...
            plMover->SetInWater(plMover->GetTerrain()->IsInWater(movementInfo.GetPos()->x, movementInfo.GetPos()->y, movementInfo.GetPos()->z));
        }
        ...
        plMover->UpdateMovementActivityTimer();

        // super smart decision; rework required
        ObjectGuid lootGuid;

        if ((lootGuid = plMover->GetLootGuid()) && !lootGuid.IsItem())
