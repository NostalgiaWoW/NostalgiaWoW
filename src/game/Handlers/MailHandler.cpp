...
void WorldSession::HandleSendMailCallback(WorldSession::AsyncMailSendRequest* req)
{
    ...
    if (reqmoney < req->money)
    {
        ...
        sLog.outError("Player %s (Account: %u) tried to send free mails with money overflow!", GetPlayerName(), GetAccountId());
    ...
    // Antispam checks
    if (loadedPlayer->GetSession()->GetAccountId() != rc_account && loadedPlayer->getLevel() < sWorld.getConfig(CONFIG_UINT32_MAILSPAM_LEVEL) /*
        req->money < sWorld.getConfig(CONFIG_UINT32_MAILSPAM_MONEY) &&
        (sWorld.getConfig(CONFIG_BOOL_MAILSPAM_ITEM) && !req->itemGuid)*/)
    {
        ChatHandler(this).PSendSysMessage(LANG_MAIL_ANTISPAM, sWorld.getConfig(CONFIG_UINT32_MAILSPAM_LEVEL));
        pl->SendMailResult(0, MAIL_SEND, MAIL_ERR_INTERNAL_ERROR);
        return;
    }

    // cannot send money in a COD mail, only possible with client modifications
    if (req->money && req->COD)
    {
        _anticheat->RecordCheat(CHEAT_ACTION_INFO_LOG, "MailCheck", "Attempt to dupe gold via COD mail");
        sLog.outError("Player %s (Account: %u) tried to dupe gold via COD mail!", GetPlayerName(), GetAccountId());
        return;
    }
    ...
    if (loadedPlayer->CanInstantMail())
        needItemDelay = false;
    ...
    if (!item && req->COD)
    {
        ...
        sLog.outError("Player %s (Account: %u) tried to send COD mail without any item!", GetPlayerName(), GetAccountId());
    }
...
void WorldSession::HandleMailTakeItem(WorldPacket & recv_data)
{
    ...
    Mail* m = pl->GetMail(mailId);
    if (!m || m->state == MAIL_STATE_DELETED || m->deliver_time > time(nullptr) || !m->HasRetrievableItems())
    {
        pl->SendMailResult(mailId, MAIL_ITEM_TAKEN, MAIL_ERR_INTERNAL_ERROR);
        return;
    }

    ASSERT(!m->items.empty());
