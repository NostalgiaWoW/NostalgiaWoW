...
bool LoginQueryHolder::Initialize()
{
    ...
    res &= SetPQuery(PLAYER_LOGIN_QUERY_LOADFROM,            "SELECT guid, account, name, race, class, gender, level, xp, money, playerBytes, playerBytes2, playerFlags, "
                     ...
                     "world_phase_mask, customFlags, regexFilterCount FROM characters WHERE guid = '%u'", m_guid.GetCounter());
    ...
    res &= SetPQuery(PLAYER_LOGIN_QUERY_BROKEN_ITEMS,        "SELECT item_template, item FROM character_inventory as ci WHERE ci.guid = '%u' AND NOT EXISTS (SELECT * FROM item_instance AS ic WHERE ic.`guid` = ci.`item`)", m_guid.GetCounter());
...
{
    const static uint32 c_MaxCharacters = 10;

    uint32 l_LimitMin = m_Socket->GetCharacterPageIdx() * c_MaxCharacters;
    uint32 l_LimitMax = l_LimitMin + c_MaxCharacters;
    if (m_Socket->GetCharacterPageIdx() > 0)
        m_notOnFirstPage = true;
    ...
    CharacterDatabase.AsyncPQuery(&chrHandler, &CharacterHandler::HandleCharEnumCallback, GetAccountId(),
                                  ...
                                  "LIMIT %u,%u",
                                  PET_SAVE_AS_CURRENT, GetAccountId(), l_LimitMin, l_LimitMax);
}
...
void WorldSession::HandleCharCreateOpcode(WorldPacket & recv_data)
{
    ...
    /// Don't send Surprise Box for new characters
    CharacterDatabase.PExecute("INSERT INTO `received_surprise_box` (`account`, `charGuid`) VALUES ('%u', '%u')", GetAccountId(), pNewChar->GetGUIDLow());
...
void WorldSession::HandlePlayerLogin(LoginQueryHolder *holder)
{
    ...
    if (sWorld.getConfig(CONFIG_BOOL_ENABLE_CLIENT_REGEX))
	...
	MasterPlayer* mPlayer = pCurrChar->GetSession()->GetMasterPlayer();
	PlayerSocialPointer social = sSocialMgr.GetPlayerSocial(mPlayer->GetObjectGuid());
	social->SendFriendList();
	social->SendIgnoreList();
    ...
    if (alreadyOnline)
    {
        ...
        if (pCurrChar->getDeathState() == CORPSE && pCurrChar->FindMap())
    ...
    time_t l_LastClaimTime = GetLastClaim();
    ...
    if (l_LastClaim->tm_year != l_Now.tm_year || l_LastClaim->tm_mday != l_Now.tm_mday || l_LastClaim->tm_mon != l_Now.tm_mon)
    {
        ...
        if (GetLastBan() < (time(nullptr) - (MONTH * 6)))
            l_Points += 2;

        if (GetActivityDays() > 13)
            l_Points += 1;

        if (GetActivityDays() > 59)
            l_Points += 1;

        if (GetActivityDays() > 179)
            l_Points += 1;

        if (GetActivityDays() > 359)
            l_Points += 1;

        AddLoyaltyPoints(l_Points, "Daily reward");
        SetLastClaim(time(nullptr));
        LoginDatabase.PExecute("REPLACE INTO account_loyalty(AccountID, LastClaim, LastEventReset) VALUES (%u, %u, %u)", GetAccountId(), GetLastClaim(), GetLastEventReset());
    }
    ...
    if (sWorld.getConfig(CONFIG_BOOL_TRANSMOG_ENABLED))
        ...
        pCurrChar->SetForced_X1_XpRate(false);
     pCurrChar->ShowCurrentXpRate();
}
