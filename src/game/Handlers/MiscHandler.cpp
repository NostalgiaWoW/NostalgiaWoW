...
void WorldSession::HandleLogoutRequestOpcode(WorldPacket & /*recv_data*/)
{
    ...
    if (GetPlayer()->HasFlag(PLAYER_FLAGS, PLAYER_FLAGS_RESTING) || GetPlayer()->IsTaxiFlying() ||
            GetSecurity() >= (AccountTypes)sWorld.getConfig(CONFIG_UINT32_INSTANT_LOGOUT) || GetPlayer()->GetAreaId() == 2401 
			|| GetPlayer()->GetAreaId() == 2317)
    {
        WorldPacket data(SMSG_LOGOUT_RESPONSE, 1 + 4);
        data << uint32(0);
        data << uint8(1);
...
void WorldSession::HandleFriendListOpcode(WorldPacket & recv_data)
{
	...
	sSocialMgr.GetPlayerSocial(GetMasterPlayer()->GetObjectGuid())->SendFriendList();
}
...
void WorldSession::HandleAddFriendOpcode(WorldPacket & recv_data)
{
    ...
    if (friendGuid)
    {
        ...
        else if (sSocialMgr.GetPlayerSocial(GetMasterPlayer()->GetObjectGuid())->HasFriend(friendGuid))
        ...
        else
        {
            ...
            if (!sSocialMgr.GetPlayerSocial(GetMasterPlayer()->GetObjectGuid())->AddToSocialList(friendGuid, false))
            {
...
void WorldSession::HandleDelFriendOpcode(WorldPacket & recv_data)
{
	...
	sSocialMgr.GetPlayerSocial(GetMasterPlayer()->GetObjectGuid())->RemoveFromSocialList(friendGuid, false);
...
void WorldSession::HandleAddIgnoreOpcode(WorldPacket & recv_data)
{
    ...
    if (ignoreGuid)
    {
        ...
        else if (sSocialMgr.GetPlayerSocial(GetMasterPlayer()->GetObjectGuid())->HasIgnore(ignoreGuid))
        ...
        else
        {
            ...
            // ignore list full
            if (!sSocialMgr.GetPlayerSocial(GetMasterPlayer()->GetObjectGuid())->AddToSocialList(ignoreGuid, true))
...
void WorldSession::HandleDelIgnoreOpcode(WorldPacket & recv_data)
{
	...
	sSocialMgr.GetPlayerSocial(GetMasterPlayer()->GetObjectGuid())->RemoveFromSocialList(ignoreGuid, true);
...
void WorldSession::HandleAreaTriggerOpcode(WorldPacket & recv_data)
{
    ...
    if (!pl->isGameMaster())
    {
        ...
        /// Molten Core
        /// If player want to enter Molten Core from Blackrock Depths, he must be able to do it even without quest
        if (at->target_mapId == 409 && at->requiredQuest && pl->GetMapId() == 230)
            missingQuest = 0;
