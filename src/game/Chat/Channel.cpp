        ...
        m_securityLevel(0), m_area_dependant(true), m_levelRestricted(true)
{
    ...
    else                                                    // it's custom channel
    {
        ...
        if (m_name == u8"World" || m_name == u8"Twink" || m_name == u8"Ru")
        {
...
void Channel::Invite(ObjectGuid p, const char *newname)
{
    ...
    WorldPacket data;
	PlayerSocialPointer social = sSocialMgr.GetPlayerSocial(newp->GetSession()->GetMasterPlayer()->GetObjectGuid());
    if (!social->HasIgnore(p))
    {
        MakeInvite(&data, p);
...
void Channel::SendToAll(WorldPacket *data, ObjectGuid p)
{
    for (PlayerList::const_iterator i = m_players.begin(); i != m_players.end(); ++i)
    {
		...
		{
			PlayerSocialPointer social = sSocialMgr.GetPlayerSocial(plr->GetSession()->GetMasterPlayer()->GetObjectGuid());
			if (!social->HasIgnore(p))
				plr->GetSession()->SendPacket(data);
		}
    }
