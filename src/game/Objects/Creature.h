...
enum CreatureFlagsExtra
{
    ...
    CREATURE_FLAG_EXTRA_DISABLE_PATHFINDING          = 0x00004000,       // creature will not use pathfinding
    CREATURE_FLAG_EXTRA_DISABLE_MOVEMENT             = 0x00008000,       // creature can't move
};
...
enum EvadeMode
{
    EVADE_MODE_NONE = 0,        // not in evade mode
    EVADE_MODE_WAITING,         // waiting for target to become reachable again
    EVADE_MODE_REGENERATING,    // waiting for target but also regenerating
    EVADE_MODE_RESETTING        // given up waiting for target.  moving home.
};
...
struct VendorItem
{
    VendorItem(uint32 _item, uint32 _maxcount, uint32 _incrtime, uint32 _conditionId)
...
struct VendorItemData
{
    ...
    void AddItem( uint32 item, uint32 maxcount, uint32 ptime, uint32 conditionId)
    {
        m_items.push_back(new VendorItem(item, maxcount, ptime, conditionId));
    }
...
class MANGOS_DLL_SPEC Creature : public Unit
{
    ...
    public:
        ...
        EvadeMode GetEvadeMode() const { return m_evadeMode; }

        // this isn't exactly right as there are other evade modes, but the places where
        // the other modes are relevant are currently being handled manually/individually
        bool IsInEvadeMode() const { return GetEvadeMode() == EVADE_MODE_RESETTING; }
        ...
        void setRegeneratingHealth(bool regen) { m_regenHealth = regen; }
    ...
    private:
        static constexpr uint32 EvadeWait = 5 * IN_MILLISECONDS;
        static constexpr uint32 EvadeRegenerate = 10 * IN_MILLISECONDS;
        static constexpr uint32 EvadeReset = 15 * IN_MILLISECONDS;

        uint32 m_evadeTimer;
        EvadeMode m_evadeMode;

        void UpdateEvadeMode(uint32 diff);
