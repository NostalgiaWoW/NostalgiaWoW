...
enum UnitFlags
{
    ...
    UNIT_FLAG_UNK_0                 = 0x00000001, //1
    UNIT_FLAG_NON_ATTACKABLE        = 0x00000002, //2        // not attackable
    UNIT_FLAG_DISABLE_MOVE          = 0x00000004, //3
    UNIT_FLAG_PVP_ATTACKABLE        = 0x00000008, //4       // allow apply pvp rules to attackable state in addition to faction dependent state, UNIT_FLAG_UNKNOWN1 in pre-bc mangos
    UNIT_FLAG_PET_RENAME            = 0x00000010, //5       // Old pet rename: moved to UNIT_FIELD_BYTES_2,2 in TBC+
    UNIT_FLAG_PET_ABANDON           = 0x00000020, //6       // Old pet abandon: moved to UNIT_FIELD_BYTES_2,2 in TBC+
    UNIT_FLAG_UNK_6                 = 0x00000040, //7
    UNIT_FLAG_OOC_NOT_ATTACKABLE    = 0x00000100, //9       // (OOC Out Of Combat) Can not be attacked when not in combat. Removed if unit for some reason enter combat (flag probably removed for the attacked and it's party/group only)
    UNIT_FLAG_PASSIVE               = 0x00000200, //10      // makes you unable to attack everything. Almost identical to our "civilian"-term. Will ignore it's surroundings and not engage in combat unless "called upon" or engaged by another unit.
    UNIT_FLAG_PVP                   = 0x00001000, //13
    UNIT_FLAG_SILENCED              = 0x00002000, //14      // silenced, 2.1.1
    UNIT_FLAG_UNK_14                = 0x00004000, //15
    UNIT_FLAG_USE_SWIM_ANIMATION    = 0x00008000, //16
    UNIT_FLAG_UNK_16                = 0x00010000, //17      // removes attackable icon
    UNIT_FLAG_PACIFIED              = 0x00020000, //18
    UNIT_FLAG_DISABLE_ROTATE        = 0x00040000, //19
    UNIT_FLAG_IN_COMBAT             = 0x00080000, //20
    UNIT_FLAG_NOT_SELECTABLE        = 0x02000000, //22
    UNIT_FLAG_SKINNABLE             = 0x04000000, //23
    UNIT_FLAG_AURAS_VISIBLE         = 0x08000000, //24      // magic detect
    UNIT_FLAG_SHEATHE               = 0x40000000, //27
    //UNIT_FLAG_UNK_31              = 0x80000000            // no effect in 1.12.1
    UNIT_FLAG_NO_KILL_REWARD        = 0x80000000, //28      // Unit should yield no reward (Honor/XP/Rep) on kill
...
class MANGOS_DLL_SPEC Unit : public WorldObject
{
    public:
        ...
        bool CanMove() const
        {
            return !hasUnitState(UNIT_STAT_CAN_NOT_MOVE) && !HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_DISABLE_MOVE);
        }
        ...
        void SendPacketSetWaterWalking(bool apply);
        ...
        void HandleEmoteAfterQuestGossip(uint32 emote_id);
        ...
        /* Ends the current movement instruction
           force MUST be set when spline is disabled and the unit is a player.
           If it's not set in this case, this function has almost zero effect.
           Best likely case is to force = IsPlayer()
        */
        ...
        // general case of the function above, which would check if the unit would be in melee range if it were located at the given position
        bool IsWithinMeleeRange(float x, float y, float z, const Unit *obj, float dist = 0.0f) const;
        ...
        bool HasEmoteAfterQuestGossip() const;
        uint32 GetEmoteAfterQuestGossip() const;
        void RegisterAttack(ObjectGuid guid, uint8 level);
        std::unordered_map<ObjectGuid, uint8> const& GetAllAttackers() const { return m_allAttackers; }
    ...
    protected:
        ...
        bool m_spellUpdateHappening = false;
    ...
    protected:
        ...
        uint32 m_emoteOnPlayerGossip = 0;
    ...
    private:
        ...
        std::unordered_map<ObjectGuid, uint8> m_allAttackers;
