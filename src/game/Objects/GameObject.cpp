...
bool GameObject::IsVisible(const Player* player) const
{
    if (m_visible)
    {
        if (GameObjectAI* goAI = AI())
        {
            return goAI->IsVisibleForPlayer(player);
        }
        return true;
    }
    return false;
        ...
        case GAMEOBJECT_TYPE_FISHINGNODE:                   //17 fishing bobber
        {
            ...
            switch (getLootState())
            {
                case GO_READY:                              // ready for loot
                {
                    ...
                    //Record fishing attempt even if it was failed
