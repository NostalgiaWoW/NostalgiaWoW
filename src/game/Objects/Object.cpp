...
float WorldObject::GetDistance(const WorldObject* obj) const
{
    ...
    return (dist > FLT_SMALL_NUMBER ? dist : 0);
}

float WorldObject::GetDistance(Position pos) const
{
    float sizefactor = GetObjectBoundingRadius();
    float dist = pos.getDistance(GetPositionX(), GetPositionY(), GetPositionZ()) - sizefactor;
    return (dist > FLT_SMALL_NUMBER ? dist : 0);
}
...
public:
    ...
    void operator()(WorldPacket& data, int32 loc_idx)
    {
        ...
        vsnprintf(textFinal, sizeof(textFinal), text, argsCpy);
        va_end(argsCpy);
...
void WorldObject::GetGameObjectListInGrid(std::list<GameObject*>& lList, float fMaxSearchRange)
{
    CellPair pair(MaNGOS::ComputeCellPair(GetPositionX(), GetPositionY()));
    Cell cell(pair);
    cell.SetNoCreate();

    MaNGOS::AllGameObjectsInRange check(this, fMaxSearchRange);
    MaNGOS::GameObjectListSearcher<MaNGOS::AllGameObjectsInRange> searcher(lList, check);
...
void WorldObject::GetCreatureListInGrid(std::list<Creature*>& lList, float fMaxSearchRange)
{
    CellPair pair(MaNGOS::ComputeCellPair(GetPositionX(), GetPositionY()));
    Cell cell(pair);
    cell.SetNoCreate();

    MaNGOS::AllCreaturesInRange check(this, fMaxSearchRange);
    MaNGOS::CreatureListSearcher<MaNGOS::AllCreaturesInRange> searcher(lList, check);
...
const char* TempSummonType_ToString(TempSummonType value)
{
    switch (value)
    {
    case TEMPSUMMON_TIMED_OR_DEAD_DESPAWN:
        return "TEMPSUMMON_TIMED_OR_DEAD_DESPAWN";
    case TEMPSUMMON_TIMED_OR_CORPSE_DESPAWN:
        return "TEMPSUMMON_TIMED_OR_CORPSE_DESPAWN";
    case TEMPSUMMON_TIMED_DESPAWN:
        return "TEMPSUMMON_TIMED_DESPAWN";
    case TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT:
        return "TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT";
    case TEMPSUMMON_CORPSE_DESPAWN:
        return "TEMPSUMMON_CORPSE_DESPAWN";
    case TEMPSUMMON_CORPSE_TIMED_DESPAWN:
        return "TEMPSUMMON_CORPSE_TIMED_DESPAWN";
    case TEMPSUMMON_DEAD_DESPAWN:
        return "TEMPSUMMON_DEAD_DESPAWN";
    case TEMPSUMMON_MANUAL_DESPAWN:
        return "TEMPSUMMON_MANUAL_DESPAWN";
    case TEMPSUMMON_TIMED_COMBAT_OR_DEAD_DESPAWN:
        return "TEMPSUMMON_TIMED_COMBAT_OR_DEAD_DESPAWN";
    case TEMPSUMMON_TIMED_COMBAT_OR_CORPSE_DESPAWN:
        return "TEMPSUMMON_TIMED_COMBAT_OR_CORPSE_DESPAWN";
    default:
        return "UNDEFINED";
    }
}
...
Position::Position(std::initializer_list<float> list) : Position()
{
    const float* listIter = list.begin();

    int32 InitNum = 0;
    while (listIter != list.end() && InitNum < 4)
    {
        switch (InitNum)
        {
        case 0:
...
float Position::getDistance(Position& other) const
{
    return sqrt(getDistanceSquared(other));
}

float Position::getDistance(float InX, float InY, float InZ) const
{
    return sqrt(getDistanceSquared(InX, InY, InZ));
}

float Position::getDistanceSquared(Position& other) const
{
    return getDistanceSquared(other.x, other.y, other.z);
}

float Position::getDistanceSquared(float InX, float InY, float InZ) const
{
    float dX = x - InX;
    float dY = y - InY;
    float dZ = z - InZ;
