...
struct Position
{
    ...
    Position(std::initializer_list<float> list);
    float x, y, z, o;

    float getDistance(Position& other) const;
    float getDistance(float InX, float InY, float InZ) const;
    float getDistanceSquared(Position& other) const;
    float getDistanceSquared(float InX, float InY, float InZ) const;
...
class MANGOS_DLL_SPEC Object
{
    public:
        ...
        ObjectGuid GetObjectGuidSafe() const 
        { 
            if (m_uint32Values)
...
class MANGOS_DLL_SPEC WorldObject : public Object
{
    ...
    public:
        ...
        void GetPosition( WorldLocation &loc, Transport* onTransport = nullptr) const
            { loc.mapid = m_mapId; GetPosition(loc.coord_x, loc.coord_y, loc.coord_z, onTransport); loc.orientation = GetOrientation(); }
        ...
        float GetDistance(Position pos) const;
        ...
        void GetGameObjectListInGrid(std::list<GameObject*>& lList, float fMaxSearchRange);
        void GetCreatureListWithEntryInGrid(std::list<Creature*>& lList, uint32 uiEntry, float fMaxSearchRange);
        void GetCreatureListInGrid(std::list<Creature*>& lList, float fMaxSearchRange);
