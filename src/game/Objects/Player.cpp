...
int32 Player::getMaxTimer(MirrorTimerType timer)
{
    ...
    {
        case FATIGUE_TIMER:
            if (GetSession()->GetSecurity() >= (AccountTypes)sWorld.getConfig(CONFIG_UINT32_TIMERBAR_FATIGUE_GMLEVEL) || GetZoneId() == 2401)
...
{
    public:
        void HandleStoreItemCallback(QueryResult * result, uint64 guid)
        {
            if (Player* player = ObjectAccessor::FindPlayer(guid))
                player->LoadShopItem(result);

            delete result;
        }

        void HandleStoreLevelCallback(QueryResult* result, uint64 guid)
        {
            if (Player* player = ObjectAccessor::FindPlayer(guid))
                player->LoadShopLevel(result);

            delete result;
        }

        void HandleStoreGoldCallback(QueryResult* result, uint64 guid)
        {
            if (Player* player = ObjectAccessor::FindPlayer(guid))
                player->LoadShopGold(result);

            delete result;
        }

        void HandleStoreRaceChangeCallback(QueryResult* result, uint64 guid)
        {
            if (Player* player = ObjectAccessor::FindPlayer(guid))
                player->LoadShopRaceChange(result);

            delete result;
        }
} shopHandler;

void Player::LoadShopItem(QueryResult* result)
{
    if (result)
    {
        uint32 ShopError = 0;
        do
        {
            ...
            // noSpaceForCount > 0 = more items to add
            if (AddItem(ShopItemid, ShopCount, &noSpaceForCount) && !noSpaceForCount)
            {
                CharacterDatabase.BeginTransaction();
                CharacterDatabase.PExecute("DELETE FROM store_item WHERE transaction = %u", transaction);
                CharacterDatabase.PExecute("INSERT INTO log_store_item (transaction, date, guid, itemid, count) VALUES (%u, NOW(), %u, %u, %u)", transaction, GetGUIDLow(), ShopItemid, ShopCount);
                CharacterDatabase.CommitTransaction();
				
                sLog.out(LOG_CHAR, "[SHOP]: Tried to add %u x %u item(s) to player %u with transaction %u.", ShopItemid, ShopCount, GetGUIDLow(), transaction);
            }
            else
            {
                CharacterDatabase.BeginTransaction();
                CharacterDatabase.PExecute("UPDATE store_item SET count = %u WHERE transaction = %u", noSpaceForCount, transaction);
                ...
                {
                    CharacterDatabase.PExecute("INSERT INTO log_store_item (transaction, date, guid, itemid, count) VALUES (%u, NOW(), %u, %u, %u)", transaction, GetGUIDLow(), ShopItemid, ShopCount);
                    sLog.out(LOG_CHAR, "[SHOP]: Added %u x %u item(s) to player %u with transaction %u.", ShopItemid, ShopCount, GetGUIDLow(), transaction);
                }
        ...
        if (ShopError)
        {
            ...
            sLog.out(LOG_CHAR, "[SHOP]: Problem with adding item(s) to character %u, most likely no bag space.", GetGUIDLow());
        }
    }
}

void Player::LoadShopGold(QueryResult* result)
{
    // Load shop gold
    if (result)
    {
        ...
        do
        {
            ...
            int32 gold = (fieldGold[0].GetInt32()) * GOLD;
            uint32 transaction = fieldGold[1].GetUInt32();

            uint32 moneyBefore = GetMoney();

            int32 newMoney = int32(moneyBefore + gold);
            if (newMoney > MAX_MONEY_AMOUNT)
            {
                GetSession()->SendNotification("Total sum of you gold and gold that you buy is more then WoW gold limit");
                GetSession()->SendNotification("Please transfer you gold to bank or spend it. This operation will repeat at next login");
                break;
            }

            if (newMoney < 0)
            {
                sLog.out(LOG_CHAR, "[SHOP]: Player %u (%s) trying to cheat with gold selling. Current money %u, db %i, result %i", GetGUIDLow(), GetName(), moneyBefore, gold, newMoney);
                break;
            }
            ...
            CharacterDatabase.BeginTransaction();
            CharacterDatabase.PExecute("DELETE FROM store_gold WHERE transaction = %u", transaction);
            CharacterDatabase.PExecute("INSERT INTO log_store_gold (transaction, date, guid, gold, goldBefore, goldAfter) VALUES (%u, NOW(), %u, %i, %u, %u)", transaction, GetGUIDLow(), (gold / GOLD), (moneyBefore / GOLD), (GetMoney() / GOLD));
            CharacterDatabase.CommitTransaction();
        ...
        if (goldCount)
        {
            GetSession()->SendNotification("'%d' gold was added to your account", (goldCount / 10000));
            sLog.out(LOG_CHAR, "[SHOP]: Error while adding gold to player %u.", GetGUIDLow());
        }
    }
}

void Player::LoadShopLevel(QueryResult* result)
{
    // Load powerlevels
    if (result)
    {
        ...
        if (level < getLevel())
        {
            GetSession()->SendNotification("Internal server error (target powerlevel (%u) is less than player level (%u). Please contact a GM.", level, getLevel());
            sLog.out(LOG_CHAR, "[SHOP]: Target level %u is lower than player level %u of character %u.", level, getLevel(), GetGUIDLow());
        }
        else if (level > DEFAULT_MAX_LEVEL)
        {
            GetSession()->SendNotification("Internal server error (target powerlevel (%u) is greater then current game level limit (%u). Please contact a GM.", level, (uint32)DEFAULT_MAX_LEVEL);
            sLog.out(LOG_CHAR, "[SHOP]: Target level %u is greater than player level %u of character %u.", level, getLevel(), GetGUIDLow());
        }
        else
        {
            GiveLevel(level);

            CharacterDatabase.PExecute("DELETE FROM store_level WHERE guid = %u", GetGUIDLow());
        }
    }
}

void Player::LoadShopRaceChange(QueryResult* result)
{
    if (result)
    {
        Field* fields = result->Fetch();
        uint8 targetRace     = fields[0].GetUInt8();
        uint8 targetGender   = fields[1].GetUInt8();
        uint32 PlayerByte1   = fields[2].GetUInt32();
        uint32 PlayerByte2   = fields[3].GetUInt32();
        uint32 TransactionId = fields[4].GetUInt32();

        //Save previous race, gender, playerbyte1, playerbyte2
        uint8 PreviousRace          = getRace();
        uint8 PreviousGender        = getGender();
        uint32 PreviousPlayerByte1  = GetUInt32Value(PLAYER_BYTES);
        uint32 PreviousPlayerByte2  = GetUInt32Value(PLAYER_BYTES_2);
        uint32 GuidLow = GetGUIDLow();

        Team newTeam = TeamForRace(targetRace);
        bool bChangeTeam = (TeamForRace(PreviousRace) != newTeam);
        
        if (ChangeRace(targetRace, targetGender, PlayerByte1, PlayerByte2))
        {
            //WARNING: Player pointer is invalid at this point!!!
            //Delete request, and add log record

            CharacterDatabase.BeginTransaction();
            CharacterDatabase.PExecute("DELETE FROM store_racechange WHERE guid = %u", GuidLow);
            CharacterDatabase.PExecute("INSERT INTO `log_store_racechange` (`transaction`, `date`, `guid`, `previous_race`, `previous_gender`, `previous_playerbyte1`, `previous_playerbyte2`, `race`, `gender`, `playerbyte1`, `playerbyte2`)"
                                                                  " VALUES ('%u', NOW(), %u, '%hhu', '%hhu', '%u', '%u', '%hhu', '%hhu', '%u', '%u')",
                                                                  TransactionId, GuidLow, PreviousRace, PreviousGender, PreviousPlayerByte1, PreviousPlayerByte2, targetRace, targetGender, PlayerByte1, PlayerByte2);
            
            if (bChangeTeam)
            {
                // Wipe player's honour for that week
                CharacterDatabase.PExecute("DELETE FROM `character_honor_cp` WHERE `guid`=%u AND (`date` >= (SELECT `lastHonorMaintenanceDay` FROM `saved_variables` LIMIT 1))", GuidLow);
            }
            CharacterDatabase.CommitTransaction();

            sLog.out(LOG_CHAR, "Successfully change race of GUID: %u to race %hhu, gender %hhu", GuidLow, targetRace, targetGender);
        }
        else
        {
            //Don't delete request, we can fix issue and try again on next login
            sLog.out(LOG_CHAR, "Can't change race of Player \"%s\" (GUID: %llu) to race %hhu, gender %hhu", GetName(), GetObjectGuid().GetRawValue(), targetRace, targetGender);
        }
...
void Player::Update(uint32 update_diff, uint32 p_time)
{
    ...
    if (!_shopCallBackInitialized)
    {
        CharacterDatabase.AsyncPQuery(&shopHandler, &ShopHandler::HandleStoreItemCallback, GetGUID(), "SELECT itemid, count, transaction FROM store_item WHERE guid = %u", GetGUIDLow());
        CharacterDatabase.AsyncPQuery(&shopHandler, &ShopHandler::HandleStoreLevelCallback, GetGUID(), "SELECT level FROM store_level WHERE guid = %u", GetGUIDLow());
        CharacterDatabase.AsyncPQuery(&shopHandler, &ShopHandler::HandleStoreGoldCallback, GetGUID(), "SELECT gold, transaction FROM store_gold WHERE guid = %u", GetGUIDLow());
        CharacterDatabase.AsyncPQuery(&shopHandler, &ShopHandler::HandleStoreRaceChangeCallback, GetGUID(), "SELECT race, gender, playerbytes1, playerbytes2, transaction FROM store_racechange WHERE guid = %u", GetGUIDLow());

        _shopCallBackInitialized = true;
    }
...
bool Player::ExecuteTeleportFar(ScheduledTeleportData *data)
{
    Map * oldmap = IsInWorld() ? GetMap() : NULL;
    // check if we can enter before stopping combat / removing pet / totems / interrupting spells
    if (oldmap && !oldmap->CanExit(this))
    {
        ChatHandler(this).SendSysMessage("You can't leave this instance right now!");
        return false;
    }
...
void Player::AddToWorld()
{
    ...
    GetPosition(m_lastSecondPosition);
    GetPosition(m_last3MinPosition);
...
void Player::RemoveFromWorld()
{
    ...
    if (IsInWorld())
    {
        sChatHookMgr.ClearAllHooksRelatedToListener(GetObjectGuid());
...
{
	MasterPlayer* mPlayer = GetSession()->GetMasterPlayer();
	if (mPlayer == nullptr) return false;
	PlayerSocialPointer social = sSocialMgr.GetPlayerSocial(mPlayer->GetObjectGuid());
...
void Player::ShowCurrentXpRate()
{
    bool value = Forced_X1_XpRate();

    if (value)
    {
        ChatHandler(m_session).PSendSysMessage(80000, "1.0");
        m_session->SendNotification(80000, "1.0");
    }
    else
    {
        float xp = sWorld.getConfig(CONFIG_FLOAT_RATE_XP_KILL);

        if (HasDynamicFlag(PLAYER_DYNFLAG_XP5))
            xp *= 5;
        else if (HasDynamicFlag(PLAYER_DYNFLAG_XP3))
            xp *= 3;
        else if (HasDynamicFlag(PLAYER_DYNFLAG_XP2))
            xp *= 2;
        std::string xp_str = std::to_string(xp);
        ChatHandler(m_session).PSendSysMessage(80000, (xp_str.erase(xp_str.find_last_of('.')+2, xp_str.size())).c_str());
        m_session->SendNotification(80000, (xp_str.erase(xp_str.find_last_of('.')+2, xp_str.size())).c_str());
    }
}


void Player::GiveXP(uint32 xp, Unit* victim, bool isQuest)
{
    if (xp < 1)
        return;
    ...
    if (level >= sWorld.getConfig(CONFIG_UINT32_MAX_PLAYER_LEVEL) || HasDynamicFlag(PLAYER_DYNFLAG_XP_STOP))
        return;
...
void Player::GiveLevel(uint32 level)
{
    ...
    GetSession()->CompleteLoyaltyEvent(LoyaltyEvent::LevelUp);

    // Log achievement if a player has reached level 60, is not a GM and got at least 6 hours of playtime
    if (level == 60 && GetSession()->GetSecurity() == SEC_PLAYER && (GetTotalPlayedTime() > 21600))
    {
        CharacterDatabase.PExecute("INSERT INTO character_achievement (guid, name, race, class, totaltime, time, value, type) VALUES ('%u', '%s', '%u', '%u', '%u', '" UI64FMTD "', '%u', '%u')", GetGUIDLow(), GetName(), getRace(), getClass(), GetTotalPlayedTime(), uint64(time(NULL)), level, 1);

        if (sWorld.getConfig(CONFIG_BOOL_LEVEL_ANNOUNCE))
            sWorld.SendWorldText(LANG_LEVEL_ANNOUNCE, GetName(), level);
    }
...
uint32 Player::resetTalentsCost()
{
    ...
    if (!m_resetTalentsMultiplier) // initial respec
        cost = sWorld.getConfig(CONFIG_UINT32_RESPEC_BASE_COST) * GOLD;
    else
    {
        ...
        if (m_resetTalentsMultiplier > maxMulti)
            cost = (multiCost * maxMulti) * GOLD;
        else
            cost = (m_resetTalentsMultiplier * multiCost) * GOLD;
    }

    if (uint32 mod = GetSpecChangeDiscount())
        cost -= (cost * mod) / 100;
...
void Player::KillPlayer()
{
    ...
    StopMoving(true);   // for right show falling
...
uint32 Player::DurabilityRepair(uint16 pos, bool cost, float discountMod)
{
    ...
    if (cost)
    {
        ...
        if (LostDurability > 0)
        {
            ...
            if (uint32 mod = GetItemsRepairDiscount())
                costs -= (costs * mod) / 100;
...
void Player::RepopAtGraveyard()
{
	...
	if (ClosestGrave)
	{
		...
		// Hyjal Graveyard
		if (GetZoneId() == 616)
			TeleportTo(0, 4622.09f, -3703.05f, 11.000000f, 954.82f);
		else
			TeleportTo(ClosestGrave->map_id, ClosestGrave->x, ClosestGrave->y, ClosestGrave->z, GetOrientation(), 0, std::move(recover));
	}
}
...
bool Player::UpdateSkillPro(uint16 SkillId, int32 Chance, uint32 step)
{
    ...
    if (Roll <= Chance)
    {
        if (uint32 mod = GetSkillModifier(SkillId))
            step += (step * mod) / 100;
...
bool Player::CanGetXPFromExploration() const
{
    //m_lastAreaExplorationTime
    if ((getLevel() >= sWorld.getConfig(CONFIG_UINT32_MAX_PLAYER_LEVEL)) || HasItemCount(TOKEN_XP_STOP, 1))
        return false;

    if (uint32 timeThreshold = sWorld.getConfig(CONFIG_UINT32_AREA_EXPLORATION_TIME_THRESHOLD))
        if (WorldTimer::getMSTimeDiffToNow(m_lastAreaExplorationTime) <= timeThreshold * IN_MILLISECONDS)
            return false;
...
void Player::CheckAreaExploreAndOutdoor()
{
    ...
    if (!(currFields & val))
    {
        ...
        else if (p->AreaLevel > 0)
        {
			...
			// Item 55555 - twink stop XP token
            if (CanGetXPFromExploration())
            {
                int32 diff = int32(getLevel()) - p->AreaLevel;
                float XP_mod = Forced_X1_XpRate() ? 1.0f : sWorld.getConfig(CONFIG_FLOAT_RATE_XP_EXPLORE);
                uint32 XP = 0;
                if (diff < -5)
                    XP = uint32(sObjectMgr.GetBaseXP(getLevel() + 5) * XP_mod);
                else if (diff > 5)
                {
                    ...
                    XP = uint32(sObjectMgr.GetBaseXP(p->AreaLevel) * exploration_percent / 100 * XP_mod);
                }
                else
                    XP = uint32(sObjectMgr.GetBaseXP(p->AreaLevel) * XP_mod);

                GiveXP(XP, NULL);

                sXPLogMgr->FromExploration(GetObjectGuid(), XP);

                m_lastAreaExplorationTime = WorldTimer::getMSTime();
            }
...
void Player::UpdateZone(uint32 newZone, uint32 newArea)
{
    ...
    /// If horde use taxi to undead zone (eastern plaguelands/ashenvale) PVP flag shouldn't be triggered
    if (HasUnitMovementFlag(MOVEFLAG_ONTRANSPORT) && (zoneEntry->Id == 139 || zoneEntry->Id == 331) && pvpInfo.inHostileArea && GetTeam() == HORDE)
        pvpInfo.inHostileArea = false;
...
const static uint32 s_ItemsForSurpriseBox[]
{
    13583, 13584, 20371, 12529, 19450, 8494, 8485, 10398, 23007, 8489
};
...
void Player::SendLoot(ObjectGuid guid, LootType loot_type, Player* pVictim)
{
    ...
    switch (guid.GetHigh())
    {
        case HIGHGUID_GAMEOBJECT:
        {
            ...
            else if (go->getLootState() == GO_READY && !go->isSpawned())
            {
                SendLootRelease(guid);
                go->SetLootState(GO_JUST_DEACTIVATED);
                sLog.outError("%s attempted to loot from not spawned object GUID: %u, ID: %u - deactivating it",
                    GetName(), go->GetGUID(), go->GetEntry());
                return;
            }
            break;
        }
        case HIGHGUID_ITEM:
        {
            ...
            if (!item->HasGeneratedLoot())
            {
                ...
                if (item->GetProto()->ItemId == 100000)
                {
                    if (sWorld.getConfig(CONFIG_BOOL_SURPRISE_BOX))
                    {
                        uint8 l_ContainerSize = (sizeof(s_ItemsForSurpriseBox) / sizeof(uint32)) - 1;
                        uint32 l_ItemId = s_ItemsForSurpriseBox[urand(0, l_ContainerSize)];

                        if (ItemPrototype const* l_ItemTemplate = ObjectMgr::GetItemPrototype(l_ItemId))
                        {
                            LootStoreItem l_Item = LootStoreItem(l_ItemTemplate->ItemId, 100, 0, 0, 1, 1);
                            loot->AddItem(l_Item);
                            loot->gold = urand(sWorld.getConfig(CONFIG_UINT32_MIN_GOLD_FOR_SURPRISE_BOX), sWorld.getConfig(CONFIG_UINT32_MAX_GOLD_FOR_SURPRISE_BOX)) * GOLD;
                ...
                else
                {
                    if (Group* group = GetGroup())
                    {
                        if (group == recipient->GetGroup())
                        {
                            ...
                            {
                                case MASTER_LOOT:
                                    permission = group->GetMasterLooterGuid() == GetObjectGuid() ? MASTER_PERMISSION : RESTRICTED_PERMISSION;
                                    break;
                                case FREE_FOR_ALL:
    ...
    SendLoot(guid, loot_type, *loot, permission);
}

void Player::SendLoot(const ObjectGuid &guid, LootType loot_type, Loot& loot, PermissionTypes permission)
{
    SetLootGuid(guid);
...
Item* Player::StoreItem(ItemPosCountVec const& dest, Item* pItem, bool update, bool toBank)
{
    if (!pItem)
        return NULL;
    ...
    UpdateDonateTokenFlag(lastItem->GetEntry(), (lastItem->GetEntry() == TOKEN_XP_STOP && toBank) ? false : true);
...
void Player::RemoveItem(uint8 bag, uint8 slot, bool update)
{
    ...
    if (pItem)
    {
        ...
        RemoveEnchantmentDurations(pItem);
        RemoveItemDurations(pItem);
        UpdateDonateTokenFlag(pItem->GetEntry(), false);
...
void Player::DestroyItem(uint8 bag, uint8 slot, bool update)
{
    ...
    if (pItem)
    {
        ...
        if (pItem->IsBag())
        {
            // Prevent crash if a condition to check if bag is empty was bypassed somehow.
...
void Player::SendPreparedGossip(WorldObject *pSource)
{
    ...
    if (pSource->GetTypeId() == TYPEID_UNIT)
    {
                ...
                && !((Creature*)pSource)->HasFlag(UNIT_NPC_FLAGS, UNIT_NPC_FLAG_VENDOR)) // Fix mobs vendeurs avec des quetes.
        {
            ...
            if (Unit* unit = dynamic_cast<Unit*>(pSource))
            {
                if (unit->HasEmoteAfterQuestGossip())
                {
                    unit->HandleEmoteCommand(unit->GetEmoteAfterQuestGossip());
                    //clear
                    unit->HandleEmoteAfterQuestGossip(0);
                }
            }
            return;
        }
...
bool Player::CanTakeQuest(int32 questId) const
{
...
void Player::CompleteQuest(uint32 quest_id)
{
    if (quest_id)
    {
        ...
        GetSession()->CompleteLoyaltyEvent(LoyaltyEvent::CompleteQuest);
    }
}
...
void Player::RewardQuest(Quest const *pQuest, uint32 reward, WorldObject* questGiver, bool announce)
{
    ...
    // Not give XP in case already completed once repeatable quest
     float XP_mod = Forced_X1_XpRate() ? 1.0f : sWorld.getConfig(CONFIG_FLOAT_RATE_XP_QUEST);
     uint32 XP = q_status.m_rewarded ? 0 : uint32(pQuest->XPValue(this) * XP_mod);

    if (getLevel() < sWorld.getConfig(CONFIG_UINT32_MAX_PLAYER_LEVEL))
    {
        GiveXP(XP, NULL, true);
        sXPLogMgr->FromQuest(GetObjectGuid(), XP);
    }
    else
...
bool Player::SatisfyQuestLevel(Quest const* qInfo, bool msg) const
{
    ...
    else if (getLevel() > qInfo->GetMaxLevel())
        return false;
...
bool Player::CanGiveQuestSourceItemIfNeed(Quest const *pQuest, ItemPosCountVec* dest) const
{
    if (uint32 srcitem = pQuest->GetSrcItemId())
    {
        if (ItemPrototype const *pProto = ObjectMgr::GetItemPrototype(srcitem))
        {
            ...
            count = has_count > count ? 0 : count - has_count;  // real need amount
...
void Player::LogModifyMoney(int32 d, const char* type, ObjectGuid fromGuid, uint32 data)
{
    ...
    {
        sLog.out(LOG_MONEY_TRADES, "[%s] %s had %uc and gets %ic (data: %u|%s)", type, GetShortDescription().c_str(), GetMoney(), d, data, fromGuid.GetString().c_str());
        if (d > 0)
...
bool Player::LoadFromDB(ObjectGuid guid, SqlQueryHolder *holder)
{
    ...
    // 57                58           59
    //"world_phase_mask, customFlags, regexFilterCount FROM characters WHERE guid = '%u'", GUID_LOPART(m_guid));
    ...
    m_RegexFilterCount = fields[59].GetUInt32();
    ...
    _LoadBrokenItems(holder->GetResult(PLAYER_LOGIN_QUERY_BROKEN_ITEMS));
    _LoadInventory(holder->GetResult(PLAYER_LOGIN_QUERY_LOADINVENTORY), time_diff);
    _LoadItemLoot(holder->GetResult(PLAYER_LOGIN_QUERY_LOADITEMLOOT));

    // Fill donate items flags
    FillDonateTokensFlags();
...
void Player::_LoadBrokenItems(QueryResult * result)
{
    ///< SELECT item_template, item FROM character_inventory as ci WHERE ci.guid = '%u' AND NOT EXISTS (SELECT * FROM item_instance AS ic WHERE ic.`guid` = ci.`item`)

    if (result)
    {
        std::list<std::pair<uint32, uint32>> itemsToRestore;

        do
        {
            Field* fields = result->Fetch();
            uint32 itemId = fields[0].GetUInt32();
            uint32 itemGuid = fields[1].GetUInt32();

            if (ItemPrototype const* proto = ObjectMgr::GetItemPrototype(itemId))
                itemsToRestore.push_back({ itemId, itemGuid });

        } while (result->NextRow());

        while (!itemsToRestore.empty())
        {
            ...
            bool send = false;
            for (int i = 0; !itemsToRestore.empty() && i < MAX_MAIL_ITEMS; ++i)
            {
                std::pair<uint32, uint32> kvp = itemsToRestore.front();
                if (Item* item = Item::CreateItem(kvp.first, 1, this))
                {
                    item->SetItemRandomProperties(item->GenerateItemRandomPropertyId(kvp.first));
                    item->SaveToDB();
                    draft.AddItem(item);
                    send = true;

                    CharacterDatabase.PExecute("DELETE FROM character_inventory WHERE item = '%u'", kvp.second);
                }

                itemsToRestore.pop_front();
            }
...
void Player::_LoadInventory(QueryResult *result, uint32 timediff)
{
    ...
    if (result)
    {
        ...
        do
        {
            ...
            {
                CharacterDatabase.PExecute("DELETE FROM character_inventory WHERE item = '%u'", item_lowguid);
                GetSession()->GetAnticheat()->RecordCheat(CHEAT_ACTION_INFO_LOG,
                    "ItemCheck", "Duplicate item (via AH) %u GUID: %u, count: %u, bag: %u, slot: %u",
                    item_id, item_lowguid, item->GetCount(), bag_guid, static_cast<uint32>(slot));
                continue;
            }
    ...
    if (result)
    {
        do
        {
            ...
            if (pQuest)
            {
                ...
                if (pQuest->HasSpecialFlag(QUEST_SPECIAL_FLAG_TIMED) && !GetQuestRewardStatus(quest_id) && questStatusData.m_status != QUEST_STATUS_NONE && questStatusData.m_status != QUEST_STATUS_COMPLETE)
                {
                    AddTimedQuest(quest_id);
...
void Player::SaveToDB(bool online, bool force)
{
    ...
    SqlStatement uberInsert = CharacterDatabase.CreateStatement(insChar, "REPLACE INTO characters (guid,account,name,race,class,gender,level,xp,money,playerBytes,playerBytes2,playerFlags,"
                              ...
                              "area, world_phase_mask, customFlags, regexFilterCount) "
                              "VALUES ( ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,"
                              "?, ?, ?, ?, ?, "
                              "?, ?, ?, "
                              "?, ?, ?, ?, ?, ?, ?, "
    ...
    uberInsert.addUInt32(m_RegexFilterCount);
    uberInsert.Execute();
...
bool Player::SaveAura(SpellAuraHolder* holder, AuraSaveStruct& saveStruct)
{
    ...
    // Don't save VIP's visuals
    if (sWorld.GetVIPVisuals().find(holder->GetId()) != sWorld.GetVIPVisuals().end())
        return false;
    ...
    for (size_t i = 0; i < m_itemUpdateQueue.size(); ++i)
    {
        ...
        if (item->GetState() != ITEM_REMOVED)
        {
            ...
            if (!test)
            {
                ...
                GetSession()->GetAnticheat()->RecordCheat(CHEAT_ACTION_INFO_LOG,
                    "ItemCheck",
                    "_SaveInventory: Item update queue contains item entry %u guid 0x%llx count %u state %u at bag slot %u slot %u but no item found at that slot, deleting item",
                    item->GetEntry(), item->GetGUID(), item->GetCount(), item->GetState(), item->GetBagSlot(), item->GetSlot());
            ...
            else if (test != item)
            {
                GetSession()->GetAnticheat()->RecordCheat(CHEAT_ACTION_INFO_LOG,
                    "ItemCheck",
                    "_SaveInventory: Item update queue contains item entry %u guid %llx count %u state %u at bag slot %u slot %u but found item entry %u guid %llx count %u state %u",
                    item->GetEntry(), item->GetGUID(), item->GetCount(), item->GetState(), item->GetBagSlot(), item->GetSlot(),
                    test->GetEntry(), test->GetGUID(), test->GetCount(), test->GetState());
...
void Player::_SaveArmoryStats()
{
    auto dmg = [this](uint32 idx) -> std::string
    {
        std::ostringstream damage;
        damage << GetFloatValue(idx) << " - " << GetFloatValue(idx + 1);
        return damage.str();
    };

    auto weaponSpeed = [this](WeaponAttackType type) -> float
    {
        return GetFloatValue(UNIT_FIELD_BASEATTACKTIME + type) / 1000.0f;
    };

    auto modifier = [this](uint32 idx) -> float
    {
        float value = GetFloatValue(idx);
        return (1.0f / value - 1.0f) * 100.0f;
    };

    static SqlStatementID delStats;
    static SqlStatementID insertStats;

    SqlStatement stmt = CharacterDatabase.CreateStatement(delStats, "DELETE FROM character_armory_stats WHERE guid = ?");
    stmt.PExecute(GetGUIDLow());

    stmt = CharacterDatabase.CreateStatement(insertStats, "INSERT INTO character_armory_stats "
        "(guid, maxhealth, maxpower1, maxpower2, maxpower3, maxpower4, maxpower5, "
        ...
        "blockPct, dodgePct, parryPct, meleeCritPct, rangedCritPct, attackPower, rangedAttackPower, "
        "meleeDamage, rangedDamage, meleeWeaponSpeed, rangedWeaponSpeed, castSpeed, "
        "meleeHit, rangedHit, spellHit) "
        "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
    ...
    for (int i = 0; i < MAX_SPELL_SCHOOL; ++i)
        stmt.addUInt32(GetResistance(SpellSchools(i)));
    for (int i = 0; i < MAX_SPELL_SCHOOL; ++i)
        stmt.addUInt32(GetUInt32Value(PLAYER_FIELD_MOD_DAMAGE_DONE_POS + i));
    stmt.addFloat(GetFloatValue(PLAYER_BLOCK_PERCENTAGE));
    stmt.addFloat(GetFloatValue(PLAYER_DODGE_PERCENTAGE));
    stmt.addFloat(GetFloatValue(PLAYER_PARRY_PERCENTAGE));
    stmt.addFloat(GetFloatValue(PLAYER_CRIT_PERCENTAGE));
    stmt.addFloat(GetFloatValue(PLAYER_RANGED_CRIT_PERCENTAGE));
    stmt.addFloat(GetTotalAttackPowerValue(WeaponAttackType::BASE_ATTACK));
    stmt.addFloat(GetTotalAttackPowerValue(WeaponAttackType::RANGED_ATTACK));
    stmt.addString(dmg(UNIT_FIELD_MINDAMAGE));
    stmt.addString(dmg(UNIT_FIELD_MINRANGEDDAMAGE));
    stmt.addFloat(weaponSpeed(WeaponAttackType::BASE_ATTACK));
    stmt.addFloat(weaponSpeed(WeaponAttackType::RANGED_ATTACK));
    stmt.addFloat(modifier(UNIT_MOD_CAST_SPEED));
    stmt.addFloat(m_modMeleeHitChance);
    stmt.addFloat(m_modRangedHitChance);
    stmt.addFloat(m_modSpellHitChance);
    stmt.Execute();
}
...
void Player::SummonCritter(uint32 pet_entry)
{
    if (!pet_entry)
        return;
    ...
    if (!cInfo)
    {
        sLog.outErrorDb("Player::SummonCritter: creature entry %u not found.", pet_entry);
        return;
    }

    Pet* old_critter = GetMiniPet();
    ...
    {
        sLog.outError("Player::SummonCritter: no such creature entry %u", pet_entry);
        delete critter;
        return;
    }
    ...
    critter->SetOwnerGuid(GetObjectGuid());
    critter->SetCreatorGuid(GetObjectGuid());
    critter->setFaction(getFaction());
...
void Player::Mount(uint32 mount, uint32 spellId /*= 0*/, bool IsTaxi /*= false*/)
{
    if (!mount)
    {
    ...
    Unit::Mount(mount, spellId, IsTaxi);
...
void Player::SendInitialPacketsAfterAddToMap(bool login)
{
    ...
    if (HasAuraType(SPELL_AURA_WATER_WALK))
        SendPacketSetWaterWalking(true);
...
void Player::RewardSinglePlayerAtKill(Unit* pVictim)
{
    ...
    auto canDivide = [&]() -> bool
    {
        if (!pVictim || pVictim->GetTypeId() != TYPEID_UNIT)
            return false;
        if (pVictim->GetAllAttackers().size() <= 1)
            return false;

        Map const* map = GetMap();
        if (map->IsNonRaidDungeon() && !sWorld.getConfig(CONFIG_BOOL_XP_DIVIDE_IN_DUNGEON))
            return false;
        else if (map->IsRaid() && !sWorld.getConfig(CONFIG_BOOL_XP_DIVIDE_IN_RAID))
            return false;
        else if (map->IsContinent() && !sWorld.getConfig(CONFIG_BOOL_XP_DIVIDE_IN_WORLD))
            return false;

        return true;
    };

    int32 levelThreshold = sWorld.getConfig(CONFIG_INT32_XP_DIVIDE_THRESHOLD);
    if (xp > 0 && levelThreshold != 0 && canDivide())
    {
        uint8 min = 255, max = 0;
        for (auto const& pair : pVictim->GetAllAttackers())
        {
            uint8 const level = pair.second;
            if (level < min)
                min = level;
            if (level > max)
                max = level;
        ...
        ///< Divide by amount of attackers
        if ((max - min) >= levelThreshold)
            xp /= pVictim->GetAllAttackers().size();
    }
    ...
    if (!PvP)
    {
        ...
        if (pVictim->GetTypeId() == TYPEID_UNIT)
            sXPLogMgr->FromNpcKill(GetObjectGuid(), pVictim->GetEntry(), xp);
...
void Player::UpdateUnderwaterState()
{
    ...
    {
        if (SpellEntry const *spellInfo = sSpellMgr.GetSpellEntry(liq->SpellId))
        {
            if (res & (LIQUID_MAP_UNDER_WATER | LIQUID_MAP_IN_WATER))
            {
...
bool Player::TeleportToHomebind(uint32 options, bool cooldown)
{
    if (cooldown)
        if (SpellEntry const *spellInfo = sSpellMgr.GetSpellEntry(8690))
...
#define CHANGERACE_LOG(format, ...) sLog.out(LOG_CHAR, "[RaceChanger/Log] " format, ##__VA_ARGS__)
#define CHANGERACE_ERR(format, ...) sLog.out(LOG_CHAR, "[RaceChanger/Err] " format, ##__VA_ARGS__)

bool ShouldLearnSkill(uint32 Skill, Team team)
{
    if (team == ALLIANCE)
    {
        if (IsHordeLanguageSkill(Skill))
            return false;
    }
    if (team == HORDE)
    {
        if (IsAllianceLanguageSkill(Skill))
            return false;
    }
    return true;
}

bool Player::ChangeRace(uint8 newRace, uint8 newGender /*= 255*/, uint32 playerbyte1 /*= 0*/, uint32 playerbyte2 /*= 0*/)
{
    ...
    bool bChangeTeam = (TeamForRace(oldRace) != newTeam);
    ...
    if (GuildId != 0)
    {
        if (Guild* guild = sGuildMgr.GetGuildById(GetGuildId()))
        {
            ...
            {
                // Check if player a guild leader, but there is one - delete guild with member link
                if (guild->GetMemberSize() == 1)
                {
                    CharacterDatabase.BeginTransaction();

                    // not cascade delete ?
                    CharacterDatabase.PExecute("DELETE FROM guild WHERE guildid = %u", GuildId);
                    CharacterDatabase.PExecute("DELETE FROM guild_eventlog WHERE guildid = %u", GuildId);
                ...
                else // in other case if we won`t type in web about /q from guild - try select any player in guild and pass the leader to him
                    guild->DelMember(GetObjectGuid(), false);
            }
        }
    }
    ...
    //Leave current group
    RemoveFromGroup();
    //Dump current skill values
    //Restore after we learn new spells

    //Key - SkillId, Value - Skill value
    std::unordered_map<uint32, std::pair<uint16, uint16>> SkillValues;
    for (const auto& SkillElemPair : mSkillStatus)
    {
        uint32 SkillId = SkillElemPair.first;
        SkillValues[SkillId] = std::make_pair(GetSkillValue(SkillId), GetPureMaxSkillValue(SkillId));
    }

    std::unordered_map<uint16, int32> baseFactions;
    if (TeamForRace(oldRace) == TeamForRace(newRace))
    {
        for (auto const& pair : GetReputationMgr().GetStateList())
            if (FactionEntry const *factionEntry = sFactionStore.LookupEntry(pair.second.ID))
    ...
    //Drop friend and ignore list
    {
        PlayerSocialPointer playerSocial = sSocialMgr.GetPlayerSocial(GetObjectGuid());
        playerSocial->Clear();
    }

    //## Start changing all character data
    ...
    //Apply first portion of player data
    if (playerbyte1 != 0)
    {
    ...
    for (const auto& SkillElemPair : mSkillStatus)
    {
        ...
        if (!IsLanguageSkill(SkillId))
        {
            ...
            if (SkillValueIter != SkillValues.end())
                SetSkill(SkillId, SkillValueIter->second.first, SkillValueIter->second.second);
        }
    }
    ...
    {
        FactionEntry const *factionEntry = sFactionStore.LookupEntry(pair.first);
        if (!factionEntry)
            continue;
...
bool Player::ConvertSpell(uint32 oldSpellId, uint32 newSpellId)
{
    ...
    {
        const char* OldSpellName = nullptr;
        const char* NewSpellName = nullptr;
        if (const SpellEntry* oldSpellEntry = sSpellMgr.GetSpellEntry(oldSpellId))
        {
            OldSpellName = oldSpellEntry->SpellName[0];
        }
        if (const SpellEntry* newSpellEntry = sSpellMgr.GetSpellEntry(newSpellId))
        {
            NewSpellName = newSpellEntry->SpellName[0];
        }
        CHANGERACE_LOG("Changing spell %s (%u) -> %s (%u)", OldSpellName ? OldSpellName : "NULL", oldSpellId, NewSpellName ? NewSpellName : "NULL", newSpellId);
...
bool Player::ChangeSpellsForRace(uint8 oldRace, uint8 newRace)
{
    ...
    for (PlayerCreateInfoSpells::const_iterator itr = info->spell.begin(); itr != info->spell.end(); ++itr)
    {
        ...
        //FIX: Default spells for classes (knowed from 1 lvl) should'nt removed from character.
        bool bLearnedHighRank = false;

        auto SpellLearnedFunc = [this, &bLearnedHighRank, tspell](const uint32 spellId)
        {
            //already found a spell
            if (bLearnedHighRank) return;

            if (spellId != tspell)
            {
                if (HasActiveSpell(spellId))
                {
                    bLearnedHighRank = true;
                    return;
                }
            }
        };

        sSpellMgr.doForHighRanks(tspell, SpellLearnedFunc);
...
bool Player::ChangeItemsForRace(uint8 oldRace, uint8 newRace)
{
    ...
    Team newTeam = TeamForRace(newRace);
    // 1- Convert mounts
    bool bConvertMountResult = ForEveryPlayerItem([oldRace, newRace](Item* item) -> bool
    {
        if (item->GetProto()->Class == ITEM_CLASS_JUNK && item->GetProto()->RequiredSkill == 762)
        {
            Races currMountRace;
            uint8 currRaceNum = 0;
            if (!sObjectMgr.GetMountDataByEntry(item->GetEntry(), currMountRace, currRaceNum))
            {
                CHANGERACE_ERR("Mount with ItemId %s (%u) is not listed in `player_factionchange_mounts`.", item->GetProto()->Name1, item->GetEntry());
                return true;
            }
            ...
            if (!newMountId)
            {
                CHANGERACE_ERR("Mount for raceId %u don't have declaration in `player_factionchange_mounts` for itemId %s (%u).", mountNewRace, item->GetProto()->Name1, item->GetEntry());
                return true;
            }
            ...
            {
                CHANGERACE_ERR("Failed to convert mount for ItemId %s (%u).", item->GetProto()->Name1, item->GetEntry());
                return false;
            }
            else
            {
                CHANGERACE_LOG("Changing mount from %s (%u) to %s (%u).", item->GetProto()->Name1, item->GetEntry(), pNewMountProto->Name1, newMountId);
            }
        }
        return true;
    });

    if (!bConvertMountResult) return false;

    // 2- Convert items (without equipment)
    if (oldTeam != newTeam)
    {
        for (std::map<uint32, uint32>::const_iterator it = sObjectMgr.factionchange_items.begin(); it != sObjectMgr.factionchange_items.end(); ++it)
        {
            ...
            bool bConvertItemsResult = ForEveryPlayerItem([removeItemId, pNewItemProto](Item* item) -> bool
            {
                if (item->GetEntry() == removeItemId)
                {
                    CHANGERACE_LOG("Changing item %s (%u) -> %s (%u).", item->GetProto()->Name1, removeItemId, pNewItemProto->Name1, pNewItemProto->ItemId);
            ...
            if (!bConvertItemsResult) return false;
        }
    }

    // 3- Convert items, only equipment
    bool bConvertEquipmentResult = ForEveryPlayerItem([newRace](Item* item) -> bool
    {
        ...
        {
            if ((it2->first == item->GetEntry()) || (item->GetEntry() == it2->second))
            {
                previouslyHandled = true;
                CHANGERACE_LOG("Item %s (%u) skipped for second pass.", item->GetProto()->Name1, item->GetEntry());
                break;
            }
        }
        ...
        if (!canEquip)
        {
            CHANGERACE_ERR("Item %s (%u) can't equiped after convertation! Please check player_factionchange_items for errors.", pProto->Name1, pProto->ItemId);
    ...
    if (!bConvertEquipmentResult) return false;
...
Item* Player::AddItem(uint32 itemId, uint32 count, uint32* noSpaceForCount)
{
...
void Player::RewardHonor(Unit* uVictim, uint32 groupSize)
{
    ...
    if (uVictim->GetTypeId() == TYPEID_UNIT)
    {
        ...
        {
            // Dishonorable kills only from 1.5
        ...
        {
            m_honorMgr.Add(100.0, HONORABLE, cVictim);
...
void Player::OnReceivedItem(Item* item, uint32 count, uint32 itemid, bool save)
{
    if (item == nullptr)
    {
        sLog.outError("ERROR: Player::OnReceivedItem invoked with item == nullptr. Player name: %s", GetName());
        return;
    }
    // Get quality of the item
    uint32 quality = item->GetProto()->Quality;

    // Save quickly if quality is high (Config setting)
    if (quality >= sWorld.getConfig(CONFIG_UINT32_ITEM_INSTANTSAVE_QUALITY) && save)
        SetSaveTimer(1);
    ...
    if (quality >= sWorld.getConfig(CONFIG_UINT32_ITEM_RARELOOT_QUALITY))
        sLog.out(LOG_RARELOOTS, "%s loots %ux%u with quality %u", GetShortDescription().c_str(), count, itemid, quality);
}
...
void Player::SendSurpriseBox()
{
    if (!sWorld.getConfig(CONFIG_BOOL_SURPRISE_BOX) || getLevel() < sWorld.getConfig(CONFIG_UINT32_MIN_LVL_FOR_SURPRISE_BOX))
        return;

    QueryResult* l_Result = CharacterDatabase.PQuery("SELECT `boxSended` FROM `received_surprise_box` WHERE `charGuid` = '%u'", GetGUIDLow());

    if (Item* l_SurpriseBox = Item::CreateItem(100000, 1, nullptr))
    {
        if (l_Result)
        {
            Field* l_Filed = l_Result->Fetch();

            if (l_Filed->GetUInt8() != 0)
                return;
        }

        MailDraft l_Draft;
        std::string l_Subject = "Elysium!";
        std::string l_Body = "Take a peek at your in game mail — I hear there’s a special someone waiting to meet you! \nSincerely, \nElysium Team";

        l_SurpriseBox->SaveToDB();
        l_Draft.AddItem(l_SurpriseBox);
        l_Draft.SetSubjectAndBody(l_Subject, l_Body);

        MailSender l_Sender(MAIL_NORMAL, uint32(0), MAIL_STATIONERY_GM);

        l_Draft.SendMailTo(MailReceiver(this, GetGUID()), l_Sender);

        uint32 l_AccountId = sObjectMgr.GetPlayerAccountIdByGUID(GetGUID());

        if (!l_Result)
            CharacterDatabase.PExecute("INSERT INTO `received_surprise_box` (`account`, `charGuid`, `boxSended`) VALUES ('%u', '%u', '1')", l_AccountId, GetGUIDLow());
        else
            CharacterDatabase.PExecute("UPDATE `received_surprise_box` SET `boxSended`='1' WHERE `charGuid`='%u'", GetGUIDLow());
    }
}
...
void Player::SetMover(Unit* target)
{
    ...
    m_mover_guid = m_mover->GetObjectGuid();
}

void Player::ValidateMover()
{
    //Giperion: Check if mover is valid
    if (!GetMap()->IsValid(m_mover_guid))
    {
        sLog.outError("[CRASH]: Mover is not valid or located on different map");
        SetMover(this);
    }
}

void Player::UpdateDonateTokenFlag(uint32 itemId, bool added)
{
    PlayerDynamicFlags flag = PLAYER_DYNFLAG_NONE;
    switch (itemId)
    {
        case TOKEN_XP_STOP:
            flag = PLAYER_DYNFLAG_XP_STOP;
            break;
        case TOKEN_XP_X5:
            flag = PLAYER_DYNFLAG_XP5;
            break;
        case TOKEN_XP_X3:
            flag = PLAYER_DYNFLAG_XP3;
            break;
        case TOKEN_XP_X2_1DAY:
        case TOKEN_XP_X2_7DAYS:
        case TOKEN_XP_X2_UNLIM:
            flag = PLAYER_DYNFLAG_XP2;
            break;
        default: return;
    }
...
void Player::FillDonateTokensFlags()
{
    if (HasItemCount(TOKEN_XP_STOP, 1))
        m_dynamicFlags |= PLAYER_DYNFLAG_XP_STOP;
    else if (HasItemCount(TOKEN_XP_X5, 1, true))
        m_dynamicFlags |= PLAYER_DYNFLAG_XP5;
    else if (HasItemCount(TOKEN_XP_X3, 1, true))
        m_dynamicFlags |= PLAYER_DYNFLAG_XP3;
    else if (HasItemCount(TOKEN_XP_X2_1DAY, 1, true) || HasItemCount(TOKEN_XP_X2_7DAYS, 1, true) || HasItemCount(TOKEN_XP_X2_UNLIM, 1, true))
        m_dynamicFlags |= PLAYER_DYNFLAG_XP2;
}
...
void Player::SendRaidWarning(const char* text)
{
    ...
    if (WorldSession* Sess = GetSession())
    {
        ChatHandler::FillMessageData(&data, GetSession(), CHAT_MSG_RAID_WARNING, LANG_UNIVERSAL, text);
        Sess->SendPacket(&data);
    }
}

bool Player::RemoveItems(uint32 itemId, uint32 count)
{
...
void Player::SendMovementInfo(OpcodesList opcode /*= MSG_MOVE_HEARTBEAT*/)
{
    if (WorldSession* Sess = GetSession())
    {
        if (IsInWorld())
        {
            //m_movementInfo
            //this->movem
            WorldPacket data(opcode, 120);
            data << GetObjectGuid().WriteAsPacked();             // write guid
            m_movementInfo.UpdateTime(WorldTimer::getMSTime());
            m_movementInfo.Write(data);                               // write data
            SendMovementMessageToSet(std::move(data), true);
        }
    }
}
...
float Player::GetVelocity(uint32 deltaTime, WorldLocation PrevPos) const
{
    ...
    float dx = CurrentPos.coord_x - PrevPos.coord_x;
    float dy = CurrentPos.coord_y - PrevPos.coord_y;
...
void Player::UnlockAllTaxiPathes()
{
    if (GetTeam() == ALLIANCE)
        m_taxi.LoadTaxiMask("3456411898 2148078928 49991 0 0 0 0 0 ");
    else
        m_taxi.LoadTaxiMask("561714688 282102432 52408 0 0 0 0 0 ");
}

bool Player::HasPremiumAccount() const
{
    return GetSession()->GetAccountFlags() & ACCOUNT_FLAG_PREMIUM;
}

bool Player::CanInstantFly() const
{
    if (!HasPremiumAccount())
        return false;

    return sWorld.getConfig(CONFIG_BOOL_PREMIUM_INSTANT_FLY_ENABLED);
}

bool Player::CanInstantMail() const
{
    if (!HasPremiumAccount())
        return false;

    return sWorld.getConfig(CONFIG_BOOL_PREMIUM_INSTANT_INGAME_MAIL_ENABLED);
}

uint32 Player::GetSpecChangeDiscount() const
{
    if (!HasPremiumAccount())
        return 0;

    return sWorld.getConfig(CONFIG_UINT32_PREMIUM_SPEC_CHANGE_DISCOUNT);
}

uint32 Player::GetItemsRepairDiscount() const
{
    if (!HasPremiumAccount())
        return 0;

    return sWorld.getConfig(CONFIG_UINT32_PREMIUM_ITEMS_REPAIR_DISCOUNT);
}

uint32 Player::GetHearthstoneCDReduction() const
{
    if (!HasPremiumAccount())
        return 0;

    return sWorld.getConfig(CONFIG_UINT32_PREMIUM_HEARTHSTONE_CD_REDUCTION);
}

uint32 Player::GetSkillModifier(uint16 skillID) const
{
    if (!HasPremiumAccount())
        return 0;
    ...
    {
    case SKILL_HERBALISM:
        return sWorld.getConfig(CONFIG_UINT32_PREMIUM_HERBALISM_SKILL_MOD);
    case SKILL_MINING:
        return sWorld.getConfig(CONFIG_UINT32_PREMIUM_MINING_SKILL_MOD);
    case SKILL_SKINNING:
        return sWorld.getConfig(CONFIG_UINT32_PREMIUM_SKINNING_SKILL_MOD);
    case SKILL_ALCHEMY:
        return sWorld.getConfig(CONFIG_UINT32_PREMIUM_ALCHEMY_SKILL_MOD);
    case SKILL_BLACKSMITHING:
        return sWorld.getConfig(CONFIG_UINT32_PREMIUM_BLACKSMITHING_SKILL_MOD);
    case SKILL_ENCHANTING:
        return sWorld.getConfig(CONFIG_UINT32_PREMIUM_ENCHANTING_SKILL_MOD);
    case SKILL_ENGINEERING:
        return sWorld.getConfig(CONFIG_UINT32_PREMIUM_ENGINEERING_SKILL_MOD);
    case SKILL_LEATHERWORKING:
        return sWorld.getConfig(CONFIG_UINT32_PREMIUM_LEATHERWORKING_SKILL_MOD);
    case SKILL_TAILORING:
        return sWorld.getConfig(CONFIG_UINT32_PREMIUM_TAILORING_SKILL_MOD);
    case SKILL_COOKING:
        return sWorld.getConfig(CONFIG_UINT32_PREMIUM_COOKING_SKILL_MOD);
    case SKILL_FIRST_AID:
        return sWorld.getConfig(CONFIG_UINT32_PREMIUM_FIRST_AID_SKILL_MOD);
    case SKILL_FISHING:
        return sWorld.getConfig(CONFIG_UINT32_PREMIUM_FISHING_SKILL_MOD);
    }
