...
void TemporarySummon::Summon(TempSummonType type, uint32 lifetime)
{
    ...
    //Temp log
    //sLog.outInfo("Summoned %s with guid %llu and type %s for %d seconds", GetName(), GetObjectGuid().GetRawValue(), TempSummonType_ToString(m_type), m_timer / IN_MILLISECONDS);
}
...
void TemporarySummon::UnSummon(uint32 delayDespawnTime /*= 0*/)
{
    if (delayDespawnTime)
    {
        ...
        //sLog.outInfo("Temp summoned %s with guid %llu change type %s and despawn timer now is %d seconds", GetName(), GetObjectGuid().GetRawValue(), TempSummonType_ToString(m_type), m_timer / IN_MILLISECONDS);
    }
    else
    {
        //sLog.outInfo("Temp summoned %s with guid %llu type %s now DESPAWNED", GetName(), GetObjectGuid().GetRawValue(), TempSummonType_ToString(m_type), m_timer / IN_MILLISECONDS);
        CombatStop();
