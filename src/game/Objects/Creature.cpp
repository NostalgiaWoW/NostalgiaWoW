...
Creature::Creature(CreatureSubtype subtype) :
    Unit(), i_AI(nullptr),
    loot(this), lootForPickPocketed(false), lootForBody(false), lootForSkin(false), skinningForOthersTimer(5000),
    ...
    m_AlreadySearchedAssistance(false), m_evadeTimer(0), m_evadeMode(EVADE_MODE_NONE),
    ...
    m_CombatDistance(0.0f), _playerDamageTaken(0), _nonPlayerDamageTaken(0), m_creatureInfo(nullptr),
    m_AI_InitializeOnRespawn(false), m_callForHelpDist(5.0f), m_combatPulseTimer(0), m_combatWithZoneState(false), _lastDamagedTime(0)
{
    m_regenTimer = 200;
...
void Creature::AddToWorld()
{
    ...
    if (GetEntry() == 1000553)
        sWorld.SendWorldText(LANG_EVENTMESSAGE, "Ragtheradon the Invader: I have arrived. This world will be consumed for the Burning Legion! Prepare yourselves mortals!");
}
...
bool Creature::InitEntry(uint32 Entry, Team team, CreatureData const* data /*=NULL*/, GameEventCreatureData const* eventData /*=NULL*/)
{
    ...
    if (cinfo->flags_extra & CREATURE_FLAG_EXTRA_DISABLE_PATHFINDING)
    {
        addUnitState(UNIT_STAT_IGNORE_PATHFINDING);
    }

    if (cinfo->flags_extra & CREATURE_FLAG_EXTRA_DISABLE_MOVEMENT)
    {
...
void Creature::Update(uint32 update_diff, uint32 diff)
{
    ...
    switch (m_deathState)
    {
        ...
        case ALIVE:
        {
            ...
            auto const oldEvadeMode = GetEvadeMode();
            UpdateEvadeMode(update_diff);
            auto const newEvadeMode = GetEvadeMode();
            ...
            if (auto const ai = AI())
            {
                ...
                try
                {
                    // only inform the AI about this change if we are resetting
                    if (oldEvadeMode != newEvadeMode && newEvadeMode == EVADE_MODE_RESETTING)
                        ai->EnterEvadeMode();
...
void Creature::RegenerateAll(uint32 update_diff)
{
    ...
    if (!isInCombat() || IsPolymorphed() || GetEvadeMode() >= EVADE_MODE_REGENERATING)
        RegenerateHealth();
...
float Creature::GetAttackDistance(Unit const* pl) const
{
    ...
    float RetDistance = (float)GetCreatureInfo()->aggroRadius;
    if (RetDistance < 1.0f)
        RetDistance = 20.0f;
...
void Creature::RemoveCombatWithZone()
{
    ...
    if (!pMap->IsDungeon())
    {
        sLog.outError("Creature entry %u call RemoveCombatWithZone for map (id: %u) that isn't an instance.", GetEntry(), pMap->GetId());
        return;
    }
...
// returns true when there is a target which is unreachable.  note that this is not the
// opposite of having a reachable target.  it is also possible that there is no target.
bool Creature::UnreachableTarget() const
{
    auto const &motion = i_motionMaster;

    // if we are not moving, we arent trying to reach a target
    if (motion.empty())
        return false;

    // if we have no target, we obviously do not have an unreachable target
    if (!getVictim())
        return false;

    // if we are not chasing our victim, we are not trying to reach them
    if (motion.GetCurrentMovementGeneratorType() != CHASE_MOTION_TYPE)
        return false;

    // if we can reach them with melee, they are reachable
    if (CanReachWithMeleeAttack(getVictim()))
        return false;
    ...
    // if there is a path to them, they are reachable
    if (motion->IsReachable())
        return false;

    // if the target is a player who is in the middle of a knockback, temporarily consider them reachable
    if (auto const victimPlayer = getVictim()->ToPlayer())
        if (victimPlayer->GetSession()->GetAnticheat()->IsInKnockBack())
            return false;

    // TODO: it is probably the case that if a caster cannot reach melee range, but can reach spell-cast
    // range and has line of sight, then the target should be considered reachable.  however this should
    // not happen normally, and at this level we do not currently have information about what range is
    // necessary.  nor do we have a mechanism to determine what point the creature should move to in order
    // to be able to cast its spell.

    // if we reach here, we must have an unreachable target
    return true;
}

void Creature::UpdateEvadeMode(uint32 diff)
{
    auto const movingHome = !i_motionMaster.empty() && i_motionMaster.GetCurrentMovementGeneratorType() == HOME_MOTION_TYPE;

    // we can never move from resetting to anything else unless we are no longer moving home
    if (m_evadeMode == EVADE_MODE_RESETTING)
    {
        if (!movingHome)
            m_evadeMode = EVADE_MODE_NONE;

        return;
    }

    auto const inDungeon = GetMap()->IsDungeon();
    auto const oldEvadeMode = GetEvadeMode();
    auto const oldEvadeTimer = m_evadeTimer;

    if (UnreachableTarget())
    {
        m_evadeTimer += diff;

        if (GetMapId() == 30 && CanHaveThreatList() && m_evadeTimer > 1000) // Alterac Valley exploit fix
            getThreatManager().modifyThreatPercent(getVictim(), -101);
    }
    // if we are moving to home, always set mode to resetting
    else if (movingHome)
    {
        m_evadeMode = EVADE_MODE_RESETTING;
        return;
    }
    else
        m_evadeTimer = 0;

    // if less than EvadeWait milliseconds have gone by, not in any evade mode
    if (m_evadeTimer < EvadeWait)
        m_evadeMode = EVADE_MODE_NONE;
    // if less than EvadeRegenerate milliseconds have gone by, evade wait mode
    else if (m_evadeTimer < EvadeRegenerate)
        m_evadeMode = EVADE_MODE_WAITING;
    // if less than EvadeReset milliseconds have gone by, evade regenerate mode
    else
    {
        // if we just switched into regenerate mode, zero the threat on the current target and find another
        if (oldEvadeTimer < EvadeRegenerate)
        {
            auto &threat = getThreatManager();
            auto const victim = getVictim();

            // using -100 here will set their threat to zero, but keep them in the table.  this
            // will allow them to remain targetted if there is nobody else to choose from.
            threat.modifyThreatPercent(victim, -100);

            // if there is another target, switch to them, do not evade
            if (SelectHostileTarget())
            {
                auto const newVictim = getVictim();

                // if we find a new victim, attack it and do not evade
                if (newVictim && newVictim != victim)
                {
                    i_AI->AttackStart(newVictim);
        ...
        // if we have not waited long enough or we are in a dungeon, regenerate, otherwise reset
        m_evadeMode = (m_evadeTimer < EvadeReset || inDungeon) ? EVADE_MODE_REGENERATING : EVADE_MODE_RESETTING;
    }

    // if we still are not in evade mode, and we are not in a dungeon, check to see if we should evade due to leashing
    if (!inDungeon)
    {
        // if we dont have a victim nothing below matters
        auto const victim = getVictim();
        if (!victim)
            return;

        // don't check distance to home position if recently damaged, this should include taunt auras
        if ((_lastDamagedTime + MAX_AGGRO_RESET_TIME) > sWorld.GetGameTime() || HasAuraType(SPELL_AURA_MOD_TAUNT))
            return;

        // at this point we know that it has been long enough since the last damaging attack.
        // check if we have gone too far and evade if so.

        //Use AttackDistance in distance check if threat radius is lower. This prevents creature bounce in and out of combat every update tick.
        auto const dist = m_CombatDistance + std::max(GetAttackDistance(victim), sWorld.getConfig(CONFIG_FLOAT_THREAT_RADIUS));

        if (Unit *unit = GetCharmerOrOwner())
        {
            // if the victim is close enough to the owner, do not evade
            if (victim->IsWithinDist(unit, dist))
                return;
        }
        else
        {
            ...
            if (isInCombat())
                GetCombatStartPosition(homeX, homeY, homeZ);
            else
                GetRespawnCoord(homeX, homeY, homeZ);

            // if the victim is close enough to the initial combat/spawn point, do not evade
            if (victim->IsWithinDist3d(homeX, homeY, homeZ, dist))
                return;
        }

        // if we reach here, it means we should reset immediately due to leashing
        m_evadeMode = EVADE_MODE_RESETTING;
    }
}
