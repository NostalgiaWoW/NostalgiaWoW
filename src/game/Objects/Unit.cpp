...
void Unit::DealDamageMods(Unit *victim, uint32 &damage, uint32* absorb)
{
    if (!victim->isAlive()     ||
        victim->IsTaxiFlying() ||
        (victim->GetTypeId() == TYPEID_UNIT && ((Creature*)victim)->GetEvadeMode() != EVADE_MODE_NONE) || // no damage in any evasion mode
        (getClass() == CLASS_PRIEST && HasAura(27827))) // [Nostalrius] No damage in Spirit of Redemption
    {
        if (absorb)
            *absorb += damage;
        damage = 0;
    ...
    auto const originalDamage = damage;
...
uint32 Unit::DealDamage(Unit *pVictim, uint32 damage, CleanDamage const* cleanDamage, DamageEffectType damagetype, SpellSchoolMask damageSchoolMask, SpellEntry const *spellProto, bool durabilityLoss, Spell* spell)
{
	...
	if (GetTypeId() == TYPEID_PLAYER && pVictim->GetTypeId() == TYPEID_UNIT)
        pVictim->RegisterAttack(GetObjectGuid(), getLevel());
    ...
    else                                                    // if (health <= damage)
    {
        ...
        if (pVictim->GetTypeId() != TYPEID_PLAYER)
        {
            ...
            if (damagetype != DOT && damage > 0 && !pVictim->GetOwnerGuid().IsPlayer() && (!spellProto || !spellProto->IsAuraAddedBySpell(SPELL_AURA_DAMAGE_SHIELD)))
                pVictim->ToCreature()->SetLastDamagedTime(sWorld.GetGameTime() + MAX_AGGRO_RESET_TIME);
            ...
            if (damagetype != DOT)
            {
                for (uint32 i = CURRENT_FIRST_NON_MELEE_SPELL; i < CURRENT_MAX_SPELL; ++i)
                {
                    ...
                    if (Spell* spell = pVictim->GetCurrentSpell(CurrentSpellTypes(i)))
                    {
                        ...
                        {
                            //#CLUTCH: Easter boss rabbit should not interrupt opening eggs
                            if (GetEntry() != 17997)
                            {
...
void Unit::Kill(Unit* pVictim, SpellEntry const *spellProto, bool durabilityLoss)
{
    ...
    // Hack: player can't die on taxi
    if (ToPlayer() && ToPlayer()->IsTaxiFlying())
        return;

    if (IsPlayer() && pVictim->IsPlayer() && GetMap()->IsContinent())
        ToPlayer()->GetSession()->CompleteLoyaltyEvent(LoyaltyEvent::KillPlayers);
...
/*              if (this != player_tap && GetMap()->IsDungeon() && !player_tap->IsInSameGroupWith(ToPlayer()))
                    player_tap = ToPlayer();
               }  */     
	...
	if (IsPlayer())
	{
        //Old logging
#if 0
 		sLog.outInfo("Player '%s' is kill a '%s'. Loot %s. Player_tap is '%s'. Group_tap is '%u'",
 			GetName(),
 			pVictim->GetName(),
 			allowLoot ? "allowed" : "NOT allowed",
 			player_tap ? player_tap->GetName() : "nullptr",
 			group_tap ? group_tap->GetId() : 0);
#endif
        //New logging
        if (GetMap()->IsDungeon())
        {
...
void Unit::DealSpellDamage(SpellNonMeleeDamage *damageInfo, bool durabilityLoss)
{
    ...
    if (!pVictim->isAlive() || pVictim->IsTaxiFlying() || (pVictim->GetTypeId() == TYPEID_UNIT && ((Creature*)pVictim)->GetEvadeMode() != EVADE_MODE_NONE))
        return;
...
void Unit::HandleEmoteAfterQuestGossip(uint32 emote_id)
{
    m_emoteOnPlayerGossip = emote_id;
}
...
MeleeHitOutcome Unit::RollMeleeOutcomeAgainst(const Unit *pVictim, WeaponAttackType attType, int32 crit_chance, int32 miss_chance, int32 dodge_chance, int32 parry_chance, int32 block_chance, bool SpellCasted) const
{
    ...
    if (pVictim->GetTypeId() == TYPEID_UNIT && ((Creature*)pVictim)->GetEvadeMode() != EVADE_MODE_NONE)
...
int32 Unit::MagicSpellHitChance(Unit *pVictim, SpellEntry const *spell, Spell* spellPtr)
{
    ...
    // HACK: It was 7 : 11 before, but caster with 50 lvl would never hit 60 lvl with such settings, it's not blizzlike
    // Now for example with 10 lvl difference (without additional resistances):
    // PVP - 96 - (10 - 2) * 4 = 64% hit chance
    // PVE - 96 - (10 - 2) * 7 = 40% hit chance
    int32 lchance = pVictim->GetTypeId() == TYPEID_PLAYER ? 4 : 7;
...
>>>>>>> parent of 27b7d4a... [Core] Remove Binary Spell mechanic
...
ReputationRank Unit::GetReactionTo(Unit const* target) const
{
    ...
    /// Summoned Voidwalker
    if (!IsPlayer() && !IsPet() && target->GetEntry() == 5676)
        return REP_NEUTRAL;
    ...
    if (HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_PVP_ATTACKABLE))
    {
        if (target->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_PVP_ATTACKABLE))
        {
            if (selfPlayerOwner && targetPlayerOwner)
            {
                ...
                /// GM Island
                if (selfPlayerOwner->GetZoneId() == 876)
                    return REP_FRIENDLY;
...
bool Unit::Attack(Unit *victim, bool meleeAttack)
{
    ...
    //#DANGER
    //#REMOVE_ME
    if (GetEntry() == 17997) return false;
...
bool Unit::IsGuardian(Unit *unit) const
{
    for (auto const &p : m_guardianPets)
        if (unit->GetObjectGuid() == p)
            return true;
...
int32 Unit::SpellBonusWithCoeffs(SpellEntry const *spellProto, int32 total, int32 benefit, int32 ap_benefit,  DamageEffectType damagetype, bool donePart, Unit *pCaster, Spell* spell)
{
    ...
    if (benefit)
    {
        ...
        if (spellProto->Id == 19993)
        {
            ...
            /// Increased Flash of Light Healing (Libram of Divinity)
            if (HasAura(28853))
                total += 53.0f;
...
void Unit::Mount(uint32 mount, uint32 spellId /*= 0*/, bool IsTaxi /*= false*/)
{
    if (!mount)
        return;
    ...
    //Giperion Elysium: Apply fly mode if this a gryphon
    if (!IsTaxi && mount == MODELID_GRYPHON)
    {
        SetFlying(true);
    }
}
...
bool Unit::_IsValidAttackTarget(Unit const* target, SpellEntry const* bySpell, WorldObject const* obj) const
{
            ...
            target->GetReactionTo(this) == REP_NEUTRAL)
    {
                ...
                !(!playerAffectingTarget && !playerAffectingAttacker))
        {
            ...
            {
                if (player && !(player->GetReputationMgr().GetForcedRankIfAny(factionTemplate)))
...
bool Unit::isVisibleForOrDetect(Unit const* u, WorldObject const* viewPoint, bool detect, bool /*inVisibleList*/, bool* alert) const
{
    ...
    /// Call of Fire - Minor Manifestation of Fire (ID: 5893) shouldn't see a player without Fire Sapta (ID: 6636) 
    if (u && u->GetEntry() == 5893 && !HasAura(8898))
        return false;
...
void Unit::AddThreat(Unit* pVictim, float threat /*= 0.0f*/, bool crit /*= false*/, SpellSchoolMask schoolMask /*= SPELL_SCHOOL_MASK_NONE*/, SpellEntry const *threatSpell /*= NULL*/)
{
    ...
    if (CanHaveThreatList() && (!ToCreature() || !ToCreature()->IsInEvadeMode()))
...
void Unit::TauntFadeOut(Unit *taunter)
{
    ...
    {
        // Nostalrius - no evade when we charm something.
        if (!GetCharmGuid())
            OnLeaveCombat();
...
bool Unit::SelectHostileTarget()
{
    ...
    {
        // Nostalrius : Correction bug sheep/fear
        if (!IsPolymorphed() && (!hasUnitState(UNIT_STAT_STUNNED | UNIT_STAT_PENDING_STUNNED | UNIT_STAT_DIED | UNIT_STAT_CONFUSED | UNIT_STAT_FLEEING) && (!HasAuraType(SPELL_AURA_MOD_FEAR) || HasAuraType(SPELL_AURA_PREVENTS_FLEEING)) && !HasAuraType(SPELL_AURA_MOD_CONFUSE)))
        {
            SetInFront(target);
    ...
    // if we are in a dungeon and there already is a target and we have no better choice, keep it and succeed.
    // the target may not be reachable but that is okay.  the evasion mechanic is implemented elsewhere.
    if (m_attacking && GetMap()->IsDungeon())
        return true;

    // if there are no targets left, evade immediately
    OnLeaveCombat();
...
void Unit::CleanupsBeforeDelete()
{
    ...
    {
        Uncharm();                                          // fix bug when server trying to remove charm effects after unit delete and it stuck
    ...
    if (procFlag & MELEE_BASED_TRIGGER_MASK && pTarget)
    {
        ...
        if (procExtra & (PROC_EX_CRITICAL_HIT | PROC_EX_PARRY | PROC_EX_DODGE | PROC_EX_BLOCK))
        {
            ...
            if (isVictim)
            {
                ...
                if (procExtra & PROC_EX_DODGE && pTarget->GetTypeId() == TYPEID_PLAYER && pTarget->getClass() == CLASS_WARRIOR)
                {
                    ((Player*)pTarget)->AddComboPoints(this, 1);
                    StartReactiveTimer(REACTIVE_OVERPOWER, this->GetObjectGuid());
                }
...
{
    return IsWithinMeleeRange(GetPositionX(), GetPositionY(), GetPositionZ(), obj, dist);
}

bool Unit::IsWithinMeleeRange(float x, float y, float z, const Unit *obj, float dist) const
{
    if (!obj || !IsInMap(obj))
        return false;
...
bool Unit::HasEmoteAfterQuestGossip() const
{
    return m_emoteOnPlayerGossip != 0;
}

uint32 Unit::GetEmoteAfterQuestGossip() const
{
    return m_emoteOnPlayerGossip;
}

void Unit::RegisterAttack(ObjectGuid guid, uint8 level)
{
    if (m_allAttackers.find(guid) == m_allAttackers.end())
        m_allAttackers.emplace(guid, level);
}
    ...
    else
    {
        ...
        if (rec > 0 || catrec > 0)
        {
            if (Player* modOwner = GetSpellModOwner())
            {
                ...
                //if (spellInfo->Id == 8690)
                //    if (uint32 mod = modOwner->GetHearthstoneCDReduction())
                //        catrec -= (catrec * mod) / 100;
            }
            if (Player* player = GetAffectingPlayer())
                if (player != this)
...
void Unit::SendPacketSetWaterWalking(bool apply)
{
    WorldPacket data;
    if (apply)
