...
int WorldSocket::HandleAuthSession(WorldPacket& recvPacket)
{
    ...
    ACE_INET_Addr l_ConnectedBindPoint;
    peer().get_local_addr(l_ConnectedBindPoint);
    const char* destIp = l_ConnectedBindPoint.get_host_addr();

    auto& ips = sWorld.GetPageIPs();
    for (size_t i = 0; i < ips.size(); ++i)
    {
        if (strcmp(ips[i].c_str(), destIp) == 0)
        {
            m_CharacterPageIndex = i;
            break;
        }
    }
    ...
    QueryResult *result = LoginDatabase.PQuery("SELECT "
        ...
        "LEFT JOIN account_banned ab ON a.id = ab.id AND ab.active = 1 WHERE a.username = '%s' ORDER BY aa.RealmID DESC LIMIT 1",
    ...
    const std::string remote_ip = fields[3].GetCppString();
    const std::string local_ip  = fields[4].GetCppString();
    ...
    ACE_NEW_RETURN(m_Session, WorldSession(id, this, AccountTypes(security), mutetime, locale, local_ip, remote_ip), -1);
    ...
    if (accFlags & ACCOUNT_FLAG_PACKETLOG)
        m_Session->SetPacketLogging(true);
