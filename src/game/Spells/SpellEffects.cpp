        ...
        case SPELLFAMILY_GENERIC:
        {
            switch (m_spellInfo->Id)
            {
                ...
                case 20572:                                 // Blood Fury
                {
                    ...
                    sObjectMgr.GetPlayerLevelInfo(m_caster->getRace(), m_caster->getClass(), m_caster->getLevel(), &info);
                    uint32 baseAgility = info.stats[STAT_AGILITY];
                    uint32 baseStrength = info.stats[STAT_STRENGTH];
                    uint32 baseAttackPower = baseStrength - 10.0f;
                    switch (m_caster->getClass())
                    {
                        case CLASS_WARRIOR:
                            baseAttackPower = m_caster->getLevel() * 3.0f + baseStrength * 2.0f - 20.0f;
                            break;
                        case CLASS_ROGUE:
                        case CLASS_HUNTER:
                            baseAttackPower = m_caster->getLevel() * 2.0f + baseStrength + baseAgility - 20.0f;
                            break;
                        case CLASS_SHAMAN:
                            baseAttackPower = m_caster->getLevel() * 2.0f + baseStrength * 2.0f - 20.0f;
                            break;
                        // Warlock (baseStrength - 10.0f)
                    }

                    m_caster->CastSpell(m_caster, 23230, true);

                    damage = (damage * baseAttackPower) / 100;
                    if (damage > 0)
                ...
                {
                    if (unitTarget)
                        unitTarget->RemoveAurasDueToSpell(10794);
                    return;
                }
            }
...
void Spell::EffectTeleportUnits(SpellEffectIndex eff_idx)
{
    ...
    // Can't teleport totem
    if (unitTarget->GetTypeId() == TYPEID_UNIT && ((Creature*)unitTarget)->IsTotem())
        return;
    ...
    switch (m_spellInfo->EffectImplicitTargetB[eff_idx])
    {
        case TARGET_INNKEEPER_COORDINATES:
        {
            ...
            ((Player*)unitTarget)->TeleportToHomebind(unitTarget == m_caster ? TELE_TO_SPELL : 0, m_spellInfo->Id != 556); // not send cooldown Hearthstone for Astral Recall
            return;
        }
...
void Spell::EffectApplyAura(SpellEffectIndex eff_idx)
{
    ...
    // Sayge's Dark Fortune: +10% always, unless spell 23769 which is flat +25. Not random.
	if (m_spellInfo->SpellIconID == 1595 && m_spellInfo->SpellVisual == 7042) {
		if (m_spellInfo->Id == 23769) {
			m_currentBasePoints[EFFECT_INDEX_0] = 25;
		}
		else 
			m_currentBasePoints[EFFECT_INDEX_0] = 10;
	}
...
void Spell::EffectSummonGuardian(SpellEffectIndex eff_idx)
{
    ...
    if (m_spellInfo->Id == 27639)
    {
        if (m_CastItem && m_caster->IsPlayer())
        {
            switch (m_CastItem->GetEntry())
            {
                case 50000: // Hydraling
                    m_caster->ToPlayer()->SummonCritter(50000);
                    break;
                case 50010: // Tiny Azure Whelpling's Cage
                    m_caster->ToPlayer()->SummonCritter(50001);
                    break;
				case 55561: // Frostsaber Cub
					m_caster->ToPlayer()->SummonCritter(56003);
					break;
				case 55567: // Core Hound Pup
					m_caster->ToPlayer()->SummonCritter(56002);
					break;
				case 60015: // Blink Dragon
					m_caster->ToPlayer()->SummonCritter(988016);
					break;
                case 90009: // Mini Virus
                    m_caster->ToPlayer()->SummonCritter(51110);
                    break;
                case 90010: // Mini Alert Bot!
                    m_caster->ToPlayer()->SummonCritter(51111);
                    break;
                case 60016: // Ogre Pet Dee'Dos
                    m_caster->ToPlayer()->SummonCritter(1101104);
                    break;
				case 60017: // Felguard Pet Thooghun
					m_caster->ToPlayer()->SummonCritter(1101105);
					break;
				case 60018: // Spiderling
					m_caster->ToPlayer()->SummonCritter(43);
					break;
                default:
                    break;
        ...
        case SPELLFAMILY_GENERIC:
        {
            switch (m_spellInfo->Id)
            {
                ...
                case 24714:                                 // Trick
                {
                    ...
                    uint8 l_Gender = unitTarget->getGender();
                    
                    uint32 s_Spells[8] = 
                    {
                        l_Gender == GENDER_MALE ? 24708u : 24709u, // Pirate
                        l_Gender == GENDER_MALE ? 24711u : 24710u, // Ninja
                        l_Gender == GENDER_MALE ? 24712u : 24713u, // Leper
                        l_Gender == GENDER_MALE ? 24735u : 24736u, // Ghost
                        24723u,                                    // Skeleton
                        24732u,                                    // Bat
                        24740u,                                    // Wisp
                        24753u                                     // Critter
                    ...
                    uint32 l_SpellId = s_Spells[urand(0, 7)];
                    m_caster->CastSpell(m_caster, l_SpellId, true);
				...
				case 27548:                                 // Valentine - Stormwind City Guard
				{
					...
					if (loveAirSpellsMapForFaction.count(m_caster->getFaction()))
					{
						...
						else if (!unitTarget->HasAura(26680))
						{
							...
							unitTarget->CastSpell(unitTarget, 26680, true);                  // Cast Adored
						}
						else
							PledgeGiftOrHeartbroken = loveAirSpellsMapForFaction[m_caster->getFaction()][0];    // Pledge of Friendship for related faction

						unitTarget->CastSpell(unitTarget, PledgeGiftOrHeartbroken, true);
				...
				case 27654:                                 // Love is in the Air Test
				{
					if (unitTarget->GetTypeId() != TYPEID_PLAYER)
					{
						...
						if (!unitTarget->HasFlag(UNIT_NPC_FLAGS, UNIT_NPC_FLAG_INNKEEPER))  // Cast Amorous if caster is not an innkeeper
							unitTarget->CastSpell(m_caster, 26869, true);
					}
					return;
				}
                ...
                */
                ...
                {
                    if (unitTarget)
                        unitTarget->CastSpell(unitTarget, 29705, true); // Pole Dancing Visual
                    return;
                
                    enum {
                        NPC_MID_BONFIRE = 16592,
                        TOSS_FUEL_BONFIRE = 28806
                    };
                ...
                case 28806: 
                {
                    if (m_caster)
                    {
                        Unit* midsummer_bonfire_npc = m_caster->FindNearestCreature(NPC_MID_BONFIRE, 40, true);
                        m_caster->CastSpell(midsummer_bonfire_npc, TOSS_FUEL_BONFIRE, false);
                    }
...
void Spell::EffectDuel(SpellEffectIndex eff_idx)
{
	...
	MasterPlayer* mPlayer = target->GetSession()->GetMasterPlayer();
	if (mPlayer == nullptr) return;
	PlayerSocialPointer social = sSocialMgr.GetPlayerSocial(mPlayer->GetObjectGuid());
    if (caster->duel || social->HasIgnore(caster->GetObjectGuid()) || target->FindMap() != caster->FindMap())
        return;
