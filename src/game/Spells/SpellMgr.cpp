...
uint32 GetSpellCastTime(SpellEntry const* spellInfo, Spell* spell)
{
    ...
    if (spellInfo->Id == 27639 && spell && spell->m_CastItem)
        castTime = 1000;
...
bool SpellMgr::IsRankSpellDueToSpell(SpellEntry const *spellInfo_1, uint32 spellId_2) const
{
    ...
    if (spellInfo_1->SpellFamilyName == spellInfo_2->SpellFamilyName &&
        ...
        spellInfo_1->Effect[1] == spellInfo_2->Effect[1] && // added this to allow the two auras created by Sheen of Zanza to stack (https://github.com/elysium-project/classic-bug-tracker/issues/754)
        spellInfo_1->Effect[2] == spellInfo_2->Effect[2] && // added this because why would we exclude it if we are already checking the other two
...
SpellCastResult SpellMgr::GetSpellAllowedInLocationError(SpellEntry const *spellInfo, Unit const* caster, Player const* player, uint32* param1)
{
    // bg spell checks
        ...
        if (saBounds.first != saBounds.second)
        {
            ...
            for (SpellAreaMap::const_iterator itr = saBounds.first; itr != saBounds.second; ++itr)
            {
                if (itr->second.IsFitToRequirements(player, zone, area))
                    return SPELL_CAST_OK;
                else if (param1)    // for spell fail client output
                    *param1 = itr->second.areaId;
            }            
            return SPELL_FAILED_REQUIRES_AREA;
        }
    }
