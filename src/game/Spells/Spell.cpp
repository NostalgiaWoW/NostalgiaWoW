            ...
            case TARGET_SELF:
                switch (m_spellInfo->EffectImplicitTargetB[i])
                {
                    case TARGET_NONE:
                        ...
                        {
                            if (Unit* pUnitTarget = m_caster->SelectMagnetTarget(m_targets.getUnitTarget(), this, SpellEffectIndex(i + 1)))
                                if (m_caster->IsPlayer() && m_caster->IsValidAttackTarget(pUnitTarget))
...
void Spell::AddUnitTarget(Unit* pVictim, SpellEffectIndex effIndex)
{
    ...
    if ((m_spellInfo->AttributesEx & SPELL_ATTR_EX_CANT_TARGET_SELF) && (m_spellInfo->EffectImplicitTargetA[effIndex] != TARGET_SELF) &&
        (m_spellInfo->EffectImplicitTargetB[effIndex] != TARGET_SELF) && (pVictim->GetObjectGuid() == m_originalCaster->GetObjectGuid()))
        return;
...
void Spell::CheckAtDelay(TargetInfo* pInf)
{
    ...
    if (pTarget != m_caster && (pTarget->IsImmuneToDamage(GetSpellSchoolMask(m_spellInfo), m_spellInfo) || pTarget->IsImmuneToSpell(m_spellInfo, pTarget == m_caster)))
        pInf->missCondition = SPELL_MISS_IMMUNE;

    // spells should evade for all evade modes
    if (pTarget->GetTypeId() == TYPEID_UNIT && ((Creature*)pTarget)->GetEvadeMode() != EVADE_MODE_NONE)
...
void Spell::DoAllEffectOnTarget(TargetInfo *target)
{
    ...
    if (real_caster)
    {
        // Restore energy on miss/dodge for spells that grant combo points
        ...
        // Warriors and Druids are refunded 80% of the rage cost on dodge/parry/miss
...
void Spell::DoSpellHitOnUnit(Unit *unit, uint32 effectMask)
{
    ...
    // Sidsukana: Temporary fix. Sets PvP flag for totem's owner if totem do attack enemy
    if (m_caster->ToCreature() && m_caster->ToCreature()->IsTotem() && !IsPositiveSpell(m_spellInfo->Id))
    {
        Unit* owner = m_caster->GetCharmerOrOwner();
        if (owner && owner->IsPlayer() && !owner->IsPvP())
        {
            Player* pOwner = owner->ToPlayer();
            if (unitTarget && (unitTarget->IsPlayer() && unitTarget->IsPvP() && !pOwner->IsInDuelWith((Player*)unitTarget)) || // PvP flagged players
                (unitTarget->IsCreature() && unitTarget->IsPvP()))                                           // PvP flagged creatures
            {
...
void Spell::SetTargetMap(SpellEffectIndex effIndex, uint32 targetMode, UnitList& targetUnitMap)
{
    ...
    switch (targetMode)
    {
        ...
        case TARGET_AREAEFFECT_INSTANT:
        {
            ...
            const SpellTargets targetB = IsPositiveEffect(m_spellInfo, effIndex) ? SPELL_TARGETS_FRIENDLY : SPELL_TARGETS_AOE_DAMAGE;
            ...
            else
            {
                ...
                for (auto const &iter : tempTargetUnitMap)
                {
                    ...
                    for (auto i_spellST = bounds.first; i_spellST != bounds.second; ++i_spellST)
                    {
                        ...
                        if (i_spellST->second.type == SPELL_TARGET_TYPE_GUARDIAN && m_caster->IsGuardian(iter))
                            targetUnitMap.push_back(iter);
                        else if (i_spellST->second.type == SPELL_TARGET_TYPE_DEAD && ((Creature*)iter)->IsCorpse())
                            targetUnitMap.push_back(iter);
        ...
        case TARGET_DEST_CASTER_FRONT_LEAP:
        {
            ...
            //#GIPERION: Check Dest coords accourding spell info
            if (!pUnitTarget && dis > FLT_SMALL_NUMBER)
            {
                Position spellPos;
                m_targets.getDestination(spellPos);
                float selectedDist = m_caster->GetDistance(spellPos);
                if (selectedDist > dis)
                {
                    DETAIL_LOG("SpellSystem: '%s' tried to cast spell '%s' with teleport effect (TARGET_DEST_CASTER_FRONT_LEAP) that have max distance '%f', but requested with distance '%f'", 
                        m_caster->GetName(), m_spellInfo->SpellName[0],
                        dis, selectedDist);

                    //truncate distance
                    WorldLocation CurrentPos;
                    m_caster->GetPosition(CurrentPos);
                    Vector3 diff(spellPos.x - CurrentPos.coord_x,
                        spellPos.y - CurrentPos.coord_y,
                        spellPos.z - CurrentPos.coord_z);

                    Vector3 truncatedVector = diff.direction() * dis;
                    m_targets.setDestination(CurrentPos.coord_x + truncatedVector.x, 
                        CurrentPos.coord_y + truncatedVector.y,
                        CurrentPos.coord_z + truncatedVector.z);
                }
                break;
            }
...
void Spell::SendCastResult(SpellCastResult result, uint32* param1 /*= nullptr*/, uint32* param2 /*= nullptr*/) const
{
    ...
    SendCastResult(m_caster->ToPlayer(), m_spellInfo, result, param1, param2);
}

void Spell::SendCastResult(Player* caster, SpellEntry const* spellInfo, SpellCastResult result, uint32* param1 /*= nullptr*/, uint32* param2 /*= nullptr*/)
{
...
{
    size_t effectCount = 0LL;
    for (auto const& cont : m_executeLogInfo)
        effectCount += cont.size();
    ...
    for (uint32 i = 0; i < MAX_EFFECT_INDEX; ++i)
    {
        ...
        for (ExecuteLogInfo const& info : m_executeLogInfo[i])
        {
        ...
        if (proto->Spells[i].SpellId)
        {
            ...
            if (proto->Spells[i].SpellCharges)
            {
                ...
                if (charges)
                {
                    ...
                    if (proto->Stackable < 2 || proto->Stackable == 200)
...
SpellCastResult Spell::CheckCast(bool strict, uint32* param1 /*= nullptr*/, uint32* param2 /*= nullptr*/)
{
    ...
    if (m_caster->GetTypeId() == TYPEID_PLAYER)
    {
        ...
        if (m_caster->IsMoving())
        {
            ...
            // Sidsukana: Custom mini pets
            if (m_spellInfo->Id == 27639 && m_CastItem)
                return SPELL_FAILED_MOVING;
        }
    ...
    if (Unit *target = m_targets.getUnitTarget())
    {
        ...
        if (non_caster_target)
        {
            ...
            //Giperion Elysium: Don't check target friendlines on triggered spells
            if (!m_IsTriggeredSpell)
            {
    ...
    // zone check
    SpellCastResult locRes = sSpellMgr.GetSpellAllowedInLocationError(m_spellInfo, m_caster, m_caster->GetCharmerOrOwnerPlayerOrPlayerItself(), param1);
    ...
    if (!m_IsTriggeredSpell)
    {
        if (!m_triggeredByAuraSpell)
        {
            ...
            if (Player* playerCaster = m_caster->ToPlayer())
                ...
                {
                    castResult = bg->CheckSpellCast(playerCaster, m_spellInfo, &m_targets);
                    if (castResult != SPELL_CAST_OK)
                        return castResult;
                }
        }
    ...
    for (int i = 0; i < MAX_EFFECT_INDEX; ++i)
    {
        switch (m_spellInfo->EffectApplyAuraName[i])
        {
            ...
            case SPELL_AURA_MOD_SHAPESHIFT:
            {
                ...
                if (m_spellInfo->Id == 783 || m_spellInfo->Id == 2645)
                    if (m_caster->GetAreaId() == 35)
                        return SPELL_FAILED_NO_MOUNTS_ALLOWED;
