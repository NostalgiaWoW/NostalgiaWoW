...
#include "PlayerAI.h"
#include <custom/project_social.h>
...
Aura::Aura(SpellEntry const* spellproto, SpellEffectIndex eff, int32 *currentBasePoints, SpellAuraHolder *holder, Unit *target, Unit *caster, Item* castItem) :
    m_spellmod(nullptr), m_periodicTimer(0), m_periodicTick(0), m_customVirusTimer(urand((TIMER_INFECT / 2), TIMER_INFECT)), m_removeMode(AURA_REMOVE_BY_DEFAULT),
    ...
    m_applied(false), m_afterInitOnce(false)
{
    ...
    int32 miscValue = spellproto->EffectMiscValue[eff];
    ...
    if (castItem)
    {
        enum eCustomMounts
        {
			...
			JadeRaptor = 55573,
			JadeRaptorVisual = 6506,

			SpectralGryphon = 55574,
			SpectralGryphonVisual = 1000246,
        ...
        case Zhewra:
            if (eff == EFFECT_INDEX_1)
                ...
                miscValue = damage = ZhewraVisual;
            break;
        case Rakshiri:
            if (eff == EFFECT_INDEX_1)
                damage = 100;
            else if (eff == EFFECT_INDEX_0)
                miscValue = damage = RakshiriVisual;
            break;
        case Tallstrider:
            if (eff == EFFECT_INDEX_1)
                damage = 100;
            else if (eff == EFFECT_INDEX_0)
                miscValue = damage = TallstriderVisual;
            break;
        case Hippogryph:
            if (eff == EFFECT_INDEX_1)
                damage = 100;
            ...
            if (eff == EFFECT_INDEX_1)
                ...
                miscValue = damage = SnowHippogryphVisual;
            break;
        case ObsidianRaptor:
            if (eff == EFFECT_INDEX_1)
                damage = 100;
            else if (eff == EFFECT_INDEX_0)
                miscValue = damage = ObsidianRaptorVisual;
            break;
        case WhiteSaber:
            if (eff == EFFECT_INDEX_1)
                damage = 100;
            else if (eff == EFFECT_INDEX_0)
                miscValue = damage = WhiteSaberVisual;
            break;
        case ShadowStag:
            if (eff == EFFECT_INDEX_1)
                damage = 0;
            else if (eff == EFFECT_INDEX_0)
                miscValue = damage = ShadowStagVisual;
            break;
        case WhiteWolf:
            if (eff == EFFECT_INDEX_1)
                damage = 100;
            else if (eff == EFFECT_INDEX_0)
                miscValue = damage = WhiteWolfVisual;
            break;
		case ShadowCharger:
			if (eff == EFFECT_INDEX_1)
				damage = 100;
			else if (eff == EFFECT_INDEX_0)
				miscValue = damage = ShadowChargerVisual;
			break;
		...
		case BatLord:
			if (eff == EFFECT_INDEX_1)
				...
				miscValue = damage = BatLordVisual;
			break;
        ...
        case WhiteUnicorn:
            {
                if (eff == EFFECT_INDEX_1)
                    ...
                    miscValue = damage = WhiteUnicornVisual;
                break;
            }
		...
		case JadeRaptor:
		{
			if (eff == EFFECT_INDEX_1)
				...
				miscValue = damage = JadeRaptorVisual;
			break;
		}

		case SpectralGryphon:
		{
			if (eff == EFFECT_INDEX_1)
				damage = 100;
			else if (eff == EFFECT_INDEX_0)
				miscValue = damage = SpectralGryphonVisual;
			break;
		}
		case WildRazorbeak:
		{
			if (eff == EFFECT_INDEX_1)
				...
				miscValue = damage = WildRazorbeakVisual;
			break;
		}
    ...
    SetModifier(AuraType(spellproto->EffectApplyAuraName[eff]), damage, spellproto->EffectAmplitude[eff], miscValue);
...
void Aura::Update(uint32 diff)
{
    ...
    if (!m_afterInitOnce)
    {
        ...
        {
            //When player loading, you can't get Item from GetCastItemGuid(), since inventory not loaded yet
            //We can't change load order, so here we need do additional initializing
            const ObjectGuid& ItemGuid = GetCastItemGuid();

            //if caster a player - get item type id
            if (Unit* target = GetCaster())
            {
                uint32 ItemEntry = 0;
                if (ItemGuid && target->GetTypeId() == TYPEID_PLAYER)
                    if (Item* item = ((Player*)target)->GetItemByGuid(ItemGuid))
                        ItemEntry = item->GetEntry();

                ApplyItemMountSpecific(ItemEntry, target);
            }
        }
    }

    // custom virus event
    if (GetId() == SPELL_INFECTION || GetId() == SPELL_INFECTION_NPC)
    {
        if (m_customVirusTimer <= diff)
        {
            custom_AuraUpdate(this);
            m_customVirusTimer = urand((TIMER_INFECT / 2), TIMER_INFECT);
        }
        else
            m_customVirusTimer -= diff;
    }
}
...
void Aura::TriggerSpell()
{
	...
	// not in reactive state exclude arcane missles and Exploding
	if (m_modifier.m_amount == 0 &&
        triggerTarget->hasUnitState(UNIT_STAT_CAN_NOT_REACT) &&
        !GetSpellProto()->IsFitToFamily<SPELLFAMILY_MAGE, CF_MAGE_ARCANE_MISSILES_CHANNEL>() &&
        auraId != 2893 &&   // Abolish Poison
        auraId != 25698 &&  // Explode
        auraId != 28522 &&  // Icebolt
        auraId != 27808)    // Frost Blast
		return;
...
void MorphToRabbit(Player* player)
{
    uint32 MorphTarget = RabbitModelId::RABBIT_ALLIANCE;
    if (player->GetTeam() == Team::HORDE)
    {
        MorphTarget = RabbitModelId::RABBIT_HORDE;
    }
    player->SetDisplayId(MorphTarget);
}
...
void Aura::HandleAuraDummy(bool apply, bool Real)
{
    ...
    // custom virus event
    if (GetId() == SPELL_INFECTION || GetId() == SPELL_INFECTION_NPC 
        || GetId() == SPELL_ONSET || GetId() == SPELL_CLEAN_HANDS 
        || GetId() == SPELL_CLEAN_HANDS)
        custom_HandleAuraDummy(this, target, apply, m_removeMode);
    ...
    switch (GetSpellProto()->SpellFamilyName)
    {
        case SPELLFAMILY_GENERIC:
        {
            switch (GetId())
            {
                ...
                case 6946: //nobblegarden rabbit fear (casted by rabbit boss)
                {
                    if (Player* player = target->ToPlayer())
                    {
                        if (apply)
                        {
                            MorphToRabbit(player);
                            //Move from caster
                            if (Unit* caster = GetCaster())
                            {
                                WorldLocation CasterLoc;
                                caster->GetPosition(CasterLoc);
                                WorldLocation PlayerLoc;
                                player->GetPosition(PlayerLoc);
                                Vector3 diff(PlayerLoc.coord_x - CasterLoc.coord_x,
                                    PlayerLoc.coord_y - CasterLoc.coord_y,
                                    PlayerLoc.coord_z - CasterLoc.coord_z);
                                
                                Vector3 FinalDest = diff.direction() * 150.0f;
                                
                                float dX = CasterLoc.coord_x + FinalDest.x;
                                float dY = CasterLoc.coord_y + FinalDest.y;
                                float dZ = caster->GetMap()->GetHeight(dX, dY, CasterLoc.coord_z + 20.0f, true, 50.0f);
                                player->GetMotionMaster()->MovePoint(0, dX, dY, dZ, MOVE_PATHFINDING);
                            }
                        }
                        else
                        {
                            player->DeMorph();
                            player->GetMotionMaster()->Clear(true, true);
                            player->GetMotionMaster()->MoveIdle();
                            player->StopMoving(true);
                            //player->SendMovementInfo(MSG_MOVE_STOP);
                        }
                    }
                    return;
                }
                case 8067: //nobblegarden egg polymorph
                {
                    if (Unit* caster = GetCaster())
                    {
                        if (Player* player = caster->ToPlayer())
        ...
        case SPELLFAMILY_PALADIN:
        {
            ...
            if (GetSpellProto()->SpellIconID == 237 && target->GetTypeId() == TYPEID_PLAYER)
            {
                target->HandleStatModifier(UNIT_MOD_DAMAGE_MAINHAND, BASE_PCT, -28.5f, apply);
                target->HandleStatModifier(UNIT_MOD_DAMAGE_OFFHAND, BASE_PCT, -28.5f, apply);
            }
            break;
        }
    }
...
void Aura::HandleAuraMounted(bool apply, bool Real)
{
    ...
    if (apply)
    {
        ...
        if (m_modifier.m_amount)
            misc = m_modifier.m_amount;

        CreatureInfo const* ci = ObjectMgr::GetCreatureTemplate(misc);
        if (!ci)
        {
            sLog.outErrorDb("AuraMounted: `creature_template`='%u' not found in database (only need it modelid)", misc);
            return;
        }
...
void Aura::ApplyItemMountSpecific(uint32 ItemEntry, Unit * target)
{
    // Overwriting the default DBC's values for spells: 10790, 16058, 16059, 16060
    if (ItemEntry == 50018 || ItemEntry == 12325 || ItemEntry == 12326 || ItemEntry == 12327 || ItemEntry == 8630 || ItemEntry == 55560 || ItemEntry == 55562 || ItemEntry == 13325)
    {
        //Inject speed aura effector
        //22717 - Black War Steed
        SpellEntry const *spellInfo = sSpellMgr.GetSpellEntry(22717);
        Aura *aur = CreateAura(spellInfo, EFFECT_INDEX_1, NULL, GetHolder(), target);
...
void Aura::HandleAuraWaterWalk(bool apply, bool Real)
{
    ...
    GetTarget()->SendPacketSetWaterWalking(apply);
}
...
void Aura::HandleAuraTransform(bool apply, bool Real)
{
    ...
    if (apply)
    {
        ...
        if (!target->getTransForm() || !IsPositiveSpell(GetId()) || IsPositiveSpell(target->getTransForm()))
        {
            ...
            /// Dire Wolf Form and Tree Form shouldn't change scale of Alzzin the Wildshaper
            if ((GetId() == 22660 || GetId() == 22688) && target->GetEntry() == 11492)
                mod_x = 0.0f;
...
void Aura::HandleForceReaction(bool apply, bool Real)
{
    ...
    // Hackfix: Don't remove faction when Gordok Ogre Suit (22736) removes, if player has King of the Gordok (22799), and the same if players removes King of the Gordok (22799) while Gordok Ogre Suit (22736) is up
    if (m_spellAuraHolder && m_spellAuraHolder->GetSpellProto() && ((m_spellAuraHolder->GetId() == 22736 && player->HasAura(22799)) || (m_spellAuraHolder->GetId() == 22799 && player->HasAura(22736))))
        return;
...
void Unit::ModPossess(Unit* target, bool apply, AuraRemoveMode m_removeMode)
{
    ...
    else
    {
        ...
        // fix issue with charm VS Confuse
        if (target->HasAuraType(SPELL_AURA_MOD_CONFUSE))
            target->SetConfused(true, caster->GetObjectGuid());	
    }
...
void Aura::HandleModCharm(bool apply, bool Real)
{
    ...
    else
    {
        ...
        if(target->GetTypeId() != TYPEID_PLAYER)
        {
            ...
            if (cinfo && caster->IsPlayer() && caster->getClass() == CLASS_WARLOCK && cinfo->type == CREATURE_TYPE_DEMON)
            {
        ...
        if (caster->IsPlayer())
            caster->ToPlayer()->RemovePetActionBar();
        
        target->UpdateControl();

        if (target->GetTypeId() == TYPEID_PLAYER)
            static_cast<Player*>(target)->setFactionForRace(target->getRace());
        ...
        target->SetUnitMovementFlags(MOVEFLAG_NONE);
        target->StopMoving(target->IsPlayer());
    ...
    if (apply)
    {
        ...
        switch (spellProto->SpellFamilyName)
        {
            case SPELLFAMILY_DRUID:
            {
                ...
                else if (spellProto->IsFitToFamilyMask<CF_DRUID_RAKE_CLAW>()) // Rake
                {
                    m_modifier.m_amount += int32(caster->GetTotalAttackPowerValue(BASE_ATTACK) * 2 / 100); // 2% per tick
                }
                break;
            }
            case SPELLFAMILY_ROGUE:
