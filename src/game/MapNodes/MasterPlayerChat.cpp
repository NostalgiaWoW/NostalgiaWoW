...
void MasterPlayer::Whisper(const std::string& text, uint32 language, MasterPlayer* receiver)
{
    ...
    if (this == receiver || !m_session->GetAnticheat()->IsSilenced())
    {
        ...
        SessionAnticheatInterface* receiverAnticheat = receiver->GetSession()->GetAnticheat();
        if (!receiverAnticheat->IsSilenced())
        {
            if (receiver->isDND())
            {
                data.Initialize(SMSG_MESSAGECHAT, 100);
                ...
                sWorld.LogChat(receiver->GetSession(), "Whisp", receiver->dndMsg);
                receiverAnticheat->Whisper(receiver->dndMsg, GetObjectGuid());
            }
            else if (receiver->isAFK())
            {
                ...
                sWorld.LogChat(receiver->GetSession(), "Whisp", receiver->afkMsg);
                receiverAnticheat->Whisper(receiver->afkMsg, GetObjectGuid());
            }
        }
        if (!IsAcceptWhispers())
