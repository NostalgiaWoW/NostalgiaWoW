...
void Map::DeleteFromWorld(Player* player)
{
	...
	player->GetSession()->SetPlayer(nullptr);
    delete player;
}
    ...
    for (m_mapRefIter = m_mapRefManager.begin(); m_mapRefIter != m_mapRefManager.end(); ++m_mapRefIter)
    {
        ...
        if (plr->isInCombat())
        {
            ...
            for (Creature* c : updateList)
                UpdateCellsAroundObject(now, diff, c);
        }
    }
...
bool DungeonMap::CanExit(Player* player)
{
    //Always allow for GM's. Dead players also allowed to release their body
    if (player->isGameMaster() || player->isDead()) return true;
    ...
    if (IsRaid())
    {
        if (Group* group = player->GetGroup())
        {
            for (GroupReference* itr = group->GetFirstMember(); itr != nullptr; itr = itr->next())
            {
                ...
                {
                    if (groupMember->FindMap() == this && groupMember->isInCombat())
                        return false;
                }
            }
        }
...
void DungeonMap::PermBindAllPlayers(Player *player)
{
    ...
    for (MapRefManager::iterator itr = m_mapRefManager.begin(); itr != m_mapRefManager.end(); ++itr)
    {
        ...
        if (IsRaid())
            plr->GetSession()->CompleteLoyaltyEvent(LoyaltyEvent::KillRaidBoss);
    }
}
...
void DungeonMap::UnloadAll(bool pForce)
{
    if (HavePlayers())
    {
        sLog.outError("DungeonMap::UnloadAll: there are still players in the instance at unload, should not happen! MapId: %u", GetId());
    ...
    while (!m_scriptSchedule.empty() && (iter->first <= sWorld.GetGameTime()))
    {
        ...
        switch (step.script->command)
        {
            ...
            case SCRIPT_COMMAND_TERMINATE_COND:
            {
                ...
                if (player && step.script->terminateCond.flags & 0x01)
...
bool Map::IsValid(ObjectGuid guid)
{
    if (guid.IsPlayer())
        return GetPlayer(guid) != nullptr;

    return GetAnyTypeCreature(guid) != nullptr;
}
