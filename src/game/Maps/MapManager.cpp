...
void LoadMapThreadEntry(TerrainInfo* terrain)
{
    terrain->AddRef();
    terrain->LoadAll();
}

void LoadMapRangeThreadEntry(TerrainInfo** pTerrains, uint32 Count)
{
...
MapManager::Initialize()
{
    ...
    int32 NativeThreads = (int32)std::thread::hardware_concurrency();
    std::vector<TerrainInfo*> Instances;
    Instances.reserve(sMapStorage.GetRecordCount() - 2);
    for (auto itr = sMapStorage.begin<MapEntry>(); itr < sMapStorage.end<MapEntry>(); ++itr)
    {
        ...
        if (itr->IsContinent())
        {
            threads.push_back(std::thread(LoadMapThreadEntry, terrain));
            --NativeThreads;
            NativeThreads = NativeThreads < 1 ? 1 : NativeThreads;
        }
        else
        {
            //collect all instances to array, then split to remaining threads
            Instances.push_back(terrain);
        }
    }

    int32 InstancePortionPerThread = Instances.size() / NativeThreads;
    int32 LeastPortion = Instances.size() % NativeThreads;

    for (int i = 0; i < NativeThreads; ++i)
    {
        threads.push_back(std::thread(LoadMapRangeThreadEntry, Instances.data() + (InstancePortionPerThread * i), InstancePortionPerThread));
    }

    if (LeastPortion != 0)
    {
        threads.push_back(std::thread(LoadMapRangeThreadEntry, Instances.data() + (Instances.size() - LeastPortion), LeastPortion));
    }
