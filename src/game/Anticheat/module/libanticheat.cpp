...
namespace
{
Warden *CreateWarden(WorldSession *session, const BigNumber &K, ElysiumAnticheat::SessionAnticheat *anticheat)
{
    Warden* warden;
    ...
    {
        anticheat->RecordCheatInternal(ElysiumAnticheat::CheatType::CHEAT_TYPE_WARDEN, "Unknown client operating system");
...
void LogCheat(WorldSession *session, uint32 actionMask, const std::string &info)
{
    ...
    if (auto const anticheat = dynamic_cast<ElysiumAnticheat::SessionAnticheat *>(session->GetAnticheat()))
...
void SessionAnticheat::NewPlayer()
{
    ...
    if (!player)
        ...
        _movementData = std::make_unique<ElysiumAnticheat::Movement>(player);
}
...
void SessionAnticheat::SendPlayerInfo(ChatHandler *handler) const
{
    ...
    if (includeFingerprint)
        handler->PSendSysMessage("OS: %s Build: %u Fingerprint: |cffffffff|Hplayer:0x%lx|h[0x%lx]|h|r Local IP: %s",
            _session->GetOS() == CLIENT_OS_WIN ? "Win" : "Mac", _session->GetGameBuild(), _fingerprint, _fingerprint, _session->GetLocalAddress().c_str());

    _warden->SendPlayerInfo(handler, includeFingerprint);
}

void SessionAnticheat::SendPlayerLoginInfo(ChatHandler* handler) const
{
    auto const includeFingerprint = !handler->GetSession() ||
        static_cast<uint32>(handler->GetSession()->GetSecurity()) >= sAnticheatConfig.GetFingerprintLevel();

    if (includeFingerprint)
        handler->PSendSysMessage("Account: %s (id:%s) - Online: %s - Fingerprint: |cffffffff|Hplayer:0x%lx|h[0x%lx]|h|r - Loc IP: %s",
            playerLink(_session->GetUsername()).c_str(), playerLink(std::to_string(_session->GetAccountId())).c_str(),
            playerLink(_session->GetPlayerName()).c_str(), _fingerprint, _fingerprint, (_session->GetLocalAddress()).c_str());
}
...
{
    static ElysiumAnticheat::AnticheatLib l;
    return &l;
}
