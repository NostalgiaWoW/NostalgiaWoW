...
#define DARKMOON_EVENTSTATE_OVERRIDE_VARIABLE_ID 5624
...
void DarkmoonFaire::Update()
{
    ...
    //That's a clutch, to force enabling Darkmoon faire if we want to
    uint32 DarkmoonStateOverride = sObjectMgr.GetSavedVariable(DARKMOON_EVENTSTATE_OVERRIDE_VARIABLE_ID, 0);
    if (DarkmoonStateOverride != 0)
    {
        switch (DarkmoonStateOverride)
        {
        case 1:
            event = DarkmoonState::DARKMOON_A2;
            break;
        case 2:
            event = DarkmoonState::DARKMOON_H2;
            break;
        }
    }
...
DarkmoonState DarkmoonFaire::GetDarkmoonState()
{
    ...
    uint32 firstMonday = FindMonthFirstMonday(faireAlly, timeinfo);
    uint32 tm_mday = uint32(timeinfo->tm_mday);
...
NobblegardenElysium::NobblegardenElysium()
    : WorldEvent(EVENT_NOBBLEGARDER_EVENT_ID)
{

}

void NobblegardenElysium::Disable()
{
}

void NobblegardenElysium::Enable()
{
}

void NobblegardenElysium::Update()
{
}
...
void GameEventMgr::LoadHardcodedEvents(HardcodedEventList& eventList)
{
    ...
    auto easterEvent = new NobblegardenElysium();
    eventList = { invasion, leprithus, moonbrook, nightmare, darkmoon, lunarfw, silithusWarEffortBattle, scourge_invasion, easterEvent };
}
