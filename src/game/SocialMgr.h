...
class PlayerSocial
{
    ...
    public:
		...
		void SetMasterPlayerObjectGuid(ObjectGuid m) { m_masterPlayerGuid = m; }
		ObjectGuid GetMasterPlayerGuid() const { return m_masterPlayerGuid; }
        void Clear();
...
class PlayerSocialPointer
{
public:
	PlayerSocialPointer(PlayerSocial* InSocialData);
	PlayerSocialPointer(const PlayerSocialPointer& other);

    void Init();

	~PlayerSocialPointer();

	PlayerSocial* operator->() const;

private:
	PlayerSocial* pSocialData;
    std::atomic<int>* Counter;
};

class SocialMgr
{
	friend class PlayerSocialPointer;
    public:
        SocialMgr();
        ~SocialMgr();
        // Misc
        void RemovePlayerSocial(uint32 guid);
		PlayerSocialPointer GetPlayerSocial(ObjectGuid playerGuid);
    ...
    private:
		...
		ACE_Based::RecursiveRWMutex m_socialMapGuard;
        SocialMap m_socialMap;

		PlayerSocial m_defaultSocial;
};
