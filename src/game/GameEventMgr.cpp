...
void GameEventMgr::UpdateSilithusPVP()
{
    ...
    if (event == SILITHUS_PVP_EVENT_ON)
    {
        if (!IsActiveEvent(SILITHUS_PVP_EVENT_ON))
        {
            ...
            sWorld.SendGlobalText("Silithyst collectors are repaired! Hurry back to Silithus and resume your duties!", nullptr);
        }
    }
    else if (IsActiveEvent(SILITHUS_PVP_EVENT_ON))
    {
        ...
        sWorld.SendGlobalText("The sand has gotten stuck on our Silithyst collectors, disabling the machinery in Silithus. They should be operational in about 6 hours.", nullptr);
    }
}
