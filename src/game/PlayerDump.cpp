...
DumpReturn PlayerDumpReader::LoadDump(const std::string& file, uint32 account, std::string name, uint32& guid, uint8 p_OldRealmId)
{
    // check character count
    uint32 charcount = sAccountMgr.GetCharactersCount(account);

    QueryResult* l_CheckCharacter = CharacterDatabase.PQuery("SELECT * FROM `characters` WHERE `old_guid` = '%u' AND `account`='%u' AND `old_realm`='%u'", guid, account, p_OldRealmId);

    if (charcount >= uint32(30 + (!l_CheckCharacter ? 0 : 1)))
    ...
    {
        result = CharacterDatabase.PQuery("SELECT `name` FROM `characters` WHERE `guid` = '%u'", guid);
        if (result)
        {
            Field* l_Field = result->Fetch();
            std::string l_Name = l_Field->GetCppString();
    ...
    while (!feof(fin))
    {
        ...
        switch (type)
        {
            ...
            case DTT_CHARACTER:
            {
                ...
                bool l_RenamedChar = false;

                QueryResult* l_CheckRename = CharacterDatabase.PQuery("SELECT `name` FROM `characters` WHERE `old_realm` = '%u' AND `old_guid` = '%u'", p_OldRealmId, l_OldGuid);
                if (l_CheckRename)
                {
                    Field* l_CheckRenameField = l_CheckRename->Fetch();
                    std::string l_CheckNameStr = l_CheckRenameField->GetCppString();
                    l_RenamedChar = isdigit(l_CheckNameStr[0]);
                }

                if (l_RenamedChar || name == "")
                {
                    ...
                    if (sObjectMgr.GetPlayer(name.c_str()))
                        return DUMP_CHAR_ONLINE;

                    result = CharacterDatabase.PQuery("SELECT `guid`, `honorRankPoints`, `honorHighestRank`, `honorStanding`, `honorLastWeekHK`, `honorLastWeekCP`, `honorStoredHK`, `honorStoredDK` FROM `characters` WHERE `old_realm` = '%u' AND `old_guid` = '%u'", p_OldRealmId, l_OldGuid);
                    if (result)
                    {
                        Field* l_Field = result->Fetch();
                        uint32 l_LowGuid = l_Field[0].GetUInt32();
                        std::string l_HonorRankPoints = std::to_string(l_Field[1].GetUInt32());
                        std::string l_HonorHighestRank = std::to_string(l_Field[2].GetUInt32());
                        std::string l_HonorStanding = std::to_string(l_Field[3].GetUInt32());
                        std::string l_HonorLastWeekHK = std::to_string(l_Field[4].GetUInt32());
                        std::string l_HonorLastWeekCP = std::to_string(l_Field[5].GetUInt32());
                        std::string l_HonorStoredHK = std::to_string(l_Field[6].GetUInt32());
                        std::string l_HonorStoredDK = std::to_string(l_Field[7].GetUInt32());

                        CharacterDatabase.PExecute("DELETE FROM characters WHERE guid = '%u'", l_LowGuid);
                        CharacterDatabase.PExecute("DELETE FROM character_action WHERE guid = '%u'", l_LowGuid);
                        CharacterDatabase.PExecute("DELETE FROM character_homebind WHERE guid = '%u'", l_LowGuid);
                        CharacterDatabase.PExecute("DELETE FROM character_inventory WHERE guid = '%u'", l_LowGuid);
                        CharacterDatabase.PExecute("DELETE FROM character_reputation WHERE guid = '%u'", l_LowGuid);
                        CharacterDatabase.PExecute("DELETE FROM character_skills WHERE guid = '%u'", l_LowGuid);
                        CharacterDatabase.PExecute("DELETE FROM character_spell WHERE guid = '%u'", l_LowGuid);
                        CharacterDatabase.PExecute("DELETE FROM item_instance WHERE owner_guid = '%u'", l_LowGuid);

                        delete result;

                        if (!changenth(line, 39, l_HonorRankPoints.c_str()))      // characters.honorRankPoints
                            ROLLBACK(DUMP_FILE_BROKEN);

                        if (!changenth(line, 40, l_HonorHighestRank.c_str()))
                            ROLLBACK(DUMP_FILE_BROKEN);

                        if (!changenth(line, 41, l_HonorStanding.c_str()))
                            ROLLBACK(DUMP_FILE_BROKEN);

                        if (!changenth(line, 42, l_HonorLastWeekHK.c_str()))
                            ROLLBACK(DUMP_FILE_BROKEN);

                        if (!changenth(line, 43, l_HonorLastWeekCP.c_str()))
                            ROLLBACK(DUMP_FILE_BROKEN);

                        if (!changenth(line, 44, l_HonorStoredHK.c_str()))
                            ROLLBACK(DUMP_FILE_BROKEN);

                        if (!changenth(line, 45, l_HonorStoredDK.c_str()))
                            ROLLBACK(DUMP_FILE_BROKEN);
                    }
                    else
                ...
                if (!changenth(line, 61, std::to_string(l_OldGuid).c_str()))
                    ROLLBACK(DUMP_FILE_BROKEN);

                if (!changenth(line, 62, std::to_string(p_OldRealmId).c_str()))
                    ROLLBACK(DUMP_FILE_BROKEN);
    ...
    sLog.out(LOG_CHAR, "Character GUID: %u (Name: %s) from account %u successfully imported from playerdump SQL table.", guid, name.c_str(), account);
    return DUMP_SUCCESS;
}
