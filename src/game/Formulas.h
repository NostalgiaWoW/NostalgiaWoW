    ...
    namespace XP
    {
        ...
        inline uint32 Gain(Player *pl, Unit *u)
        {
            ...
            if (Creature* crea = u->ToCreature())
            {
             ...
             float XP_mod = pl->Forced_X1_XpRate() ? 1.0f : sWorld.getConfig(CONFIG_FLOAT_RATE_XP_KILL);
             return (uint32)(xp_gain*XP_mod);
        }
        ...
        inline uint32 PetGain(Pet *pet, Unit *u)
        {
            ...
            float XP_mod = sWorld.getConfig(CONFIG_FLOAT_RATE_XP_KILL);
            ...
            if (Unit* owner = pet->GetOwner())
                    if (owner->IsPlayer())
                        XP_mod = owner->ToPlayer()->Forced_X1_XpRate() ? 1.0f : sWorld.getConfig(CONFIG_FLOAT_RATE_XP_KILL);
            }
