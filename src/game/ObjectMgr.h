...
enum ConditionType
{
	...
	CONDITION_GENDER_NPC = 40,                   // value1: creature model gender: 0=male, 1=female, 2=none (see enum Gender)
};
...
class ObjectMgr
{
    ...
    public:
        ...
        // Client side antispam
        void LoadClientSpamRegexes(bool reload = false);
        std::vector<std::string> const& GetClientSpamRegexes() const { return m_ClientSpamRegex; }

        std::shared_ptr<EasterStatistic> GetEasterStatisticForPlayer(ObjectGuid playerGuid);
        uint32 GetEasterGeneralSmallEggsCollected();
    ...
    private:
        ...
        typedef std::unordered_map<ObjectGuid, std::shared_ptr<EasterStatistic>> EasterStatisticMap;

        EasterStatisticMap m_mEasterStatistic;

        // Client Regex
        std::vector<std::string> m_ClientSpamRegex;
};
