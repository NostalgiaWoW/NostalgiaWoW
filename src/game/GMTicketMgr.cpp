...
std::string GmTicket::FormatMessageString(ChatHandler& handler, bool detailed, bool p_Comment) const
{
    ...
    for (GmTicketList::const_iterator itr = _ticketList.begin(); itr != _ticketList.end(); ++itr)
        if (!itr->second->IsClosed() && !itr->second->IsCompleted())
            if ((!onlineOnly || itr->second->GetPlayer()) && (!category || (itr->second->GetTicketType() == TicketType(category))))
                handler.SendSysMessage(itr->second->FormatMessageString(handler, false, true).c_str());
}
