            ...
            if (pCaster->GetTypeId() == TYPEID_PLAYER)
            {
               switch (m_creature->GetEntry())
				...
				case ENTRY_SHAYA:
					...
					{
						if (!bIsHealed && !bHasBuff && Spell->Id == SPELL_FORTITUDE_R1)
						{
							DoScriptText(SAY_SHAYA_THANKS, m_creature, pCaster);
							bHasBuff = true;
						}
						else if (!bIsHealed && !bHasBuff && Spell->Id == SPELL_LESSER_HEAL_R2)
						{
							DoScriptText(SAY_SHAYA_THANKS, m_creature, pCaster);
							bIsHealed = true;
						}
						else if ((!bIsHealed && bHasBuff && Spell->Id == SPELL_LESSER_HEAL_R2) || (bIsHealed && !bHasBuff && Spell->Id == SPELL_FORTITUDE_R1))
						{
...
#define GOSSIP_ITEM_RESET_INSTANCE "Yes, reset the instance lockout"
#define ONLY_ONCE "This can be done only once!"
#define GOSSIP_MENU_TEXT 55134

bool GossipHello_npc_instance_reset(Player* p_Player, Creature* p_Creature)
{
    p_Player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, GOSSIP_ITEM_RESET_INSTANCE, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 1);
    p_Player->SEND_GOSSIP_MENU(GOSSIP_MENU_TEXT, p_Creature->GetGUID());
    return true;
}

bool GossipSelect_npc_instance_reset(Player* p_Player, Creature* /*p_Creature*/, uint32 /*uiSender*/, uint32 uiAction)
{
    if (uiAction == GOSSIP_ACTION_INFO_DEF + 1)
    {
        QueryResult* l_Result = CharacterDatabase.PQuery("SELECT `cdReseted` FROM `received_surprise_box` WHERE `charGuid` = '%u'", p_Player->GetGUIDLow());

        if (l_Result)
        {
            Field* l_Field = l_Result->Fetch();

            if (l_Field->GetUInt8() != 0)
            {
                p_Player->CLOSE_GOSSIP_MENU();
                ChatHandler(p_Player).PSendSysMessage(ONLY_ONCE);
                return false;
            }
        }

        ChatHandler(p_Player).HandleInstanceUnbindCommand("all");

        if (!l_Result)
            CharacterDatabase.PExecute("INSERT INTO `received_surprise_box` (`account`, `charGuid`, `cdReseted`) VALUES ('%u', '%u', '1')", p_Player->GetSession()->GetAccountId(), p_Player->GetGUIDLow());
        else
            CharacterDatabase.PExecute("UPDATE `received_surprise_box` SET `cdReseted`='1' WHERE `charGuid` = '%u'", p_Player->GetGUIDLow());
...
/// Custom npc for New Year events
#define NEW_YEAR_ACCEPT_INVITIATION "Accept invitation"
#define NEW_YEAR_LEAVE_ISLAND "Bring me to home"
#define NEW_YEAR_GOSSIP_MENU_TEXT 50010
#define NEW_YEAR_LEAVE_EVENT_TEXT 50011
#define ENTRY_NEW_YEAR_BACK_TELEPORTER 1000010

static const WorldLocation m_GmIsland(1, 16201.635742f, 16210.977539f, 1.134165f, 1.061013f);

bool GossipHello_npc_new_year_teleporter(Player* p_Player, Creature* p_Creature)
{
    if (p_Creature->GetEntry() == ENTRY_NEW_YEAR_BACK_TELEPORTER)
    {
        p_Player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, NEW_YEAR_LEAVE_ISLAND, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 1);
        p_Player->SEND_GOSSIP_MENU(NEW_YEAR_LEAVE_EVENT_TEXT, p_Creature->GetGUID());
    }
    else
    {
        p_Player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, NEW_YEAR_ACCEPT_INVITIATION, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 1);
        p_Player->SEND_GOSSIP_MENU(NEW_YEAR_GOSSIP_MENU_TEXT, p_Creature->GetGUID());
    }
    return true;
}

bool GossipSelect_npc_new_year_teleporter(Player* p_Player, Creature* p_Creature, uint32 /*uiSender*/, uint32 uiAction)
{
    if (uiAction == GOSSIP_ACTION_INFO_DEF + 1)
    {
        if (p_Creature->GetEntry() == ENTRY_NEW_YEAR_BACK_TELEPORTER)
            ...
            p_Player->TeleportTo(m_GmIsland);
    }
...
/// Custom NPC for Orcs VS Humans Tribute Event
#define ARATHIBATTLE_ACCEPT_INVITIATION "Fight!"
#define ARATHIBATTLE_LEAVE "Bring me to home."
#define ARATHIBATTLE_GOSSIP_MENU_TEXT_ALLIANCE 50012
#define ARATHIBATTLE_GOSSIP_MENU_TEXT_HORDE 50014
#define ARATHIBATTLE_LEAVE_EVENT_TEXT 50013
#define ENTRY_ARATHIBATTLE_BACK_TELEPORTER 1000020

static const WorldLocation m_ArathiFarmsHorde(0, -1948.214722f, -4106.866699f, 2.033552f, 5.856365f);
static const WorldLocation m_ArathiFarmsAlliance(0, -1700.985962f, -4237.606934f, 1.998600f, 2.450092f);

bool GossipHello_npc_arathibattle_teleporter(Player* p_Player, Creature* p_Creature)
{
	if (p_Creature->GetEntry() == ENTRY_ARATHIBATTLE_BACK_TELEPORTER)
	{
		p_Player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, ARATHIBATTLE_LEAVE, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 1);
		p_Player->SEND_GOSSIP_MENU(ARATHIBATTLE_LEAVE_EVENT_TEXT, p_Creature->GetGUID());
	}
	else
	{
		if (p_Player->GetTeam() == HORDE)
		{
			p_Player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, ARATHIBATTLE_ACCEPT_INVITIATION, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 1);
			p_Player->SEND_GOSSIP_MENU(ARATHIBATTLE_GOSSIP_MENU_TEXT_HORDE, p_Creature->GetGUID());
		}
		else
		{
			p_Player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, ARATHIBATTLE_ACCEPT_INVITIATION, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 1);
			p_Player->SEND_GOSSIP_MENU(ARATHIBATTLE_GOSSIP_MENU_TEXT_ALLIANCE, p_Creature->GetGUID());
		}
	}
	return true;
}

bool GossipSelect_npc_arathibattle_teleporter(Player* p_Player, Creature* p_Creature, uint32 /*uiSender*/, uint32 uiAction)
{
	if (uiAction == GOSSIP_ACTION_INFO_DEF + 1 && p_Player->getLevel() <= 4)
	{
		if (p_Creature->GetEntry() == ENTRY_ARATHIBATTLE_BACK_TELEPORTER)
			p_Player->TeleportToHomebind();
		else
		{
			if (p_Player->GetTeam() == HORDE)
				p_Player->TeleportTo(m_ArathiFarmsHorde);
			else
				p_Player->TeleportTo(m_ArathiFarmsAlliance);
		}
	}
	else
		ChatHandler(p_Player).PSendSysMessage("The portals are unstable, and only characters level 4 and younger can withstand the stresses of time travel.");
...
// npc_anticrash for Tanaris (we can't fix the vmap problem so we just kick all the players out, meh, no players -- no problems

struct npc_anticrashAI : public Scripted_NoMovementAI
{
    npc_anticrashAI(Creature* pCreature) : Scripted_NoMovementAI(pCreature)
    {
    ...
    void MoveInLineOfSight(Unit *pWho)
    {
        ...
        static const WorldLocation m_crashsafearea(1, -8011.214722f, -5337.866699f, 1.033552f, 2.856365f);
        pPlayer->TeleportTo(m_crashsafearea);
        ChatHandler(pPlayer).PSendSysMessage("You are not allowed to enter this area!");
		me->MonsterYell("This area is off limits!");
...
CreatureAI* GetAI_npc_anticrash(Creature* pCreature)
{
    return new npc_anticrashAI(pCreature);
}

struct npc_blinkdragonAI : ScriptedPetAI
{
    npc_blinkdragonAI(Creature* pCreature) : ScriptedPetAI(pCreature)
    {
        m_creature->SetCanModifyStats(true);
        npc_blinkdragonAI::Reset();
    }
    ...
    void Reset() override
    {
        m_sayTimer = 5 * MINUTE * IN_MILLISECONDS;
    }

    void UpdateAI(const uint32 uiDiff) override
    {
        if (m_sayTimer < uiDiff)
        {
            uint32 random_text = urand(60000, 60005);
            m_creature->MonsterSay(random_text);
            m_sayTimer = 5 * MINUTE * IN_MILLISECONDS;
        }
        else
            m_sayTimer -= uiDiff;
...
CreatureAI* GetAI_npc_blinkdragon(Creature* pCreature)
{
    return new npc_blinkdragonAI(pCreature);
}

// npc_getthefuckoutofhere for all kind of stuff as Crogge requested

struct npc_getthefuckoutofhereAI : public Scripted_NoMovementAI
{
    npc_getthefuckoutofhereAI(Creature* pCreature) : Scripted_NoMovementAI(pCreature)
    {
        pCreature->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE);
        pCreature->ApplySpellImmune(0, IMMUNITY_DAMAGE, SPELL_SCHOOL_NORMAL, true);
        pCreature->ApplySpellImmune(0, IMMUNITY_DAMAGE, SPELL_SCHOOL_MASK_MAGIC, true);
    ...
    void MoveInLineOfSight(Unit *pWho)
    {
        ...
        if (!me->IsWithinDist(pWho, 15.0f, false))
            return;
...
CreatureAI* GetAI_npc_getthefuckoutofhere(Creature* pCreature)
{
    return new npc_getthefuckoutofhereAI(pCreature);
}

struct npc_OgrePetAI : ScriptedPetAI
{
    npc_OgrePetAI(Creature* pCreature) : ScriptedPetAI(pCreature)
    {
        m_creature->SetCanModifyStats(true);
        npc_OgrePetAI::Reset();
    }
    ...
    void UpdateAI(const uint32 uiDiff) override
    {
        if (m_sayTimer < uiDiff)
        {
            uint32 random_text = urand(60100, 60104);
            m_creature->MonsterSay(random_text);
            m_sayTimer = 5 * MINUTE * IN_MILLISECONDS;
        }
        else
...
CreatureAI* GetAI_npc_OgrePet(Creature* pCreature)
{
    return new npc_OgrePetAI(pCreature);
}

struct npc_felguardAI : ScriptedPetAI
{
	npc_felguardAI(Creature* pCreature) : ScriptedPetAI(pCreature)
	{
		m_creature->SetCanModifyStats(true);
		npc_felguardAI::Reset();
	}
	...
	void Reset() override
	{
		m_sayTimer = 4 * MINUTE * IN_MILLISECONDS;
	}
	...
	void UpdateAI(const uint32 uiDiff) override
	{
		if (m_sayTimer < uiDiff)
		{
			uint32 random_text = urand(70100, 70105);
			m_creature->MonsterSay(random_text);
			m_sayTimer = 4 * MINUTE * IN_MILLISECONDS;
		}
		else
...
CreatureAI* GetAI_npc_FelguardPet(Creature* pCreature)
{
	return new npc_felguardAI(pCreature);
}
...
void AddSC_npcs_special()
{
    ...
    */
    ...
    newscript = new Script;
    newscript->Name = "npc_instance_reset";
    newscript->pGossipHello = &GossipHello_npc_instance_reset;
    newscript->pGossipSelect = &GossipSelect_npc_instance_reset;
    newscript->RegisterSelf();

    newscript = new Script;
    newscript->Name = "npc_new_year_teleporter";
    newscript->pGossipHello = &GossipHello_npc_new_year_teleporter;
    newscript->pGossipSelect = &GossipSelect_npc_new_year_teleporter;
    newscript->RegisterSelf();

	newscript = new Script;
	newscript->Name = "npc_arathibattle_teleporter";
	newscript->pGossipHello = &GossipHello_npc_arathibattle_teleporter;
	newscript->pGossipSelect = &GossipSelect_npc_arathibattle_teleporter;
	newscript->RegisterSelf();
    
    newscript = new Script;
    newscript->Name = "npc_anticrash";
    newscript->GetAI = &GetAI_npc_anticrash;
    newscript->RegisterSelf();

	newscript = new Script;
	newscript->Name = "npc_blinkdragon";
	newscript->GetAI = &GetAI_npc_blinkdragon;
	newscript->RegisterSelf();

    newscript = new Script;
    newscript->Name = "npc_getthefuckoutofhere";
    newscript->GetAI = &GetAI_npc_anticrash;
    newscript->RegisterSelf();

    newscript = new Script;
    newscript->Name = "npc_OgrePet";
    newscript->GetAI = &GetAI_npc_OgrePet;
    newscript->RegisterSelf();

	newscript = new Script;
	newscript->Name = "npc_FelguardPet";
	newscript->GetAI = &GetAI_npc_FelguardPet;
	newscript->RegisterSelf();
}
