...
struct boss_ossirianAI : public ScriptedAI
{
    ...
    void Reset()
    {
        ...
        if (Map* map = m_creature->GetMap())
            map->SetWeather(m_creature->GetZoneId(), WeatherType::WEATHER_TYPE_FINE, 0.0f, true);
...
//                    m_creature->SetSpeedRate(MOVE_RUN, 2.0f, true);
    ...
    void Aggro(Unit* pWho)
    {
        ...
        if (Map* map = m_creature->GetMap())
            map->SetWeather(m_creature->GetZoneId(), WeatherType::WEATHER_TYPE_STORM, 1.0f, true);
    }
...
struct ossirian_crystalAI : public GameObjectAI
{
    ...
    bool OnUse(Unit* user)
    {
        ...
        // No idea what the range on the crystals should be.
        const auto crystal_range = 500.f;
        const auto ossirian = me->FindNearestCreature(NPC_OSSIRIAN, crystal_range, true);

        if (!ossirian)
        {
            sLog.outInfo("[OSSIRIAN/Crystal][Inst %03u] ERROR: No Ossirian found (searched within %f yards of guid %u)", user->GetInstanceId(), crystal_range, me->GetGUID());
            return true;
        }

        if (!ossirian->SelectHostileTarget() || !ossirian->getVictim())
        {
            sLog.outInfo("[OSSIRIAN/Crystal][Inst %03u] ERROR: Ossirian not in combat", user->GetInstanceId(), ossirian->GetGUID());
            return true;
        }
