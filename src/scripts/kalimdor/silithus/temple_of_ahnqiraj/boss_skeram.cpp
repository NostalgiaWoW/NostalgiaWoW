...
struct boss_skeramAI : public ScriptedAI
{
    ...
    void JustSummoned(Creature* skeramImage)
    {
        ...
        auto const healthPercent = m_creature->GetHealthPercent();
        auto const mana = m_creature->GetPower(POWER_MANA);
        auto const maxMana = m_creature->GetMaxPower(POWER_MANA);

        // Set health to look like the True Prophet. Will have 12.5%, 25%, finally 50% of max Skeram HP.
        skeramImage->SetMaxHealth(m_creature->GetMaxHealth() * (12.5f / std::max(healthPercent, 0.1f)) * (100 / std::max((int)healthPercent, 1)));
        skeramImage->SetHealthPercent(healthPercent);
        skeramImage->SetPower(POWER_MANA, mana);
        skeramImage->SetMaxPower(POWER_MANA, maxMana);
