    ...
    if (m_uiSavedSpriteDarter >= REQUEST_SAVED_SPRITE_DARTER)
    {
        if (Player* pPlayer = GetLeaderForFollower())
        {
            if (pPlayer->GetQuestStatus(QUEST_FREEDOM_FOR_ALL_CREATURES) == QUEST_STATUS_INCOMPLETE)
            {
                ...
                pPlayer->SetQuestStatus(QUEST_FREEDOM_FOR_ALL_CREATURES, QUEST_STATUS_COMPLETE);
  				      pPlayer->RemoveTimedQuest(QUEST_FREEDOM_FOR_ALL_CREATURES);
                
                uint16 slot = pPlayer->FindQuestSlot(QUEST_FREEDOM_FOR_ALL_CREATURES);
                if (slot < MAX_QUEST_LOG_SIZE)
                {
                    pPlayer->SetQuestSlot(slot, QUEST_FREEDOM_FOR_ALL_CREATURES, 0);
                    pPlayer->SetQuestSlotState(slot, QUEST_STATE_COMPLETE);
                }
            }
        }
