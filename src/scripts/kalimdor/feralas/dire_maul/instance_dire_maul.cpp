...
void instance_dire_maul::OnCreatureCreate(Creature* pCreature)
{
    switch (pCreature->GetEntry())
    {
        case NPC_TRIBUTE:
            ...
            if (GetData(TYPE_KING_DEAD) == DONE)
            {
        ...
        case NPC_KING_GORDOK:
            ...
            if (pCreature->isAlive() && GetData(TYPE_KING_DEAD) == DONE)
            {
                SetData(TYPE_KING_DEAD, NOT_STARTED);
            }
            break;
        case NPC_CHORUSH:
...
void instance_dire_maul::SetData(uint32 uiType, uint32 uiData)
{
    ...
    //SPECIAL means that Tribute counter changed, we want save that always
    if (uiData == SPECIAL || uiData == DONE)
    {
        OUT_SAVE_INST_DATA;
        ...
        saveStream << m_auiEncounter[1] << " " << m_auiEncounter[2] << " " << m_auiEncounter[3] << " " 
                   ...
                   << m_auiEncounter[11]<< " " << m_auiEncounter[12]<< " " << m_uiGuardAliveCount << " "
                   << (m_bIsGordokTributeRespawned ? "1" : "0");
...
void instance_dire_maul::Load(const char* chrIn)
{
    ...
    loadStream >> m_auiEncounter[1] >> m_auiEncounter[2] >> m_auiEncounter[3] >> m_auiEncounter[4]
               ...
               >> m_auiEncounter[10] >> m_auiEncounter[11] >> m_auiEncounter[12] >> m_uiGuardAliveCount
               >> isGordokTributeRespawned;

    m_bIsGordokTributeRespawned = isGordokTributeRespawned != 0;
...
struct npc_arcane_aberrationAI : public ScriptedAI
{
    ...
    void Reset()
    {
        m_uiArcaneBoltTimer = urand(1000, 2000);
...
struct npc_residual_montruosityAI : public ScriptedAI
{
    ...
    void JustDied(Unit* pKiller)
    {
        m_creature->CastSpell(m_creature, SPELL_SUMMON_MANABURSTS, true);
    }
    ...
    void UpdateAI(const uint32 uiDiff)
    {
        ...
        if (m_uiArcaneBlastTimer < uiDiff)
        {
            ...
            m_uiArcaneBlastTimer = urand(15000, 45000);
            return;
        }
        else
...
enum
{
    ...
    SPELL_KING_OF_THE_GORDOK    = 22799,
...
{
    if (pCreature->isQuestGiver() && pPlayer->HasAura(SPELL_KING_OF_THE_GORDOK))
        pPlayer->PrepareQuestMenu(pCreature->GetObjectGuid());
...
struct npc_alzzins_minionAI : ScriptedAI
{
    ...
    void JustDied(Unit* /* pKiller */) override
    {
        if (Unit* pSummoner = m_creature->GetMap()->GetUnit(((TemporarySummon*)m_creature)->GetSummonerGuid()))
        {
            ...
            {
                m_creature->RemoveAllAuras(AURA_REMOVE_BY_DEFAULT);
