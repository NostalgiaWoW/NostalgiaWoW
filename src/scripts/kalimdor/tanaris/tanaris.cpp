    ...
    void UpdateFollowerAI(const uint32 uiDiff)
    {
        if (!m_creature->SelectHostileTarget() || !m_creature->getVictim())
        {
            ...
            else if (HasFollowState(STATE_FOLLOW_INPROGRESS))
            {
                ...
                if (m_uiCheckPlayer <= uiDiff)
                {
                    m_uiCheckPlayer = 5000;

                    Player* pPlayer = m_creature->GetMap()->GetPlayer(GetLeaderGuid());
                    if (!pPlayer || pPlayer->GetQuestStatus(QUEST_TOOGA) != QUEST_STATUS_INCOMPLETE)
                        SetFollowComplete();
                }
                else
                    m_uiCheckPlayer -= uiDiff;
            }
