...
#define GOSSIP_ITEM_TOME_OF_VALOR   "Duthorian, I lost Tome of Valor. Can you help me ?"
#define GOSSIP_ITEM_TOME_OF_DIVINITY   "Duthorian, I lost Tome of Divinity. Can you help me ?"

enum DuthorianGossipMenu
{
    QUEST_THE_TOME_OF_VALOR_1 = 1793,
    QUEST_THE_TOME_OF_VALOR_2 = 1649,
    QUEST_THE_TOME_OF_DIVINITY_1 = 1641,
    QUEST_THE_TOME_OF_DIVINITY_2 = 1642,
    ITEM_TOME_OF_VALOR = 6776,
    ITEM_TOME_OF_DIVINITY = 6775
};

bool GossipHello_npc_duthorian_rall(Player* pPlayer, Creature* pCreature)
{
    if (pCreature->isQuestGiver())
        pPlayer->PrepareQuestMenu(pCreature->GetGUID());

    if (pPlayer->GetQuestStatus(QUEST_THE_TOME_OF_VALOR_1) == QUEST_STATUS_COMPLETE && pPlayer->GetQuestStatus(QUEST_THE_TOME_OF_VALOR_2) != QUEST_STATUS_COMPLETE && !pPlayer->HasItemCount(ITEM_TOME_OF_VALOR, 1, true))
        pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, GOSSIP_ITEM_TOME_OF_VALOR, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF);

    if (pPlayer->GetQuestStatus(QUEST_THE_TOME_OF_DIVINITY_1) == QUEST_STATUS_COMPLETE && pPlayer->GetQuestStatus(QUEST_THE_TOME_OF_DIVINITY_2) != QUEST_STATUS_COMPLETE && !pPlayer->HasItemCount(ITEM_TOME_OF_DIVINITY, 1, true))
        pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, GOSSIP_ITEM_TOME_OF_DIVINITY, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF+1);
...
bool GossipSelect_npc_duthorian_rall(Player* pPlayer, Creature* pCreature, uint32 uiSender, uint32 uiAction)
{
    switch (uiAction)
    {
        case GOSSIP_ACTION_INFO_DEF:
            pPlayer->CLOSE_GOSSIP_MENU();
            pPlayer->AddItem(ITEM_TOME_OF_VALOR, 1);
            break;
        case GOSSIP_ACTION_INFO_DEF+1:
            pPlayer->CLOSE_GOSSIP_MENU();
            pPlayer->AddItem(ITEM_TOME_OF_DIVINITY, 1);
            break;
    }
    return true;
}

enum AngusSternValues
{
    TXT_ALL_HAIL  = 265,
    TXT_BLUE_RECLUSE_HOLDS_A_FEAST = 266,
    TXT_HOORAY = 267,
    GUID_NPC_STEVEN = 90446,
    GUID_NPC_JOACHIM = 52924,
    NPC_STEVEN = 3628,
    NPC_JOACHIM = 1311,
    QUEST_FEAST_AT_THE_BLUE = 1271,
    GO_FEAST_FRUIT = 187990,
    GO_FEAST_GOBLET = 187991,
    GO_FEAST_FISH = 187992,
    GO_FEAST_BOAR = 187993
};

struct GameObjectGuidEntry
{
    uint32 entry;
    uint32 guidLow;
};

static const GameObjectGuidEntry FeastGameobjectIds[] =
{
    { 187990, 1264365 },
    { 187990, 1264359 },
    { 187990, 1264352 },
...
//Giperion Elysium
struct npc_Angus_Stern : public ScriptedAI
{
    npc_Angus_Stern(Creature* creature)
        : ScriptedAI(creature)
    {}
    ...
    void FeastAtTheBlueRecluseCompleted(ObjectGuid PlayerTarget)
    {
        m_eventNum = STAGE_FEASTQUEST;
        m_lastPlayerGuid = PlayerTarget;
    }
    ...
    virtual void UpdateAI(const uint32 deltaTime) override
    {
        ...
        switch (m_eventNum)
        {
        case STAGE_FEASTQUEST:
            m_eventTimer = 4000;
            m_eventNum = STAGE_FEAST1;

            //spawn food around
            //#TODO TBC: Replace that shit to lambda function with type id of 4 feast GO, not a direct guid's
            if (Map* map = m_creature->GetMap())
            {
                if (Player* pl = map->GetPlayer(m_lastPlayerGuid))
                {
                    m_creature->MonsterSay(NOST_TEXT(TXT_ALL_HAIL), 0, pl);
                }
                for (int i = 0; i < sizeof(FeastGameobjectIds) / sizeof(GameObjectGuidEntry); ++i)
                {
                    const GameObjectGuidEntry& iter = FeastGameobjectIds[i];
                    if (GameObject* GO = map->GetGameObject(ObjectGuid(HIGHGUID_GAMEOBJECT, iter.entry, iter.guidLow)))
                    {
                        //GO->Refresh();
        ...
        case STAGE_FEAST1:
            ...
            if (Map* map = m_creature->GetMap())
            {
                if (Player* pl = map->GetPlayer(m_lastPlayerGuid))
                {
                    m_creature->MonsterSay(NOST_TEXT(TXT_BLUE_RECLUSE_HOLDS_A_FEAST), 0, pl);
                }
            }
            m_creature->HandleEmoteCommand(EMOTE_ONESHOT_POINT);
            m_eventNum = STAGE_FEAST2;
            break;
        case STAGE_FEAST2:
        {
            ObjectGuid StevenGuid(HIGHGUID_UNIT, NPC_STEVEN, (uint32)GUID_NPC_STEVEN);
            ObjectGuid JoachimGuid(HIGHGUID_UNIT, NPC_JOACHIM, (uint32)GUID_NPC_JOACHIM);
            if (Creature* pSteven = m_creature->GetMap()->GetCreature(StevenGuid))
            {
                pSteven->HandleEmoteCommand(EMOTE_ONESHOT_CHEER);
                if (Map* map = m_creature->GetMap())
                {
                    if (Player* pl = map->GetPlayer(m_lastPlayerGuid))
                    {
                        pSteven->MonsterSay(NOST_TEXT(TXT_HOORAY), 0, pl);
                    }
                }
            }
            if (Creature* pJoachim = m_creature->GetMap()->GetCreature(JoachimGuid))
            {
                pJoachim->HandleEmoteCommand(EMOTE_ONESHOT_CHEER);
                if (Map* map = m_creature->GetMap())
                {
                    if (Player* pl = map->GetPlayer(m_lastPlayerGuid))
                    {
                        pJoachim->MonsterSay(NOST_TEXT(TXT_HOORAY), 0, pl);
                    }
                }
            }
            m_eventNum = 0;
...
bool QuestRewarded_npc_Angus_Stern(Player* pPlayer, Creature* pCreature, const Quest* pQuest)
{
    if (pQuest->GetQuestId() != QUEST_FEAST_AT_THE_BLUE) return false;
    if (npc_Angus_Stern* AngusAI = dynamic_cast<npc_Angus_Stern*> (pCreature->AI()))
    {
        AngusAI->FeastAtTheBlueRecluseCompleted(pPlayer->GetObjectGuid());
    }
    return true;
}

CreatureAI* Create_npc_Angus_Stern(Creature* creature)
{
    return new npc_Angus_Stern(creature);
}

bool GossipHello_npc_Angus_Stern(Player* player, Creature* creature)
{
    Quest const* pQuest = sObjectMgr.GetQuestTemplate(QUEST_FEAST_AT_THE_BLUE);

    if (!pQuest || !pQuest->IsActive())
        return false;

    QuestStatus status = player->GetQuestStatus(QUEST_FEAST_AT_THE_BLUE);

    if (pQuest->IsAutoComplete() && player->CanTakeQuest(pQuest, false))
    {
        creature->HandleEmoteAfterQuestGossip(EMOTE_ONESHOT_BOW);
    }
...
void AddSC_stormwind_city()
{
    ...
    newscript = new Script;
    newscript->Name = "npc_angus_stern";
    newscript->GetAI = Create_npc_Angus_Stern;
    newscript->pQuestRewardedNPC = QuestRewarded_npc_Angus_Stern;
    newscript->pGossipHello = GossipHello_npc_Angus_Stern;
    newscript->RegisterSelf();
}
