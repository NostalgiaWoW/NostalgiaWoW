...
enum go_termite_barrel_Values
{
    QUEST_A_PLAGUE_UPON_THEE_P3_ALLIANCE = 6389,
    QUEST_A_PLAGUE_UPON_THEE_P3_HORDE    = 6390,
};

struct go_termite_barrel : public GameObjectAI
{
    go_termite_barrel(GameObject* go)
        : GameObjectAI(go)
    {}

    virtual bool IsVisibleForPlayer(const Player* player) const override
    {
        if (player->CanTakeQuest(QUEST_A_PLAGUE_UPON_THEE_P3_ALLIANCE)||
            player->CanTakeQuest(QUEST_A_PLAGUE_UPON_THEE_P3_HORDE))
        {
            return true;
        }
        return false;
...
GameObjectAI* Create_go_termite_barrel (GameObject* go)
{
    return new go_termite_barrel(go);
}

bool QuestRewared_go_lumbermill_crate(Player* player, GameObject* go, Quest const* quest)
{
    player->ProcessRelocationVisibilityUpdates();
    return true;
}
...
void AddSC_western_plaguelands()
{
    ...
    newscript = new Script;
    newscript->Name = "go_termite_barrel";
    newscript->GOGetAI = Create_go_termite_barrel;
    newscript->RegisterSelf();

    newscript = new Script;
    newscript->Name = "go_lumbermill_crate";
    newscript->pQuestRewardedGO = QuestRewared_go_lumbermill_crate;
    newscript->RegisterSelf();
}
