...
struct boss_marliAI : public ScriptedAI
{
    ...
    void Reset()
    {
        ...
        GetGameObjectListWithEntryInGrid(lSpiderEggs, m_creature, SMALL_GO_EGG, DEFAULT_VISIBILITY_INSTANCE);
        if (lSpiderEggs.empty())
            sLog.outDebug("boss_marli, no Eggs with the entry %u were found", SMALL_GO_EGG);
        else
        {
    ...
    GameObject* SelectNextEgg()
    {
        ...
        GetGameObjectListWithEntryInGrid(lEggs, m_creature, SMALL_GO_EGG, DEFAULT_VISIBILITY_INSTANCE);
        if (lEggs.empty())
            sLog.outDebug("boss_marli, no Eggs with the entry %i were found", SMALL_GO_EGG);
        else
        {
