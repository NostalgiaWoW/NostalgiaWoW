...
// Servant of Doomlord: Razelikh 7668, Allistarj 7670, Sevinie 7671, Grol 7669
struct npc_servant_of_doomlordAI : public ScriptedAI
{
    npc_servant_of_doomlordAI(Creature* pCreature) : ScriptedAI(pCreature)
    {
        Reset();
    }
    ...
    {
        if ((damage >= me->GetHealth() || me->HealthBelowPct(10)) && !hasTriggered)
        {
            hasTriggered = true;
            DoCast(me, SPELL_SPIRIT_SHOCK);
        }
    }
...
CreatureAI* GetAI_npc_servant_of_doomlord(Creature* pCreature)
{
    return new npc_servant_of_doomlordAI(pCreature);
}
...
void AddSC_blasted_lands()
{
    ...
    newscript = new Script;
    newscript->Name = "npc_servant_of_doomlord";
    newscript->GetAI = &GetAI_npc_servant_of_doomlord;
    newscript->RegisterSelf();
}
