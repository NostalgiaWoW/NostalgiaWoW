...
struct instance_blackwing_lair : public ScriptedInstance
{
    ...
    virtual void OnPlayerLeave(Player* player) override
    {
        //Giperion Elysium: Don't allow players have Burning Adrenalin, otherwise it will soon explode in a name of Allah
        if (player->HasAura(SPELL_BURNING_ADRENALINE))
        {
            player->RemoveAurasDueToSpell(SPELL_BURNING_ADRENALINE);
        }
        ScriptedInstance::OnPlayerLeave(player);
    }
