...
bool GOHello_go_rune_MC(Player* pPlayer, GameObject* pGo)
{
    if (ScriptedInstance* pInstance = (ScriptedInstance*)pGo->GetInstanceData())
    {
                ...
                pInstance->GetData(TYPE_RAGNAROS) != DONE)
        {
            ...
            const auto majordomo = pPlayer->SummonCreature(NPC_DOMO, 758.089f, -1176.71f, -118.640f, 3.12414f,
                TEMPSUMMON_MANUAL_DESPAWN, 2 * HOUR * IN_MILLISECONDS);

            // TODO Insert better checks here - what happens if this fails?
            if (majordomo != nullptr)
            {
                if (pInstance->GetData(TYPE_MAJORDOMO) != DONE)
                {
                    DoScriptText(-1409004, majordomo);
                }
            }
        }
    }
