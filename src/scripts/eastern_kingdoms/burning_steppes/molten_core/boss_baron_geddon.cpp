...
struct boss_baron_geddonAI : public ScriptedAI
{
    ...
    void Reset()
    {
        m_uiIgniteManaTimer    = urand(18000, 25000);
        m_uiLivingBombTimer    = urand(25000, 40000);
        m_uiInfernoTimer       = urand(28000, 40000);
    ...
    void UpdateAI(const uint32 diff)
    {
        ...
        if (m_uiLivingBombTimer < diff)
        {
            if (Unit* pTarget = m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, nullptr, SELECT_FLAG_PLAYER))
            {
                if (DoCastSpellIfCan(pTarget, SPELL_LIVINGBOMB) == CAST_OK)
                {
                    ...
                    m_uiLivingBombTimer = urand(15000, 25000);
        ...
        if (m_uiIgniteManaTimer < diff)
        {
            if (DoCastSpellIfCan(m_creature, SPELL_IGNITEMANA) == CAST_OK)
                m_uiIgniteManaTimer = urand(25000, 450000);
        }
        else
        ...
        if (m_uiInfernoTimer < diff)
        {
            ...
            {
                m_uiInfernoTimer = urand(18000, 35000);
                InfCount = 0;
                Tick = 1000;
                m_bInferno = true;
