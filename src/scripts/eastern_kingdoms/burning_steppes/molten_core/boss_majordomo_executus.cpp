...
static sSpawnLocation m_aBosspawnLocs[8] =
{
    ...
    {NPC_FLAMEWAKER_HEALER,  752.953f, -1163.94f, -118.869f, 3.70010f},
    {NPC_FLAMEWAKER_HEALER,  738.814f, -1197.40f, -118.018f, 1.83260f},
...
struct boss_majordomoAI : public ScriptedAI
{
    ...
    void Reset()
    {
        ...
        {
            if (m_creature->GetDistance2d(758.089f, -1176.71f) < 2.0f)
            {
                TeleportInstantly = m_pInstance->GetData(TYPE_MAJORDOMO) == DONE;
            }
    ...
    void SummonedCreatureJustDied(Creature* pSummoned)
    {
        ...
        if (!AddListe.empty())
        {
            ...
            {
                if ((*itr)->isAlive() && AddsAlive > 0)
                {
                    const CreatureInfo *cinfo = (*itr)->GetCreatureInfo();
                    (*itr)->SetBaseWeaponDamage(BASE_ATTACK, MINDAMAGE, cinfo->mindmg + cinfo->mindmg / AddsAlive);
                    (*itr)->SetBaseWeaponDamage(BASE_ATTACK, MAXDAMAGE, cinfo->maxdmg + cinfo->maxdmg / AddsAlive);
                }
            }
        }

        if (AddsAlive == 4 && Immune == 0)
        {
            if (!AddListe.empty())
            {
                for (std::list<Creature*>::iterator itr = AddListe.begin(); itr != AddListe.end(); ++itr)
    ...
    void UpdateAI(const uint32 diff)
    {
        if (m_pInstance->GetData(TYPE_MAJORDOMO) == DONE)
        {
            if (TeleportInstantly)
            {
                ...
                m_creature->setFaction(35);
                EnterEvadeMode();
                // Announce the defeat text so players have a visual indicator that the teleport has triggered
                // This is temporary.
                DoScriptText(SAY_DEFEAT, m_creature);
                DomoTP();
                return;
