...
struct npc_GorShakAI : public ScriptedAI
{
    ...
    void Reset()
    {
        //#HACK: Don't reset, if in 3 or 4 phases 
        if (uiPhase > 2 && uiPhase < 5) return;
        uiPhase = 0;
        iSumCreaCnt = 0;
        iSumCreaDead = 0;
        playerGuid.Clear();
...
struct npc_ironhand_guardianAI : public Scripted_NoMovementAI
{
    npc_ironhand_guardianAI(Creature* pCreature) : Scripted_NoMovementAI(pCreature) { }

    uint32 GoutteDeFlammes_Timer = 10 * IN_MILLISECONDS;
    uint32 Saoul_Timer = 1 * IN_MILLISECONDS;
    ...
    void UpdateAI(const uint32 diff) override
    {
        if (GoutteDeFlammes_Timer < diff)
        {
            ...
            GoutteDeFlammes_Timer = 16 * IN_MILLISECONDS;
        }
        else
            GoutteDeFlammes_Timer -= diff;

        if (Saoul_Timer < 30 * IN_MILLISECONDS) //test une seule fois. <= mwai.
        {
            if (Saoul_Timer < diff) //en fait je vois pas à quoi ca sert, et puisque ça interomp....
            {
                ...
                Saoul_Timer = 40 * IN_MILLISECONDS;
            }
            else
                Saoul_Timer -= diff;
        }
