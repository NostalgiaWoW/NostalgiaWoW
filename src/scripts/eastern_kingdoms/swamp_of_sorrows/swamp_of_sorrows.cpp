...
enum Marksen
{
    QUEST_NOTHING_BUT_THE_TRUTH_3 = 1391,
    MODELID_RAVAGEDCORPSE = 1201,
    NOST_TEXTID_FORSAKEN_MISLEAD = 236
};
...
struct npc_infiltrator_marksen : public ScriptedAI
{
    npc_infiltrator_marksen(Creature* npc) : ScriptedAI(npc)
    {
        EventNum = 0;
        Reset();
    }
    ...
    {
        if (EventNum != 4) //if Reset called after scripted death
        {
            EventNum = 0;
        }
        NextEventDelay = 0;
    ...
    virtual void UpdateAI(const uint32 diff) override
    {
        if (EventNum != 0)
        {
            ...
            {
            case 1:
                me->MonsterSay(NOST_TEXT(NOST_TEXTID_FORSAKEN_MISLEAD)); //I believe the Forsaken are misleading the allies of the Horde...
                NextEventDelay = 3 * 1000; //3 sec
                EventNum = 2;
                break;
            case 2:
                //turn into undead
                //...
                me->SetDisplayId(MODELID_RAVAGEDCORPSE);
                NextEventDelay = 3 * 1000; //3 sec
                EventNum = 3;
                break;
            case 3:
                EventNum = 4;
                NextEventDelay = 10;
                me->DealDamage(me, me->GetHealth() + 9000, nullptr, SELF_DAMAGE, SPELL_SCHOOL_MASK_SHADOW, nullptr, false);
                break;
            case 4:
                me->DeMorph();
                EventNum = 0;
    ...
    uint32 EventNum;
    int32 NextEventDelay;
};

bool QuestRewarded_npc_infiltrator_marksen(Player* pPlayer, Creature* pCreature, const Quest* pQuest)
{
    if (pQuest->GetQuestId() == QUEST_NOTHING_BUT_THE_TRUTH_3)
    {
        if (npc_infiltrator_marksen* MarksenAI = dynamic_cast<npc_infiltrator_marksen*>(pCreature->AI()))
        {
            MarksenAI->EventNum = 1;
        }
    }
...
{
    return new npc_infiltrator_marksen(pCreature);
}
...
void AddSC_swamp_of_sorrows()
{
    ...
    newscript = new Script;
    newscript->Name = "npc_infiltrator_marksen";
    newscript->GetAI = &GetAI_npc_infiltrator_marksen;
    newscript->pQuestRewardedNPC = &QuestRewarded_npc_infiltrator_marksen;
    newscript->RegisterSelf();
}
