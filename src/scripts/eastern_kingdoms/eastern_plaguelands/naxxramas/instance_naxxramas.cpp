...
void instance_naxxramas::OnCreatureCreate(Creature* pCreature)
{
    switch (pCreature->GetEntry())
    {
        ...
        case NPC_VIRUSEVENT_JAINA:
        case NPC_VIRUSEVENT_VOLJIN:
        ...
        case TYPE_NOTH:
			...
			if (uiData == FAIL)
			{
				if (sGameEventMgr.IsActiveEvent(172))
					if (Player* iPlayer = GetPlayerInMap())
						if (Creature* naxHelper = GetSingleCreatureFromStorage(iPlayer->GetTeam() == ALLIANCE ? NPC_VIRUSEVENT_JAINA : NPC_VIRUSEVENT_VOLJIN))
                            naxHelper->ForcedDespawn();
			}
            break;
        case TYPE_HEIGAN:
            m_auiEncounter[uiType] = uiData;
        ...
        case TYPE_THADDIUS:
            ...
            else if (uiData == SPECIAL)
            {
                if (Creature* pThaddius = GetSingleCreatureFromStorage(NPC_THADDIUS))
                    pThaddius->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NOT_SELECTABLE);
            }
            ...
            UpdateTeleporters(uiType, uiData);
            SaveToDB(); // doesn`t matter - we should save special state too
            break;
        case TYPE_SAPPHIRON:
            if(uiData == DONE)
...
void instance_naxxramas::OnPlayerDeath(Player* p)
{
    ...
    else if (m_auiEncounter[TYPE_KELTHUZAD] == IN_PROGRESS)
        OnKTPlayerDeath(p);
}
