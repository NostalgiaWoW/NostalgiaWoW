...
struct instance_stratholme : public ScriptedInstance
{
    ...
    bool IsCrystalsDestroyed() const
    {
        return GetData(TYPE_CRISTAL_ALL_DIE) == DONE;
    }
    ...
    void CheckSlaughterDoorSpecialCondition();
    ...
    void Update(uint32 uiDiff)
    {
        ...
        CheckSlaughterDoorSpecialCondition();
...
void instance_stratholme::CheckSlaughterDoorSpecialCondition()
{
    if (!IsCrystalsDestroyed()) return;
    //#GIPERION: Force to check if party on combat, and if not - open portcalis
    if (Player* instancePlayer = GetPlayerInMap(false, false))
    {
        if (Group* playerGroup = instancePlayer->GetGroup())
        {
            bool SomeoneInCombat = false;
            for (GroupReference* pRef = playerGroup->GetFirstMember(); pRef != nullptr; pRef = pRef->next())
            {
                if (Player* groupPlayer = pRef->getSource())
                {
