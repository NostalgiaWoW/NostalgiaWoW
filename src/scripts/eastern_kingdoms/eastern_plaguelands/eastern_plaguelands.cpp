...
enum
{
    ...
    SPELL_DEATH_DOOR        = 23127,        //casted on peasents, damage 5% of HP every 3 sec
    SPELL_QUEST_INVISIBILITY = 23196,       //casted on Eris at spawn, make her invisible
...
struct npc_eris_havenfireAI : public ScriptedAI
{
    ...
    {
        Reset();
        m_creature->CastSpell(m_creature, SPELL_QUEST_INVISIBILITY, true);
    }
    ...
    uint32 PeasentsKilled;
    uint32 PeasentsSaved;
    uint64 PlayerGUID;
    ...
    {
        if ((who->GetTypeId() == TYPEID_PLAYER || who->IsPet()) && CleanerSpawn == false && QuestInProgress == true)
        {
            if (who->GetGUID() != PlayerGUID || who->IsPet())
            {  
                //Gamemaster can enter freely
    ...
    void SummonedMovementInform(Creature* pSummoned, uint32 uiMotionType, uint32 uiPointId)
    {
        ...
        if ((pSummoned->GetEntry() == NPC_PAYSANT_0 || pSummoned->GetEntry() == NPC_PAYSANT_1) && uiPointId == POINT_FIN_EVENT)
        {
			...
			if (Player* player = GetPlayer())
			{
				ChatHandler(player).PSendSysMessage("Peasants saved: %u", PeasentsSaved);
			}
    ...
    void SummonedCreatureJustDied(Creature* pSummoned)
    {
		...
		if (Player* player = GetPlayer())
		{
			ChatHandler(player).PSendSysMessage("Peasants killed: %u", PeasentsKilled);
		}

        if (PeasentsKilled >= 15)
            EventFailed(GetPlayer(), true);
    ...
    void EventFailed(Player* pPlayer, bool npcDespawn)
    {
    ...
    void QuestSuccess(Player* pPlayer)
    {
        if (!pPlayer)
            return;
    ...
    bool IsPlayerInterfering()
    {
        ...
        if (!pl.isEmpty() && myArea && QuestInProgress == true)
        {
    ...
    {
        if (QuestInProgress == false || CleanerSpawn == true)
            return;
    ...
    {
        handler.PSendSysMessage("Quest in Progress: %s", 
            QuestInProgress ? "Yes" : "No");
        handler.PSendSysMessage("Last player guid: %llu", PlayerGUID);
        if (Player* player = GetPlayer())
        {
            handler.PSendSysMessage("\"%s\" lvl: %u", player->GetName(), player->getLevel());
        }
        else
        {
            handler.PSendSysMessage("Last player is offline, info not available");
        }

        handler.PSendSysMessage("Peasants killed: %u, Peasants saved: %u", PeasentsKilled, PeasentsSaved);
        handler.PSendSysMessage("Wave %u", Wave);
        handler.PSendSysMessage("Timer1: %u, Timer2: %u", Timer[0], Timer[1]);
        handler.PSendSysMessage("Buff Timer: %u", BuffTimer);
    }
...
bool QuestAccept_npc_eris_havenfire(Player* pPlayer, Creature* pCreature, const Quest* pQuest)
{
    ...
    if (pQuest->GetQuestId() == QUEST_BALANCE_OF_LIGHT)
        if (npc_eris_havenfireAI* pErisEventAI = dynamic_cast<npc_eris_havenfireAI*>(pCreature->AI()))
            pErisEventAI->EventStart(pPlayer);
    return true;
}
...
struct npc_eris_havenfire_peasantAI : public ScriptedAI
{
    ...
    void SpellHit(Unit* pCaster, const SpellEntry* pSpell)
    {
        ...
        {
            if (!urand(0, 10))
                m_creature->CastSpell(m_creature, SPELL_DEATH_DOOR, true);
        }
        else if (pCaster && pCaster->GetTypeId() == TYPEID_PLAYER)
        {
            ...
            {
                if (pCaster->GetGUID() != pErisEventAI->PlayerGUID && pErisEventAI->QuestInProgress == true && pErisEventAI->CleanerSpawn == false)
                {
                    pErisEventAI->Cleaning();
                }
            }
        }
    }
