...
bool AuthSocket::_HandleLogonProof()
{
    ...
    if (!memcmp(M.AsByteArray().data(), lp.M1, 20) && pinResult && approvedBuild)
    {
        ...
        LoginDatabase.PQuery("INSERT INTO account_log_ip (accountid, ip, source, error) VALUE (%u, '%s', %u, %u)", _accountId, get_remote_address().c_str(), 1, 0);
    ...
    else
    {
        ...
        LoginDatabase.PQuery("INSERT INTO account_log_ip (accountid, ip, source, error) VALUE (%u, '%s', %u, %u)", _accountId, get_remote_address().c_str(), 1, 1);
